...
\begin{verbatimwrite}{./Equations/vCntnPrdTm1.tex}
...
\end{verbatimwrite}
\input{./Equations/vCntnPrdTm1.tex}\unskip
where 
\begin{equation*}\begin{gathered}\begin{aligned}
  \vFunc_{\CntnPrdLsT}(\aNrm)  & \leftassign \DiscFac \vFunc_{\ArvlPrd}(\aNrm) 
                           \equiv \DiscFac \Ex_{\ArvlPrd} \left[\PermGroFacAdjV \vFunc_{\DcsnPrd}(\underbrace{\aNrm \RNrmByG_{\prdT} + \tranShkEmp_{\prdT}}_{{m}_{\prdT}})\right].
    \end{aligned}\end{gathered}\end{equation*}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \vFunc_{\ArvlPrd}(\aNrm)   & = \PermGroFacAdjV\int_{0}^{\infty} \frac{\left(\aNrm \RNrmByG_{\prdt}+ \tranShkEmpDummy\right)^{1-\CRRA}}{1-\CRRA}  d\FDist(\tranShkEmpDummy) \label{eq:NumDefInt}
...
\begin{equation}\begin{gathered}\begin{aligned}
  \cFunc_{\DcsnPrdLsT}(\mNrm)  & = \argmax_{\cNrm} ~~ \uFunc(\cNrm) + \DiscFac \int_{0}^{\infty} \frac{\left((\mNrm - \cNrm) \RNrmByG_{\prdt}+ \tranShkEmpDummy\right)^{1-\CRRA}}{1-\CRRA}  d\FDist(\tranShkEmpDummy) . \label{eq:cFuncTm1}
...
Given any particular value of $\mNrm$, a numerical maximization tool can now find the $\cNrm$ that solves \eqref{eq:vCntnPrdTm1} in a reasonable amount of time.
...
\begin{figure}
  ...
  \caption{$\vFunc_{\prdT-1}$ (solid) versus $\Alt{\vFunc}_{\prdT-1}(\mNrm)$ (dashed)}
...
Piecewise linear `spline' interpolation as described above works well for generating a good approximation to the true optimal consumption function. However, there is a clear inefficiency in the program: Since it uses equation \eqref{eq:vCntnPrdTm1}, for every value of $\mNrm$ the program must calculate the utility consequences of various possible choices of $\cNrm$ (and therefore $\aNrm_{\prdT-1}$) as it searches for the best choice.

For any given index $j$ in $\vctr{m}[j]$, as it searches for the corresponding optimal $a$, the algorithm will end up  calculating $\vFunc_{\CntnPrdLsT}(\tilde{\aNrm})$ for many $\tilde{\aNrm}$ values close to the optimal $a_{\prdT-1}$.  Indeed, even when searching for the optimal $a$ for a \emph{different} $\mNrm$ (say $\vctr{m}[k]$ for $k \neq j$) the search process might compute $\vFunc_{\CntnPrdLsT}(a)$ for an $a$ close to the correct optimal $a$ for $\vctr{m}[j]$. But if that difficult computation does not correspond to the exact solution to the $\vctr{m}[k]$ problem, it is discarded.  
...
\begin{figure}
  ...
  \caption{End-Of-Period Value $\vFunc_{{\cntn}(\prdT-1)}(a_{\prdT-1})$ (solid) versus $\Alt{\vFunc}_{{\cntn}(\prdT-1)}(a_{\prdT-1})$ (dashed)}
...
Nevertheless, the consumption rule obtained when the approximating $\Alt{\vFunc}_{{\cntn}(\prdT-1)}(a_{\prdT-1})$ is used instead of $\vFunc_{{\cntn}(\prdT-1)}(a_{\prdT-1})$ is surprisingly bad, as shown in figure \ref{fig:PlotComparecTm1AB}.  For example, when $\mNrm$ goes from 2 to 3, $\Alt{\cFunc}_{\prdT-1}$ goes from about 1 to about 2, yet when $\mNrm$ goes from 3 to 4, $\Alt{\cFunc}_{\prdT-1}$ goes from about 2 to about 2.05.  The function fails even to be concave, which is distressing because Carroll and Kimball~\citeyearpar{ckConcavity} prove that the correct consumption function is strictly concave in a wide class of problems that includes this one.
...
\begin{figure}
  ...
  \caption{$\uFunc^{\cFunc}(\cFunc)$ versus $\vFunc_{{\cntn}(\prdT-1)}^{\partial}(3-\cFunc), \vFunc_{{\cntn}(\prdT-1)}^{\partial}(4-\cFunc), \Alt{\vFunc}_{{\cntn}(\prdT-1)}^{\partial}(3-\cFunc), \Alt{\vFunc}_{{\cntn}(\prdT-1)}^{\partial}(4-\cFunc)$}
...
Now consider the derivative of $\Alt{\vFunc}_{(\prdT-1)}$.  Because the function is piecewise linear, its derivative $\Alt{\vFunc}_{{\cntn}(\prdT-1)}^{\partial}(a_{\prdT-1})$ is a step function: constant between adjacent gridpoints, with jumps at each gridpoint.

The solid-line step function in Figure \ref{fig:Plot_ud_VS_vCntnd} depicts the actual value of
$\Alt{\vFunc}_{{\cntn}(\prdT-1)}^{\partial}(3-\cNrm)$.  When we attempt to find optimal values of
$\cNrm$ given $\mNrm$ using $\Alt{\vFunc}_{{\cntn}(\prdT-1)}(a_{\prdT-1})$, the numerical optimization routine will
return the $\cNrm$ for which
$\uFunc^{c}(\cNrm) = \Alt{\vFunc}^{\partial}_{{\cntn}(\prdT-1)}(\mNrm-\cNrm)$.  Thus, for
$\mNrm=3$ the program will return the value of $\cNrm$ for which the downward-sloping
$\uFunc^{c}(\cNrm)$ curve intersects with the
$\Alt{\vFunc}_{{\cntn}(\prdT-1)}^{\partial}(3-\cNrm)$; as the diagram shows, this value is exactly equal to 2.
Similarly, if we ask the routine to find the optimal $\cNrm$ for $\mNrm=4$, it finds the point of
intersection of $\uFunc^{c}(\cNrm)$ with $\Alt{\vFunc}_{{\cntn}(\prdT-1)}^{\partial}(4-\cNrm)$; and as the diagram shows, this
...
We would obviously obtain much better estimates of the point of intersection between $\uFunc^{c}(\cNrm)$ and $\vFunc_{{\cntn}(\prdT-1)}^{\partial}(\mNrm-\cNrm)$ if our estimate of $\Alt{\vFunc}^{\partial}_{{\cntn}(\prdT-1)}$ were not a step function.  In fact, we already know how to construct linear interpolations to functions, so the obvious next step is to construct a linear interpolating approximation to the \textit{expected marginal value of end-of-period assets function} at the points in $\vctr{\aNrm}$:
\begin{equation}\begin{gathered}\begin{aligned}
...
and construct
$\Alt{\vFunc}_{{\cntn}(\prdT-1)}^{\partial}(a_{\prdT-1})$ as the linear
...
\begin{figure}
  ...
  \caption{$\vFunc_{{\cntn}(\prdT-1)}^{\partial}(a_{\prdT-1})$ versus $\Alt{\vFunc}_{{\cntn}(\prdT-1)}^{\partial}(a_{\prdT-1})$}
...
\begin{figure}
  ...
  \caption{$\cFunc_{\prdT-1}(\mNrm)$ (solid) Versus Two Methods for Constructing $\Alt{\cFunc}_{\prdT-1}(\mNrm)$}
...
\begin{equation}
  \vFunc^{\dm}_{\DcsnPrdLsT}(\mNrm)  = \left(\frac{\mNrm+1}{2}\right)^{-\CRRA} \label{eq:vPLin}.
...
\begin{equation}\begin{gathered}\begin{aligned}
      ...
      \left[\vFunc^{\dm}_{\DcsnPrdLsT}(\mNrm)\right]^{-1/\CRRA}  & = \frac{\mNrm+1}{2}  .
...
The key insight is that much of the nonlinearity in $\vFunc^{\partial}$ comes from raising to the power $-\CRRA$.  By inverting that operation (raising to $-1/\CRRA$), we can `unwind' it, and the remaining nonlinearity is much smaller.  Specifically, applying the foregoing insights to the end-of-period value function $\vFunc^{\partial}_{\DcsnPrdLsT}(\aNrm)$, we can define an `inverse marginal value' function
...
which would be linear in the perfect foresight case.\footnote{There is a corresponding inverse for the value function: $\vInv_{\cntn}(a_{\prdt})=((1-\CRRA)\vFunc_{\cntn})^{1/(1-\CRRA)}$, and for the marginal marginal value function etc.}  We then construct a piecewise-linear interpolating approximation to the $\vInv_{\prdt}^{\partial}$ function, $\Alt{\vInv}_{\cntn}^{\partial}(a_{\prdt})$, and for any $a$ that falls in the range $\{\vctr{\aNrm}[1],\vctr{\aNrm}[-1]\}$ we obtain our approximation of marginal value from:
\begin{equation}\begin{gathered}\begin{aligned}
      \Alt{\vFunc}_{\dcsn}^{\partial}(a) & =
      [\Alt{\vInv}^{\partial}(a)]^{-\CRRA}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \Alt{\cFunc}_{\cntn}(\aNrm) & \equiv \Alt{\vInv}^{\partial}_{\cntn}(a_{\prdt}) \label{eq:consumedfn}.    
...
\UnifiedNote{[no direct counterpart] — grid evaluation: π-vector from consumed function at xₑ gridpoints}
    \end{aligned}\end{gathered}\end{equation}
with the idea that we will construct an approximation of the consumed function $\Alt{\cFunc}_{{\cntn}(\prdT-1)}(\aNrm)$ as the interpolating function connecting these $\{\vctr{\aNrm},\vctr{c}\}$ points.
...
%\renewcommand{\prdt}{T} 
This is the appropriate moment to ask an awkward question: How should an interpolated, approximated `consumed' function like $\Alt{\cFunc}_{{\cntn}(\prdT-1)}(a_{\prdT-1})$ be extrapolated to return an estimated `consumed' amount when evaluated at an $a_{\prdT-1}$ outside the range spanned by $\{\vctr{\aNrm}[1],...,\vctr{\aNrm}[n]\}$?
...
\Alt{\cFunc}_{{\cntn}(\prdT-1)}^{\partial}(\aVecMin)$ and construct the approximation as the linear extrapolation below $\vctratm[1]$ from:
\begin{equation}\begin{gathered}\begin{aligned}
      \Alt{\cFunc}_{{\cntn}(\prdT-1)}(a)  &  \equiv \cMin + (a-\aVecMin)\varkappa_{1}  \label{eq:ExtrapLin}.
...
\begin{figure}
  ...
  \caption{$\cFunc_{\prdT-1}(m)$ (solid) versus $\Alt{\cFunc}_{\prdT-1}(m)$ (dashed)}
  \label{fig:ComparecTm1AD}
\end{figure}
Figure~\ref{fig:ComparecTm1AD} plots the actual consumption function $\cFunc_{\prdT-1}$ and the approximated consumption function $\Alt{\cFunc}_{\prdT-1}$ derived by the method of endogenous grid points. Compared to the approximate consumption functions illustrated in Figure~\ref{fig:PlotcTm1ABC}, $\Alt{\cFunc}_{\prdT-1}$ is quite close to the actual consumption function.
...
\begin{figure}
  ...
  \caption{$\cFunc_{{\cntn}(\prdT-1)}(\aNrm)$ versus
    $\Alt{\cFunc}_{{\cntn}(\prdT-1)}(\aNrm)$, Multi-Exponential \code{aVec}}
...
\begin{figure}
  ...
  \caption{$\vFunc^{\partial}_{{\cntn}(\prdT-1)}(\aNrm)$ vs.
    $\Alt{\Alt{\vFunc}}_{{\cntn}(\prdT-1)}^{\partial}(\aNrm)$, Multi-Exponential \code{aVec}}
