...
\ifthenelse{\boolean{demacromore}}{
  \usepackage{SolvingMicroDSOPs_demacro_demacro-private}}{} % demacro most things
...
which immediately earns a return factor $R_{t}$.  Simultaneously, the consumer receives noncapital income $\yLvl_{t}$, which is the product of `permanent income' $\pLvl_{t}$ and a transitory shock $\tranShkEmp_{t}$:
\hypertarget{eq-yLvl}{}
\begin{equation}\begin{gathered}\begin{aligned}
      \yLvl_{t} & = \pLvl_{t}\tranShkEmp_{t} \label{eq:yLvl}
      %
      \UnifiedNote{Part of shock space 𝒵ₐᵥ; θ (transitory shock) with E[θ]=1}
    \end{aligned}\end{gathered}\end{equation}
whose expectation is 1 (that is, before realization of the transitory shock, the consumer's expectation is that actual income will on average be equal to permanent income $\pLvl_{t}$).
...
\hypertarget{eq-mLvl}{}
\begin{equation}\begin{gathered}\begin{aligned}
      \mLvl_{t} & = \kLvl_{t}R_{t}+\yLvl_{t} \label{eq:mLvl},
      %
      \UnifiedNote{gₐᵥ: (xₐ, ζₐᵥ) → xᵥ; level: mLvl = kLvl·R + pLvl·θ; unified (no perm inc): m = kR + θ}
    \end{aligned}\end{gathered}\end{equation}
available to be spent on consumption $\cLvl_{t}$ for a consumer subject to a liquidity constraint that requires $\cLvl_{t} \leq \mLvl_{t}$ (though we are not imposing such a constraint yet\ifthenelse{\boolean{shortVersion}}{}{---see subsection~\ref{subsec:LiqConstrSelfImposed}}).  Finally we define
\hypertarget{eq-aLvl}{}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \vFuncLvl_{t}(\mLvl_{t},\pLvl_{t})  & = \frac{\mLvl_{t}^{1-\rho}}{1-\rho}. \label{eq:levelTm1}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \vFuncLvl_{t}(\mLvl_{t},\pLvl_{t}) & = \pLvl_{t}^{1-\rho}\frac{m_{t}^{1-\rho}}{1-\rho}                       \\
...
Because we are dividing $t+1$ level variables by $\pLvl_{t+1}=\PermGroFac_{t+1}\pLvl_{t}$, a normalized return factor emerges:
\hypertarget{eq-RNrmByG}{}
\begin{equation}\begin{gathered}\begin{aligned}
      \RNrmByG_{t+1} & \equiv R/\PermGroFac_{t+1} \label{eq:RNrmByG}.
...
\begin{equation}\begin{gathered}\begin{aligned}
      \vFuncLvl_{t}(\mLvl_{t},\pLvl_{t}) & =  \pLvl_{t}^{1-\rho}v_{t}(m_{t}), \label{eq:vLvlFromvFunc}
\UnifiedNote{recovery map — given the normalized MDP solution 𝒱(m), recover the level MDP value via 𝒱^lvl(mLvl, p) = p^{1−ρ} 𝒱(mLvl/p); the unified framework works with the normalized 𝒱(m) directly}
    \end{aligned}\end{gathered}\end{equation}
and so on back to all earlier periods (by backward induction: if the factorization holds at $t+1$, substituting into the Bellman equation at $t$ and using the homogeneity of CRRA utility yields the same factorization at $t$).  Hence, if we solve the problem \eqref{eq:vNormed} which has only a single state variable $m_{t}$, we can obtain the levels of the value function from \eqref{eq:vLvlFromvFunc}, and of consumption and all other variables from the corresponding permanent-income-normalized solution objects by multiplying each by $\pLvl_{t}$, e.g.\
\begin{equation*}\begin{gathered}\begin{aligned}
...
We have thus reduced the problem from two continuous state variables to one (and thereby enormously simplified its solution).\footnote{In the modular framework of section~\ref{sec:notation},
normalization can be viewed as a transformation on the stage description
itself: each stage's state space, transition functions, and value functions
are rewritten from level variables $(\mLvl, \pLvl)$ to the
permanent-income-normalized counterparts $(m)$, reducing the
dimensionality of every stage uniformly rather than applying a one-off
simplification to the composite Bellman equation.}
...
($k \xrightarrow{\text{shocks}} m \xrightarrow{\text{decision}} a$).
...
($k \xrightarrow{\text{decision}} (k,\shr) \xrightarrow{\text{shocks}} \check{m}$).
...
\begin{center}
    \begin{tabular}{r|c|c|c|l}
      ...
      {\Arrival}      & $ \arvl $ & $k$ & $\vArvl = \Ex_{\Arvl}[\vDcsn]$ & value at entry to {\stg} \\
  ...
  In contrast, items like value functions $v$ or expectations operators $\Ex$ have different meanings at different perches; we capture this using a subscript like $\arvl$.  The fact that all functions in a perch depend on the same state variables (shown in the second column) allows us to write those functions without specifying their arguments.
...
\begin{itemize}
  \item \textbf{k-type} (capital): investable assets \textit{before}
    ...
    before it becomes spendable. Examples: $k$
    (beginning-of-{\interval} capital) and $a$ (end-of-{\interval}
  ...
  \item \textbf{m-type} (market resources): spendable resources
    ...
    available for spending or allocation decisions. Examples: $m$
    (market resources at the {\Decision} {\prch}) and $\check{m}$
...
\begin{itemize}
  \item $\Cnctr(a \leftrightarrow k)$ is well-typed: both are
    k-type.
  \item $\Cnctr(\check{m} \leftrightarrow m)$ is well-typed:
    both are m-type.
  \item $\Cnctr(m \leftrightarrow k)$ would be \textit{illegal}:
    $m$ is m-type and $k$ is k-type. Market resources and
...
\begin{itemize}
  \item \StgName{cons-with-shocks}: arrival $k$ (k-type),
    continuation $a$ (k-type).
  \item \StgName{cons-noshocks}: arrival $m$ (m-type),
    continuation $a$ (k-type).
  \item \StgName{portable}: arrival $k$ (k-type),
    continuation $\check{m}$ (m-type).
...
decoration: we write $\check{m}$ for the \StgName{portable} {\stg}'s
output and $m$ for the \StgName{cons-noshocks} {\stg}'s input.
The within-{\interval} {\Cnct}
$\CnctrComp(\check{m} \leftrightarrow m)$ then renames one to
the other.
...
\texttt{mNrmCheck} for $\check{m}$, \texttt{mNrm} for $m$),
...
\item \textbf{Prior-period {\Continuation} value.}  The formula $\vCntnPrd(a) \leftassign \beta \, \vArvlPrdNxt(a)$, which the builder uses to create the prior {\interval}'s continuation value function from the successor's arrival value.
\item \textbf{Optimization.}  The definition of the intra-{\interval} decision problem $\max_{c} u(c) + \vCntnPrd(m - c)$ that yields the consumption rule $\vArvlPrd$ and the decision-perch value function $\vDcsn$.
...
\begin{equation}
  ...
  \label{eq:pile-def}
\end{equation}
where each $\pile_{t}$ is a solved {\interval} (containing its stage
list, value functions, and policy functions) and each $\Cnctr_{t,t+1}$
...
$(\pile_{t},\, \Cnctr_{t,t+1},\, \pile_{t+1})$ triple
satisfies:
\begin{enumerate}
  \item \textbf{Type consistency.} The continuation-state type of
    $\pile_{t}$'s last {\stg} matches the arrival-state type of
    $\pile_{t+1}$'s first {\stg}, and the {\Cnct} is a valid rename
    between them.
  \item \textbf{Value-function compatibility.} The {\Cnct} and builder
    together produce a continuation value function for $\pile_{t}$
    from the arrival value function of $\pile_{t+1}$.
\end{enumerate}
...
$[\StgName{shocks-only},\, \CnctrComp(\check{m} \leftrightarrow m),\,
...
$\Cnctr(a \leftrightarrow k)$ (both k-type).
...
$v_{t}(m) = u(m)$, consume everything), so initially
$\Pile = \{\pile_{t}\}$. We denote the backward builder as $\BkBldrPrd$.
Applying $\BkBldrPrd$ to the existing $\Pile$ creates a new {\Cnct} and
then the new period's solution, yielding
$\Pile = \{\pile_{t-1},\, \Cnctr_{t-1,t},\, \pile_{t}\}$.
Each subsequent application of $\BkBldrPrd$ prepends one more
$(\pile_{t},\, \Cnctr_{t,t+1})$ pair, growing the {\pileName}
from right to left. Section~\ref{sec:solving-the-next} details the
construction of $\pile_{t-1}$; with multiple {\stgs} or control
...
\begin{equation}\begin{gathered}\begin{aligned}
      % \vCntn & \leftassign \vFunc_{{\cntn}(\prdt)}(\aNrm_{\prdt}) \leftassign \DiscFac \vArvlPrdNxt(\aNrm_{\prdt}) = \DiscFac \Ex_{\ArvlPrdNxt}[\PermGroFac_{\prdt+1}^{1-\CRRA}\vFunc_{\dcsn(\prdt+1)}(\overbrace{a (\Rfree / \PermGroFac_{\prdt+1})+\tranShkEmp_{\prdt+1}}^{\mNrm_{\prdt+1}})],
  v_{{\cntn}(t)}(a_{t}) = \beta \Ex_{\ArvlPrdNxt}[\PermGroFac_{t+1}^{1-\rho}v_{\dcsn(t+1)}(\overbrace{(R / \PermGroFac_{t+1})a_{t} +\tranShkEmp_{t+1}}^{m_{t+1}})], \label{eq:vCntnExpansion}
\end{aligned}\end{gathered}\end{equation}
\ifcode{\marginpar{\scriptsize\texttt{endOfPrd.vCntnPrd}}}{}
...
\begin{equation}\begin{gathered}\begin{aligned}
      u^{\partial}(m_{t}-a_{t}) = \vCntnPrd^{\partial}(a_{t}) & = \Ex_{\ArvlPrdNxt}[\beta \RNrmByG_{t+1}\PermGroFac_{t+1}^{1-\rho}{v}^{\dm}_{\dcsn(t+1)}(m_{t+1})]  \label{eq:upceqEvtp1}
      \\                        & =  \Ex_{\ArvlPrdNxt}[\beta R\phantom{._{t+1}}\PermGroFac_{t+1}^{\phantom{1}-\rho}{v}^{\dm}_{\dcsn(t+1)}(m_{t+1})]
      %
...
\begin{equation}\begin{gathered}\begin{aligned}
      {v}^{\dm}_{\dcsn(t)}(m_{t})  & =  \Ex_{\ArvlPrdNxt} [\beta R \PermGroFac_{t+1}^{-\rho}{v}^{\dm}_{\dcsn(t+1)}(m_{t+1})] \label{eq:envelope}
      %
...
This section examines the second-to-last period in detail, illustrating a number of powerful techniques for speeding and improving its solution; in doing so it illustrates how the {\pileName} $\Pile$ (subsection~\ref{subsec:building-pile}) is built backward. We set $\PermGroFac_{t}=1$ here to reduce clutter.  
...
where 
\begin{equation*}\begin{gathered}\begin{aligned}
  v_{\CntnPrdLsT}(a)  & \leftassign \beta v_{\ArvlPrd}(a) 
                           \equiv \beta \Ex_{\ArvlPrd} \left[\PermGroFacAdjV v_{\DcsnPrd}(\underbrace{a \RNrmByG_{t} + \tranShkEmp_{t}}_{{m}_{t}})\right].
    \end{aligned}\end{gathered}\end{equation*}
The $\leftassign$ signals creation (section~\ref{subsubsec:builders}); here $\vCntn = \vCntnPrd$ by equation~\eqref{eq:last-stg-v-is-end-prd-v}.
\end{comment}
...
%      + \DiscFac \Ex_{\CntnPrdLsT} \left[\PermGroFacAdjV \vFunc_{\DcsnPrd}(\underbrace{(\mNrm-\cNrm)\RNrmByG_{\prdT} + \tranShkEmp_{\prdT}}_{{m}_{\prdT}})\right].
%     \end{aligned}\end{gathered}\end{equation*}
...
%where $\vCntnPrdLsT(\aNrm)$ is the continuation value (see section~\ref{subsubsec:builders}).
...
\begin{equation}\begin{gathered}\begin{aligned}
      v_{\ArvlPrd}(a)   & = \PermGroFacAdjV\int_{0}^{\infty} \frac{\left(a \RNrmByG_{t}+ \tranShkEmpDummy\right)^{1-\rho}}{1-\rho}  d\FDist(\tranShkEmpDummy) \label{eq:NumDefInt}
\UnifiedNote{𝒜₊(k₊) = Γ^{1−ρ} E_θ[𝒱₊((R/Γ)k + θ)]; arrival value of next period (no β); tex vCntnPrd ≡ ℰ_disc = β · 𝒜₊ (disc stage applies β); last-period 𝒱_T = u},
    \end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
  \cFunc_{\DcsnPrdLsT}(m)  & = \argmax_{c} ~~ u(c) + \beta \int_{0}^{\infty} \frac{\left((m - c) \RNrmByG_{t}+ \tranShkEmpDummy\right)^{1-\rho}}{1-\rho}  d\FDist(\tranShkEmpDummy) . \label{eq:cFuncTm1}
...
We now substitute our approximation \eqref{eq:vDiscrete} for $\vCntnPrdLsT(a)$ in \eqref{eq:vCntnPrdTm1} which is simply the sum of $n_{\tranShkEmp}$ numbers and is therefore easy to calculate (compared to the full-fledged numerical integration \eqref{eq:NumDefInt} that it replaces).
...
%   \begin{equation}\begin{gathered}\begin{aligned}
%         \vFunc_{\DcsnPrdLsT}(\mNrm)   & = \max_{\cNrm}
%         \left\{
%           \frac{\cNrm^{1-\CRRA}}{1-\CRRA} +
%           \vFunc_{\DcsnPrd}(\mNrm-\cNrm)
%         \right\}.
%         \label{eq:vCntnPrdTm1}
%       \end{aligned}\end{gathered}\end{equation}
% \input{./Equations/vCntnPrdTm1.tex}
...
Given any particular value of $m$, a numerical maximization tool can now find the $c$ that solves \eqref{eq:vCntnPrdTm1} in a reasonable amount of time.
...
\begin{figure}
  ...
  \caption{$v_{t-1}$ (solid) versus $\Alt{v}_{t-1}(m)$ (dashed)}
...
Piecewise linear `spline' interpolation as described above works well for generating a good approximation to the true optimal consumption function. However, there is a clear inefficiency in the program: Since it uses equation \eqref{eq:vCntnPrdTm1}, for every value of $m$ the program must calculate the utility consequences of various possible choices of $c$ (and therefore $a_{t-1}$) as it searches for the best choice.

For any given index $j$ in $\vctr{m}[j]$, as it searches for the corresponding optimal $a$, the algorithm will end up  calculating $v_{\CntnPrdLsT}(\tilde{a})$ for many $\tilde{a}$ values close to the optimal $a_{t-1}$.  Indeed, even when searching for the optimal $a$ for a \emph{different} $m$ (say $\vctr{m}[k]$ for $k \neq j$) the search process might compute $v_{\CntnPrdLsT}(a)$ for an $a$ close to the correct optimal $a$ for $\vctr{m}[j]$. But if that difficult computation does not correspond to the exact solution to the $\vctr{m}[k]$ problem, it is discarded.  
...
\begin{figure}
  ...
  \caption{End-Of-Period Value $v_{{\cntn}(t-1)}(a_{t-1})$ (solid) versus $\Alt{v}_{{\cntn}(t-1)}(a_{t-1})$ (dashed)}
...
Nevertheless, the consumption rule obtained when the approximating $\Alt{v}_{{\cntn}(t-1)}(a_{t-1})$ is used instead of $v_{{\cntn}(t-1)}(a_{t-1})$ is surprisingly bad, as shown in figure \ref{fig:PlotComparecTm1AB}.  For example, when $m$ goes from 2 to 3, $\Alt{\cFunc}_{t-1}$ goes from about 1 to about 2, yet when $m$ goes from 3 to 4, $\Alt{\cFunc}_{t-1}$ goes from about 2 to about 2.05.  The function fails even to be concave, which is distressing because Carroll and Kimball~\citeyearpar{ckConcavity} prove that the correct consumption function is strictly concave in a wide class of problems that includes this one.
...
\begin{figure}
  ...
  \caption{$u^{\cFunc}(\cFunc)$ versus $v_{{\cntn}(t-1)}^{\partial}(3-\cFunc), v_{{\cntn}(t-1)}^{\partial}(4-\cFunc), \Alt{v}_{{\cntn}(t-1)}^{\partial}(3-\cFunc), \Alt{v}_{{\cntn}(t-1)}^{\partial}(4-\cFunc)$}
...
Now consider the derivative of $\Alt{v}_{(t-1)}$.  Because the function is piecewise linear, its derivative $\Alt{v}_{{\cntn}(t-1)}^{\partial}(a_{t-1})$ is a step function: constant between adjacent gridpoints, with jumps at each gridpoint.

The solid-line step function in Figure \ref{fig:Plot_ud_VS_vCntnd} depicts the actual value of
$\Alt{v}_{{\cntn}(t-1)}^{\partial}(3-c)$.  When we attempt to find optimal values of
$c$ given $m$ using $\Alt{v}_{{\cntn}(t-1)}(a_{t-1})$, the numerical optimization routine will
return the $c$ for which
$u^{c}(c) = \Alt{v}^{\partial}_{{\cntn}(t-1)}(m-c)$.  Thus, for
$m=3$ the program will return the value of $c$ for which the downward-sloping
$u^{c}(c)$ curve intersects with the
$\Alt{v}_{{\cntn}(t-1)}^{\partial}(3-c)$; as the diagram shows, this value is exactly equal to 2.
Similarly, if we ask the routine to find the optimal $c$ for $m=4$, it finds the point of
intersection of $u^{c}(c)$ with $\Alt{v}_{{\cntn}(t-1)}^{\partial}(4-c)$; and as the diagram shows, this
...
and construct
$\Alt{v}_{{\cntn}(t-1)}^{\partial}(a_{t-1})$ as the linear
...
\begin{figure}
  ...
  \caption{$v_{{\cntn}(t-1)}^{\partial}(a_{t-1})$ versus $\Alt{v}_{{\cntn}(t-1)}^{\partial}(a_{t-1})$}
...
\begin{figure}
  ...
  \caption{$\cFunc_{t-1}(m)$ (solid) Versus Two Methods for Constructing $\Alt{\cFunc}_{t-1}(m)$}
...
This is where we unveil our next trick.  To understand the logic, start by considering the case where $\RNrmByG_{t} = \beta = \PermGroFac_{t} = 1$ and there is no uncertainty (that is, we know for sure that income next period will be $\tranShkEmp_{t} = 1$).  The final Euler equation (recall that we are still assuming that $t=t$) is then:
\begin{equation}\begin{gathered}\begin{aligned}
      c_{t-1}^{-\rho}  & = c_{t}^{-\rho}.
...
\begin{equation}
  v^{\dm}_{\DcsnPrdLsT}(m)  = \left(\frac{m+1}{2}\right)^{-\rho} \label{eq:vPLin}.
...
\begin{equation}\begin{gathered}\begin{aligned}
      ...
      \left[v^{\dm}_{\DcsnPrdLsT}(m)\right]^{-1/\rho}  & = \frac{m+1}{2}  .
...
which would be linear in the perfect foresight case.\footnote{There is a corresponding inverse for the value function: $\vInv_{\cntn}(a_{t})=((1-\rho)v_{\cntn})^{1/(1-\rho)}$, and for the marginal marginal value function etc.}  We then construct a piecewise-linear interpolating approximation to the $\vInv_{t}^{\partial}$ function, $\Alt{\vInv}_{\cntn}^{\partial}(a_{t})$, and for any $a$ that falls in the range $\{\vctr{a}[1],\vctr{a}[-1]\}$ we obtain our approximation of marginal value from:
\begin{equation}\begin{gathered}\begin{aligned}
      \Alt{v}_{\dcsn}^{\partial}(a) & =
      [\Alt{\vInv}^{\partial}(a)]^{-\rho}
...
\begin{equation}\begin{gathered}\begin{aligned}
      (\cFunc_{\cntn}(a))^{-\rho} & = \vCntn^{\partial}(a)
      \\ \cFunc_{\cntn}(a) & =      \left(v^{\partial}_{\cntn}(a)\right)^{-1/\rho}.
...
\begin{equation}\begin{gathered}\begin{aligned}
      \Alt{\cFunc}_{\cntn}(a) & \equiv \Alt{\vInv}^{\partial}_{\cntn}(a_{t}) \label{eq:consumedfn}.    
...
\begin{equation}\begin{gathered}\begin{aligned}
      \vctr{c} & = \cFunc_{{\cntn}(t-1)}(\vctr{a}) \label{eq:consumedfnvecs}     
\UnifiedNote{[no direct counterpart] — grid evaluation: π-vector from consumed function at xₑ gridpoints}
    \end{aligned}\end{gathered}\end{equation}
with the idea that we will construct an approximation of the consumed function $\Alt{\cFunc}_{{\cntn}(t-1)}(a)$ as the interpolating function connecting these $\{\vctr{a},\vctr{c}\}$ points.
...
\end{figure}
Figure~\ref{fig:ComparecTm1AD} plots the actual consumption function $\cFunc_{t-1}$ and the approximated consumption function $\Alt{\cFunc}_{t-1}$ derived by the method of endogenous grid points. Compared to the approximate consumption functions illustrated in Figure~\ref{fig:PlotcTm1ABC}, $\Alt{\cFunc}_{t-1}$ is quite close to the actual consumption function.
...
\begin{figure}
  ...
  \caption{$\cFunc_{{\cntn}(t-1)}(a)$ versus
    $\Alt{\cFunc}_{{\cntn}(t-1)}(a)$, Multi-Exponential \code{aVec}}
...
\begin{figure}
  ...
  \caption{$v^{\partial}_{{\cntn}(t-1)}(a)$ vs.
    $\Alt{\Alt{v}}_{{\cntn}(t-1)}^{\partial}(a)$, Multi-Exponential \code{aVec}}
...
In the terminology of section~\ref{subsubsec:builders}, each iteration of this backward loop is an invocation of the backward builder $\BkBldrPrd$: it creates the {\Cnct} (inserted into $\Pile$ between the new and existing period solutions), uses it and the already-solved next {\interval} to perform the creation of $\vCntnPrd$, and solves for the optimal consumption rule.  The dictionary \texttt{cFunc\_life} is the computational embodiment of $\Pile$---the growing structure of solved {\intervals} and {\Cncts} between them assembled by backward induction.
...
\begin{equation}
  \Ex_{t} [{m}_{t+1}/m_{t}] = 1 \text{~if~} m_{t} = \mTrgNrm  \label{eq:mTrgNrmet}
...
\UnifiedNote{[no direct counterpart] — perfect foresight consumption policy c̄(m) used as initial guess for the converged decision rule 𝒱(xᵥ)}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        \\  \Opt{v}^{m}(a_{t})  & = \beta R \PermGroFac_{t+1}^{-\rho} \bar{v}^{m}(\RNrmByG_{t+1} a_{t}+1).
...
$\Alt{\cFunc}_{t}$ by interpolating a finite set of
$(m,c)$ pairs.  A practical problem arises when the
approximation must be evaluated at values of $m$ outside the
...
\begin{itemize}
  ...
  $h_{\Cntn}$ and consumes
  \begin{equation}
    \cFuncAbove_{t}(m) \;=\;
      (m + h_{\Cntn})\,\MPCmin_{t},
      \label{eq:cFuncAboveBrief}
  \end{equation}
  where $\MPCmin_{t}$ is the minimal marginal propensity to
  ...
  \begin{equation}
    \cFuncBelow_{t}(m) \;=\;
      (m - \Min{m}_{t})\,\MPCmin_{t},
      \label{eq:cFuncBelowBrief}
  \end{equation}
  where $\Min{m}_{t}$ is the natural borrowing constraint.
  \item \textbf{Realist.}  The true solution satisfies
  $\cFuncBelow_{t}(m) < \cFunc_{t}(m) <
  \cFuncAbove_{t}(m)$ for all $m$.
\end{itemize}
...
\begin{equation}
  \omega_{t}(m) \;\equiv\;
    \frac{\cFunc_{t}(m) - \cFuncBelow_{t}(m)}
         {\cFuncAbove_{t}(m) - \cFuncBelow_{t}(m)}
    \;\in\; (0,1),
...
in $m$ and therefore easy to interpolate accurately.  Consumption
is recovered via the inverse transformation:
\begin{equation}
  \cFunc_{t}(m) \;=\;
    \cFuncBelow_{t}(m) \;+\;
    \frac{1}{1+e^{-\chi_{t}(m)}}
    \bigl(\cFuncAbove_{t}(m) - \cFuncBelow_{t}(m)\bigr),
  \label{eq:cFromChiBrief}
\end{equation}
where $\chi_{t} \equiv \log\!\bigl(\omega_{t}/(1-\omega_{t})\bigr)$
...
\begin{equation}\begin{gathered}\begin{aligned}
      \vCntn^{\partial a}(a_{t},\shr_{t})  & = \beta \left(\frac{1}{n_{\risky} n_{\tranShkEmp}}\right)\sum_{i=1}^{n_{\tranShkEmp}}\sum_{j=1}^{n_{\risky} }\Rport_{i,j} \left(\cFunc_{t+1}(\Rport_{i,j}a_{t}+\tranShkEmp_{i})\right)^{-\rho}
      \\      \vCntn^{\partial \shr}(a_{t},\shr_{t})  & = \beta \left(\frac{1}{n_{\risky} n_{\tranShkEmp}}\right)\sum_{i=1}^{n_{\tranShkEmp}}\sum_{j=1}^{n_{\risky} }(\Risky_{i,j}-R)\left(\cFunc_{t+1}(\Rport_{i,j}a_{t}+\tranShkEmp_{i})\right)^{-\rho}a_{t}.
...
Assume we have the approximations $\Alt{v}_{\Cntn}^{\partial a}$ and $\Alt{v}_{\Cntn}^{\partial \shr}$ in hand and want to proceed.  Given the formulation in \eqref{eq:Bellmanundated}, nonlinear equation solvers can find the solution to a set of simultaneous equations.  Thus we could ask one to solve
\begin{equation}\begin{gathered}\begin{aligned}
      c_{t}^{-\rho}  & = \Alt{v}^{\partial a}_{{\cntn}(t)}(m_{t}-c_{t},\shr_{t}) %\label{eq:FOCwrtcMultContr}
      \\      0  & = \Alt{v}^{\partial \shr}_{{\cntn}(t)}(m_{t}-c_{t},\shr_{t}) \label{eq:FOCwrtw}
    \end{aligned}\end{gathered}\end{equation}
...
\paragraph{Stage graphs: from lists to directed acyclic graphs.}
The stage lists presented above---e.g.,
$[\StgName{shocks-only},\, \CnctrComp,\, \StgName{cons-noshocks},\, \StgName{disc}]$---are
\textit{linear}: each {\stg} has exactly one predecessor and one successor.
More generally, the {\stgs} within a {\interval} may form a \textit{directed
acyclic graph} (DAG), in which a single predecessor {\stg} fans out into
multiple successor branches that later reconverge. Each branch represents a
distinct contingency or regime that the agent may face, and the {\Arrival}
value function of the branching {\stg} is a probability-weighted combination
of the {\Arrival} values from the branches.

\paragraph{Example: employment branches (expository).}
Consider a consumer who, at the start of each {\interval}, is either
\textit{employed} or \textit{temporarily unemployed}, with probabilities
$p_{\text{emp}}$ and $1 - p_{\text{emp}}$ respectively. The two labor-market
states imply different income processes:
\begin{itemize}
  \item \textbf{Employed branch.} The consumer receives ordinary labor income
    $\tranShkEmp$, so market resources are
    $m^{\text{emp}} = \RNrmByG\,k + \tranShkEmp$.
  \item \textbf{Unemployed branch.} The consumer receives unemployment
    insurance benefits $\mu$ (a fixed fraction of permanent income), so
    market resources are
    $m^{\text{unemp}} = \RNrmByG\,k + \mu$.
\end{itemize}
After the branch-specific income is realized, both branches feed into the
\textit{same} shock-free consumption {\stg} (\StgName{cons-noshocks}),
which chooses $c$ given $m$. The DAG for a single {\interval} is:

\begin{center}
\begin{tabular}{ccccc}
  & & \fbox{\StgName{employed-shocks}} & & \\
  & $\nearrow$ & & $\searrow$ & \\
\fbox{\StgName{branch}} & & & & \fbox{\StgName{cons-noshocks}} $\to$ \fbox{\StgName{disc}} \\
  & $\searrow$ & & $\nearrow$ & \\
  & & \fbox{\StgName{unemployed-shocks}} & & \\
\end{tabular}
\end{center}

\noindent The branching {\stg} has no optimization; it merely routes the
agent into one of the two income-realization {\stgs} with the appropriate
probability. The {\Arrival} value function at the branch point is:
\begin{equation*}
  v_{\text{branch},\arvl}(k)
  \;=\;
  p_{\text{emp}}\,v_{\text{emp},\arvl}(k)
  \;+\;
  (1 - p_{\text{emp}})\,v_{\text{unemp},\arvl}(k).
\end{equation*}
Each branch is itself a valid {\stg} (with its own {\Arrival}, {\Decision},
and {\Continuation} {\prchs}); the only new element is the
probability-weighted fan-out at the branch point and the fan-in (via a
shared {\Cnct}) before the consumption {\stg}. This example is purely
expository---no code implementation is provided---but it illustrates how the
DAG generalization accommodates heterogeneous regimes within a single
{\interval} without duplicating the consumption-{\stg} logic.

\hypertarget{disc-stage}{}
\subsubsection{The Discounting Stage (\StgName{disc})}\label{subsubsec:disc-stage}

The simplest decomposition isolates discounting into a standalone {\stg} that applies for the entire period.  We call it the \StgName{disc} {\stg} (control-name $\beta$).  From equation~\eqref{eq:trns-single-prd}, $\vCntnPrd \leftassign \beta \vArvlPrdNxt$; the factor $\beta$ was implicitly inside the backward builder $\BkBldrPrd$ that created $\vCntn$.  Instead, henceforth at the end of every {\interval},  we put the stub \StgName{disc} {\stg} which has no choice, no shocks, and a trivial decision value function $\vDcsn = \beta \cdot \vCntn$, and we set the discount factor to 1 for every prior stage in the period (leaving all the discounting to the \StgName{disc} stage).\footnote{Equations in sections~\ref{sec:the-problem}--\ref{sec:the-usual-theory}, which predate the modular decomposition, show $\beta$ inside the continuation value of the consumption problem (e.g., \eqref{eq:vNormed}).  That formulation is equivalent: the $\beta$ that appears there is the same discount factor, housed inside a composite continuation value rather than in a separate {\stg}.  The modular decomposition separates what those equations combine.}
...
\begin{table}[h]
\caption{Rosetta Stone: \StgName{disc} Stage Variables}
\label{tab:rosetta-disc}
\begin{center}
\begin{tabular}{l|l|l|l}
Math Symbol & \LaTeX{} Macro & Python Variable & Description \\
\hline
$\bullet$                    & ---                          & \texttt{state}       & Passthrough state (inherits type from predecessor) \\
$\beta$                   & \verb|\beta|              & \texttt{DiscFac}     & Time-preference discount factor \\
$v_{\beta,\dcsn}$    & \verb|v_{\beta,\dcsn}| & \texttt{vDcsn}    & Decision value: $\beta \cdot \vCntn$ (no optimization) \\
$v_{\beta,\cntn}$    & \verb|v_{\beta,\cntn}| & \texttt{vCntn}    & Continuation value function (value at exit) \\
$v_{\beta,\arvl}$    & \verb|v_{\beta,\arvl}| & \texttt{vArvl}    & Arrival value function ($= \vDcsn$; no shocks) \\
\hline
\end{tabular}
\end{center}
\end{table}
...
%  Now we specify the consumption {\stg} to have $\DiscFac = 1$; the \StgName{disc} {\stg} receives $\aNrm$ and the backward builder $\BkBldrPrd$ constructs $\vFunc_{\DiscFac,\dcsn} = \DiscFac \cdot \vFunc_{\DiscFac,\cntn}(\aNrm)$ and $\vArvl=\vDcsn$.
  ...
  The two-{\stg} {\interval} has stage list $[\StgName{cons-with-shocks}, \StgName{disc}]$ (more compactly, $[\cFunc, \beta]$).  The convention that every non-\StgName{disc} {\stg} is undiscounted and every {\interval} ends with \StgName{disc} will mean we do not have to rearrange discounting as we rearrange stages within the period.

\hypertarget{standalone-portfolio-problem}{}
\subsubsection{The Standalone Portfolio Problem}\label{subsec:MCTheory}

Consider the standalone problem of an `investor' choosing the portfolio share $\shr$.  The state variable at the start is $k$ (capital available for investment); the {\Decision} chooses $\shr$; stochastic shocks ($\Risky$, $\tranShkEmp$) are then realized, yielding the {\Continuation} state $\check{m} = \Rport(\shr)k + \tranShkEmp$.  The portfolio share must be chosen \textit{before} the return shock $\Risky$ is realized.  We write $\check{m}$ rather than $m$ for the post-shock state because a strict rule of our framework is the prohibition of multiple timings of a given variable within a period.  Note that $\check{m}$ is m-type (market resources after shocks), matching $m$; the $\check{}$ decoration distinguishes the two timings while preserving the type (see section~\ref{subsec:transitions}).
...
\item \textbf{Fixed-share mode} ($\bar{\shr} \in [0,1]$): The portfolio share is prescribed; no optimization occurs.  The value function is $\vArvl(k, \bar{\shr}) = \Ex_{\arvl}[\vCntn(\Rport(\bar{\shr})k + \tranShkEmp)]$.
\item \textbf{No-risky-investment mode} ($\bar{\shr} = 0$): A special case of fixed-share mode.  All assets earn the riskless return: $\check{m} = R k + \tranShkEmp$.  The risky return $\Risky$ is irrelevant.  The value function reduces to $\vArvl(k, 0) = \Ex_{\arvl}[\vCntn(R k + \tranShkEmp)]$.
\end{itemize}
...
\begin{table}[h]
\caption{Rosetta Stone: \StgName{portable} Stage Variables}
\label{tab:rosetta-portable}
\begin{center}
\begin{tabular}{l|l|l|l}
Math Symbol & \LaTeX{} Macro & Python Variable & Description \\
\hline
$k$             & \verb|k|             & \texttt{kNrm}        & Normalized capital (arrival state, k-type) \\
$\shr$              & \verb|\shr|              & \texttt{Shr}         & Portfolio share in risky asset (control or fixed parameter) \\
$\bar{\shr}$        & \verb|\bar{\shr}|        & \texttt{ShrFixed}    & Prescribed portfolio share ($\bot$ = optimize) \\
$\check{m}$     & \verb|\check{m}|     & \texttt{mNrmCheck}   & Post-shock market resources (continuation state, m-type) \\
$\Rport$            & \verb|\Rport|            & \texttt{Rport}       & Portfolio-weighted return factor: $R + (\Risky - R)\shr$ \\
$\Risky$            & \verb|\Risky|            & \texttt{Risky}       & Stochastic risky-asset return factor \\
$R$            & \verb|R|            & \texttt{Rfree}       & Risk-free return factor \\
$\tranShkEmp$       & \verb|\tranShkEmp|       & \texttt{tranShkEmp}  & Transitory income shock \\
$\RNrmByG$          & \verb|\RNrmByG|          & \texttt{RNrmByG}     & Normalized return factor $R / \PermGroFac$ (used when $\bar{\shr}=0$) \\
$v_{\shr,\dcsn}$ & \verb|v_{\shr,\dcsn}| & \texttt{vDcsn}  & Decision value: $\max_{\shr} \Ex[\vCntn]$ or $\Ex[\vCntn]$ if $\shr$ fixed \\
$v_{\shr,\cntn}$ & \verb|v_{\shr,\cntn}| & \texttt{vCntn}  & Continuation value function (post-shock) \\
$v_{\shr,\arvl}$ & \verb|v_{\shr,\arvl}| & \texttt{vArvl}  & Arrival value function ($= \vDcsn$; no pre-decision shocks) \\
\hline
\end{tabular}
\end{center}
\end{table}
...
\begin{center}
    \begin{tabular}{r|c|c|c|l}
      {\Prch}         & Indicator        & State    & value functions                                  & Explanation    \\ \hline
      {\Arrival}      & $ \arvl $        & $k$  & $\vArvl = \Ex_{\arvl}[\vCntn]$                   & pre-shock value \\
      {\Decision}(s)  & $\sim$           & $k$  & (none)                                            & no choice \\
      {\Continuation} & $ \cntn $        & $\check{m}$  & $\vCntn$                                         & post-shock value \\ \hline
    \end{tabular}
  \end{center}
  \end{table}
\noindent The {\Arrival} value function takes the expectation over the shocks: $v_{\arvl}(k) = \Ex_{\arvl}[v_{\cntn}(\RNrmByG k + \tranShkEmp)]$.  The {\Continuation} state $\check{m} = \RNrmByG k + \tranShkEmp$ is fully determined (non-stochastic) once the shocks are realized.

\hypertarget{consumer-pre-problem}{}
\paragraph{The shock-free consumption stage (\StgName{cons-noshocks}).}\label{subsubsec:consumer-pre-problem}  With shocks in the preceding {\stg}, the consumption {\stg} has {\Arrival} state $m$ and no shocks between {\Arrival} and {\Decision}, so $\vArvl = \vDcsn$.  We call this the \StgName{cons-noshocks} {\stg} (control-name $\cFunc$):
...
\begin{table}[h]
\caption{Rosetta Stone: \StgName{cons-noshocks} Stage Variables}
\label{tab:rosetta-cons-noshocks}
\begin{center}
\begin{tabular}{l|l|l|l}
Math Symbol & \LaTeX{} Macro & Python Variable & Description \\
\hline
$m$        & \verb|m|        & \texttt{mNrm}        & Normalized market resources (arrival state, m-type) \\
$c$        & \verb|c|        & \texttt{cNrm}        & Normalized consumption (control variable) \\
$a$        & \verb|a|        & \texttt{aNrm}        & Normalized end-of-stage assets (continuation state, k-type) \\
$v_{\cFunc,\dcsn}$ & \verb|v_{\cFunc,\dcsn}| & \texttt{vDcsn} & Decision value function: $\max_{c} u(c) + \vCntn(m - c)$ \\
$v_{\cFunc,\cntn}$ & \verb|v_{\cFunc,\cntn}| & \texttt{vCntn} & Continuation value function at stage exit \\
$v_{\cFunc,\arvl}$ & \verb|v_{\cFunc,\arvl}| & \texttt{vArvl} & Arrival value function ($= \vDcsn$; no pre-decision shocks) \\
$u$       & \verb|u|       & \texttt{uFunc}       & CRRA utility function \\
$\rho$        & \verb|\rho|        & \texttt{CRRA}        & Coefficient of relative risk aversion \\
\hline
\end{tabular}
\end{center}
\end{table}
...
If the portfolio share choice is made and stochastic shocks are realized at the \textit{end} of the {\interval}, the shock-free consumption {\stg} comes first and the $\shr$ {\stg} follows: stage list by control-name $[\cFunc,\, \CnctrComp(a \leftrightarrow k),\, \shr,\, \beta]$.  The flow is:
\begin{center}
\begin{tabular}{l|c|l}
  Element & Transition & Action \\ \hline
  \StgName{cons-noshocks} & $m \to a$ & choose $c$ \\
...
The beginning-returns problem places the $\shr$ {\stg} \textit{before} the shock-free consumption {\stg}: stage list by control-name $[\shr,\, \CnctrComp(\check{m} \leftrightarrow m),\, \cFunc,\, \beta]$.  The first-order condition for the portfolio-choice {\stg} is~\eqref{eq:FOCport}.
...
This is the three-stage decomposition from above: $[\shr(0),\, \CnctrComp(\check{m} \leftrightarrow m),\, \cFunc,\, \beta]$.  A life cycle model in which portfolio choice is available at some ages but not others is now trivially constructed: the modeler simply sets $\bar{\shr} = \bot$ for ages with active portfolio choice and $\bar{\shr} = 0$ for ages without.
...
Following the sequential approach outlined in section~\ref{subsec:JointProblem}, we solve the portfolio {\stg} numerically for the optimal $\shr$ at a vector of $\vctr{k}$ and construct an approximated optimal portfolio share function $\Alt{\shr}(k)$ as the interpolating function among the members of the $\{\vctr{k},\vctr{\shr}\}$ mapping.  Having done this, we calculate a vector of values and marginal values at that grid:
\begin{equation}\begin{gathered}\begin{aligned}
...
\UnifiedNote{c⃗ = (ℰ^{\partial}_cons_noshocks(a⃗))^{−1/ρ} [EGM inversion; ℰ_cons_noshocks includes β via disc stage; tex vCntn' ≡ ℰ^{\partial}_cons_noshocks so (vCntn')^{−1/ρ} suffices]}
    \end{aligned}\end{gathered}\end{equation}
which, following \ifthenelse{\boolean{shortVersion}}{the endogenous gridpoints method (EGM)}{the procedure in subsection~\ref{subsec:egm}}, yields an approximated consumption function $\Alt{\cFunc}_{t}(m)$.
...
The upshot is that \emph{exactly the same code} for the shock-free consumption {\stg} and for the $\shr$ {\stg} can be used in any of the three {\interval} types listed in Table~\ref{tab:three-period-types}.  Neither {\stg}'s internal logic changes; only the wiring between them does.  There is even an obvious notation for the two multi-control problems: $v_{t,(\shr\to\cFunc),\arvl}$ can be the {\interval}-arrival value function for the version where the portfolio share is chosen at the beginning of the period, and $v_{t,(\cFunc\to\shr),\arvl}$ is {\interval}-arrival value for the problem where the share choice is at the end.
...
\begin{figure}
  ...
  \caption{$\cFunc_{t-1}(m)$ With Portfolio Choice}
...
agents are dead by $s=91$ as assumed by Cagetti. % Note that in addition to a typical time-invariant discount factor $\beth$, there is a time-varying discount factor $\hat{\DiscFac}_{s}$ in (\ref{eq:lifecyclemax}) which can be used to capture the effect of age-varying demographic variables (e.g.\ changes in family size).
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        %
        \UnifiedNote{defines shock distributions for transitory (Ξ) and permanent (ψ) income shocks}
      \end{aligned}\end{gathered}\end{equation}
...
\begin{table}[h]
  ...
  \begin{center}
    \begin{tabular}{ccl}
      ...
      \\ $\hat{\beta}_{s},\LivPrb_{s}$ & figure \ref{fig:TimeVaryingParam} & Cagetti~\citeyearpar{cagettiWprofiles}
