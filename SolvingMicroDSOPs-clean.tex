...
\begin{equation}\begin{gathered}\begin{aligned}
  ...
  v_{{\cntn}(t)}(a_{t}) = \beta \Ex_{\BegPrdNxt}[\PermGroFac_{t+1}^{1-\rho}v_{\dcsn(t+1)}(\overbrace{(R / \PermGroFac_{t+1})a_{t} +\tranShkEmp_{t+1}}^{m_{t+1}})], \label{eq:vCntnExpansion}
\end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
      u^{\partial}(m_{t}-a_{t}) = \vEndPrd^{\partial}(a_{t}) & = \Ex_{\BegPrdNxt}[\beta \RNrmByG_{t+1}\PermGroFac_{t+1}^{1-\rho}{v}^{\dm}_{\dcsn(t+1)}(m_{t+1})]  \label{eq:upceqEvtp1}
      \\                        & =  \Ex_{\BegPrdNxt}[\beta R\phantom{._{t+1}}\PermGroFac_{t+1}^{\phantom{1}-\rho}{v}^{\dm}_{\dcsn(t+1)}(m_{t+1})]
      %
...
\begin{equation}\begin{gathered}\begin{aligned}
      {v}^{\dm}_{\dcsn(t)}(m_{t})  & =  \Ex_{\BegPrdNxt} [\beta R \PermGroFac_{t+1}^{-\rho}{v}^{\dm}_{\dcsn(t+1)}(m_{t+1})] \label{eq:envelope}
      %
...
\hypertarget{eq-cEuler}{}
  \begin{equation}\begin{gathered}\begin{aligned}
        u^{\partial}(c_{t})  & = \ExEndPrd[\beta R \PermGroFac_{t+1}^{-\rho}u^{\partial}(c_{t+1})] \label{eq:cEuler}.
        %
...
\begin{equation*}\begin{gathered}\begin{aligned}
  v_{\EndPrdLsT}(a)  & \leftassign \beta v_{\BegPrd}(a) 
                           \equiv \beta \Ex_{\BegPrd} \left[\PermGroFacAdjV v_{\MidPrd}(\underbrace{a \RNrmByG_{t} + \tranShkEmp_{t}}_{{m}_{t}})\right].
    \end{aligned}\end{gathered}\end{equation*}
...
\begin{equation}\begin{gathered}\begin{aligned}
      v_{\BegPrd}(a)   & = \PermGroFacAdjV\int_{0}^{\infty} \frac{\left(a \RNrmByG_{t}+ \tranShkEmpDummy\right)^{1-\rho}}{1-\rho}  d\FDist(\tranShkEmpDummy) \label{eq:NumDefInt}
...
\begin{equation}\begin{gathered}\begin{aligned}
  \cFunc_{\MidPrdLsT}(m)  & = \argmax_{c} ~~ u(c) + \beta \int_{0}^{\infty} \frac{\left((m - c) \RNrmByG_{t}+ \tranShkEmpDummy\right)^{1-\rho}}{1-\rho}  d\FDist(\tranShkEmpDummy) . \label{eq:cFuncTm1}
...
\begin{figure}
  ...
  \caption{$u^{\cFunc}(\cFunc)$ versus $v_{{\cntn}(t-1)}^{\partial}(3-\cFunc), v_{{\cntn}(t-1)}^{\partial}(4-\cFunc), \Aprx{v}_{{\cntn}(t-1)}^{\partial}(3-\cFunc), \Aprx{v}_{{\cntn}(t-1)}^{\partial}(4-\cFunc)$}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \Aprx{\cFunc}_{\cntn}(a) & \equiv \Aprx{\vInv}^{\partial}_{\cntn}(a_{t}) \label{eq:consumedfn}.    
...
\UnifiedNote{[no direct counterpart] — grid evaluation: π-vector from consumed function at xₑ gridpoints}
    \end{aligned}\end{gathered}\end{equation}
with the idea that we will construct an approximation of the consumed function $\Aprx{\cFunc}_{{\cntn}(t-1)}(a)$ as the interpolating function connecting these $\{\vctr{a},\vctr{c}\}$ points.
...
%\renewcommand{\prdt}{T} 
This is the appropriate moment to ask an awkward question: How should an interpolated, approximated `consumed' function like $\Aprx{\cFunc}_{{\cntn}(t-1)}(a_{t-1})$ be extrapolated to return an estimated `consumed' amount when evaluated at an $a_{t-1}$ outside the range spanned by $\{\vctr{a}[1],...,\vctr{a}[n]\}$?
...
\Aprx{\cFunc}_{{\cntn}(t-1)}^{\partial}(\aVecMin)$ and construct the approximation as the linear extrapolation below $\vctratm[1]$ from:
\begin{equation}\begin{gathered}\begin{aligned}
      \Aprx{\cFunc}_{{\cntn}(t-1)}(a)  &  \equiv \cMin + (a-\aVecMin)\varkappa_{1}  \label{eq:ExtrapLin}.
...
\begin{figure}
  ...
  \caption{$\cFunc_{{\cntn}(t-1)}(a)$ versus
    $\Aprx{\cFunc}_{{\cntn}(t-1)}(a)$, Multi-Exponential \code{aVec}}
...
\UnifiedNote{c⃗ = (ℰ^{\partial}_cons_noshocks(a⃗))^{−1/ρ} [EGM inversion; ℰ_cons_noshocks includes β via disc stage; tex vCntn' ≡ ℰ^{\partial}_cons_noshocks so (vCntn')^{−1/ρ} suffices]}
    \end{aligned}\end{gathered}\end{equation}
which, following \ifthenelse{\boolean{shortVersion}}{the endogenous gridpoints method (EGM)}{the procedure in subsection~\ref{subsec:egm}}, yields an approximated consumption function $\Aprx{\cFunc}_{t}(m)$.
...
\hypertarget{eq-lifecyclemax}{}
  \begin{equation}\begin{gathered}\begin{aligned}
        v_{\dcsn(t)}(\pLvl_{t},\mLvl_{t}) & =    \max_{\{\cFunc\}_{t}^{T}}~~ u(\cLvl_{t})+\ExEndPrd\left[\sum_{n=1}^{T-t}\beth^{n} \LivPrb_{t}^{t+n}\hat{\beta}_{t}^{t+n} u(\cLvl_{t+n}) \right]   \label{eq:lifecyclemax}
        %
