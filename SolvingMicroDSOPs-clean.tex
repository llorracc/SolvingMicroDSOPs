...
%  }
  ...
  Notice that the value functions at different {\move}s of the {\stg} have distinct state variables.  Only $k$ is known at the beginning of the {\stg}, and other variables take on their values with equations like $b = k \RNrm$ and $m = b+\TranShkEmp.$  We will refer to such within-the-{\stg} creation of variables as {\evltns}.%  Thus, the consumption problem has two {\evltns}: from $\kNrm$ to $\mNrm$ and from $\mNrm$ to $\aNrm$.
...
\begin{equation}\begin{gathered}\begin{aligned}
    ...
    \end{aligned}\end{gathered}\end{equation}
with the idea that we will construct an approximation of the consumed function $\Aprx{\cFunc}_{(t-1)_\cntn}(a)$ as the interpolating function connecting these $\{\vctr{a},\vctr{c}\}$ points.
