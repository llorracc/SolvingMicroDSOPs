...
\usepackage{econark-shortcuts} % econark shortcuts
...
{\yLvl}_{\prd}  & = {\pLvl}_{\prd}\tranShkEmp_{\prd}
...
\begin{frame}[label=NormalizeWhenNot]
...
\begin{frame}[label=NormalizeEndPrd]
\frametitle{Trick: View Everything from End of Period}

Define 
\input{./Equations/vCntnPrddefn.tex}
so 
\begin{equation}\begin{gathered}\begin{aligned}
...
\begin{equation}\begin{gathered}\begin{aligned}
        \vFunc^{\partial}(a_{\prd})  & =  \DiscFac \Rfree \PermGroFac_{\prd+1}^{-\CRRA} \left(\frac{1}{n}\right) \sum_{i=1}^{n} \util^{\partial}\left(\cFunc_{\prd+1}(\RNrmByG_{\prd+1} a_{\prd} + \tranShkEmp_{i})\right)
\end{aligned}\end{gathered}\end{equation}
...
using the FOC:
\input{./Equations/upEqbetaOpNoLabel.tex}
...
\begin{frame}[label=DiscretizeInterp]
\frametitle{Trick: Interpolate a Consumption Rule}
...
\begin{frame}[label=vCntnPrdSlow]
...
evaluations of
\begin{equation}\begin{gathered}\begin{aligned}
        \vFunc^{\partial}_{T-1}(\overbrace{{m}_{T-1}-\cNrm_{T-1}}^{\aNrm_{T-1}})  & =   \DiscFac_{T} \PermGroFac_{T}^{1-\CRRA}\left(\frac{1}{n}\right)\sum_{i=1}^{n}   \left( \RNrmByG_{T} a_{T-1} + \tranShkEmp_{i}\right)^{-\CRRA} \notag
\end{aligned}\end{gathered}\end{equation}
...
\frametitle{Solution: Approximate \$\vCntnPrd\$?}

\pause Given $\{\vctr{\aNrm}_{T-1},\vctr{\vCntnPrd}_{T-1}\}$, an approximate function $\grave{\vCntnPrd}_{T-1}$ 
...
\begin{frame}
\frametitle{Using $\grave{\vCntnPrd}_{T-1}$ In Optimization}
\begin{equation*}\begin{gathered}\begin{aligned}
  \vFunc_{\dcsn(\prd)}(\mNrm_{\prd})  & = \max_{\cNrm_{\prd}}~~ \util(\cNrm_{\prd}) + \grave{\vCntnPrd}_{\prd}(\mNrm_{\prd}-\cNrm_{\prd})
\end{aligned}\end{gathered}\end{equation*}
...
\pause 
\medskip 
From these we can interpolate as before to construct $\grave{\cFunc}_{\prd}(\mNrm)$.
...
\begin{equation}\begin{gathered}\begin{aligned}
  \cFunc_{\prd}(\mNrm)  & = (\mNrm+\hNrm_{\prd})\MPCmin_{\prd} 
\end{aligned}\end{gathered}\end{equation}
for market resources $\mNrm$ and end-of-period human wealth $\hNrm$.
...
& =         \lim_{{\aNrm} \downarrow \Min{a}_{T-1}} \DiscFac \Rfree \PermGroFac_{T}^{-\CRRA} \left(\frac{1}{n}\right) \sum_{i=1}^{n} \left(  \aNrm\RNrmByG_{T}+ \tranShkEmp_{i}\right)^{-\CRRA}
\\  & = \infty
\end{aligned}\end{gathered}\end{equation*}
...
\begin{equation*}\begin{gathered}\begin{aligned}
  \Alt{\cFunc}_{T-1}(\mNrm)  & = \Alt{\cFunc}_{T-1}(\vctr{\mNrm}_{T-1}[1])+\Alt{\cFunc}_{T-1}^{\partial}(\vctr{\mNrm}_{T-1}[1])(\mNrm-\vctr{\mNrm}_{T-1}[1]) \label{eq:ExtrapLin}
\end{aligned}\end{gathered}\end{equation*}

True $\cFunc$ is strictly concave 
$\Rightarrow \exists \mNrm^{-} > \Min{\mNrm}_{T-1}$  for which $\mNrm^{-}-\Alt{\cFunc}_{T-1}(\mNrm^{-}) < \Min{\aNrm}_{T-1}$
...
Theory says that
\begin{equation}\begin{gathered}\begin{aligned}
  \lim_{\mNrm \downarrow \Min{\mNrm}_{T-1}} \cFunc_{T-1}(\mNrm)  & = 0
\\ \lim_{\mNrm \downarrow \Min{\mNrm}_{T-1}} \cFunc_{T-1}^{\mNrm}(\mNrm)  & = \MPCmax_{T-1}
\end{aligned}\end{gathered}\end{equation}
...
\item Above $\mNrm_{T-1}^{\#}$, $\grave{\cFunc}_{T-1}(\mNrm)$ obtained as before
\item Below $\mNrm_{T-1}^{\#}$, $\grave{\cFunc}_{T-1}(\mNrm)=\mNrm$
\end{itemize}
...
\item Interpolate to create $\grave{\cNrm}_{\prd}(\mNrm)$
\end{enumerate}
...
\item Demographic Adjustments to $\DiscFac$
\end{itemize}
\end{frame}
...
\bibliography{SolvingMicroDSOPs-Slides}
\end{frame}
