...
evaluations of
\begin{equation}\begin{gathered}\begin{aligned}
        \vEndStp^{\prime}_{T-1}(\overbrace{{m}_{T-1}-{\cNrm}_{T-1}}^{\aNrm_{T-1}})  & =   \Discount_{T} \PermGroFac_{T}^{1-\CRRA}\left(\frac{1}{n}\right)\sum_{i=1}^{n}   \left( \RNrm_{T} {a}_{T-1} + \TranShkEmp_{i}\right)^{-\CRRA} \notag
\end{aligned}\end{gathered}\end{equation}
...
\begin{frame}
\frametitle{Approximate $\vEndStp_{t}^{\prime}(\aNrm)$?}
...
\item For each $\aNrm[j]$ compute $\vEndStp_{t}^{\prime}(\aNrm[j])$
\end{itemize}
...
$\cNrm[j]$ via FOC:
\begin{equation}\begin{gathered}\begin{aligned}
  \cNrm[j]^{-\CRRA}  & = \vEndStp_{t}^{\prime}(\aNrm[j])
\\ \cNrm[j]  & = \left(\vEndStp_{t}^{\prime}(\aNrm[j])\right)^{-1/\CRRA}
\end{aligned}\end{gathered}\end{equation}
...
\pause 
So computing $\vEndStp_{t}^{\prime}$ at a vector of $\vctr{\aNrm}$ values has produced for us the corresponding $\vctr{\cNrm}$ and $\vctr{\mNrm}$ 
...
\begin{equation}\begin{gathered}\begin{aligned}
  \cEndFunc_{t}(\aNrm)  & = (\vEndStp^{\prime}_{t}(\aNrm))^{-1/\CRRA}
\\ \cEndFunc_{t}^{\aNrm}(\aNrm)  & = -(1/\CRRA)\left(\vEndStp_{t}^{\prime}({a})\right)^{-1-1/\CRRA} \vEndStp_{t}^{\prime\prime}(\aNrm) 
\end{aligned}\end{gathered}\end{equation}
...
\begin{equation*}\begin{gathered}\begin{aligned}
        \lim_{{\aNrm} \downarrow \underline{a}_{T-1}} \vEndStp{T-1}^{\prime}({\aNrm}) 
