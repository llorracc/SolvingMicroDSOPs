...
so 
\begin{equation}\begin{gathered}\begin{aligned}
  \vFunc_{t}(\mRat_{t})  & = \max_{\cNrm_{t}}~~ \util(\cNrm_{t}) + \vEndStp{t}(\mRat_{t}-\cNrm_{t})
\end{aligned}\end{gathered}\end{equation}
with FOC
...
\begin{equation}\begin{gathered}\begin{aligned}
        \vEndStp_{t}^{\prime}({a}_{t})  & =  \Discount \Rfree \PermGroFac_{t+1}^{-\CRRA} \left(\frac{1}{n}\right) \sum_{i=1}^{n} \util^{\prime}\left(\cFunc_{t+1}(\RNrm_{t+1} {a}_{t} + \TranShkEmp_{i})\right)
\end{aligned}\end{gathered}\end{equation}
...
$\util^{\prime}(\cNrm) = \vEndStp^{\prime}_{t}(\mRat[i]-\cNrm)$
\begin{itemize}
...
$\util^{\prime}(\cNrm) = \vEndStp^{\prime}_{t}(\mRat[i]-\cNrm)$ at, say,
6 gridpoints of $\vctr{\mRat}_{T-1}$ may require hundreds or even thousands of
evaluations of
\begin{equation}\begin{gathered}\begin{aligned}
        \vEndStp^{\prime}_{T-1}(\overbrace{{m}_{T-1}-{\cNrm}_{T-1}}^{\aRat_{T-1}})  & =   \Discount_{T} \PermGroFac_{T}^{1-\CRRA}\left(\frac{1}{n}\right)\sum_{i=1}^{n}   \left( \RNrm_{T} {a}_{T-1} + \TranShkEmp_{i}\right)^{-\CRRA} \notag
\end{aligned}\end{gathered}\end{equation}
...
\pause Given $\{\vctr{\aRat}_{T-1},\vctr{\vEnd}_{T-1}\}$, an approximate function $\grave{\vEnd}_{T-1}$ 
...
\begin{frame}
\frametitle{Approximate $\vEndStp_{t}^{\prime}(\aRat)$?}
...
\item For each $\aRat[j]$ compute $\vEndStp_{t}^{\prime}(\aRat[j])$
\end{itemize}

\pause 

Each of these $\vEndStp_{t}^{\prime}[j]$ corresponds to a unique
$\cNrm[j]$ via FOC:
\begin{equation}\begin{gathered}\begin{aligned}
  \cNrm[j]^{-\CRRA}  & = \vEndStp_{t}^{\prime}(\aRat[j])
\\ \cNrm[j]  & = \left(\vEndStp_{t}^{\prime}(\aRat[j])\right)^{-1/\CRRA}
\end{aligned}\end{gathered}\end{equation}
...
\pause 
So computing $\vEndStp_{t}^{\prime}$ at a vector of $\vctr{\aRat}$ values has produced for us the corresponding $\vctr{\cNrm}$ and $\vctr{\mRat}$ 
...
\begin{equation}\begin{gathered}\begin{aligned}
  \cEndFunc_{t}(\aRat)  & = (\vEndStp^{\prime}_{t}(\aRat))^{-1/\CRRA}
\\ \cEndFunc_{t}^{\aRat}(\aRat)  & = -(1/\CRRA)\left(\vEndStp_{t}^{\prime}({a})\right)^{-1-1/\CRRA} \vEndStp_{t}^{\prime\prime}(\aRat) 
\end{aligned}\end{gathered}\end{equation}
...
\begin{equation*}\begin{gathered}\begin{aligned}
        \lim_{{\aRat} \downarrow \underline{a}_{T-1}} \vEndStp{T-1}^{\prime}({\aRat}) 
...
\begin{enumerate}
\item Redefine $\vctr{\aRat}$ {\it relative} to $\underline{\aRat}_{T-1}$
\item Construct corresponding $\vctr{\mRat}_{T-1}$ and $\vctr{\cNrm}_{T-1}$
\item Prepend $\underline{\mRat}_{T-1}$ to $\vctr{\mRat}_{T-1}$
...
\begin{frame}
\frametitle{Trick: Improving the $\aRat$ Grid}
Grid Spacing: Same $\{\underline{\aRat},\bar{\aRat}\}$ But Triple Exponential $e^{e^{e^{...}}}$ Growth
...
\begin{equation*}\begin{gathered}\begin{aligned}
    \util^{\prime}(\mRat_{T-1}^{\#})  & = \vEndStp^{\prime}_{T-1}(0.)
\\  \mRat_{T-1}^{\#}  & = \left(\vEndStp^{\prime}_{T-1}(0.)\right)^{-1/\CRRA}
\end{aligned}\end{gathered}\end{equation*}
\pause\medskip
