...
\eprem +\sigma_{\eprem}^{2}/2$, assumption the assumption about the distribution of shocks guarantees that $\log \Ex[{\tShkEmp}] = 0$ which means that
...
\ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny Go from the first to the last equation in \eqref{eq:FOCTm1} by substituting $\util(c)=c^{-\CRRA}$ and use the approximation to the integral.}}{}
