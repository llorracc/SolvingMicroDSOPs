...
\provideboolean{snippets}\setboolean{snippets}{true}
\setboolean{snippets}{false} 
...
\bibliographystyle{\econtexBibStyle}
  ...
  \noindent  Note: The GitHub {\SMDSOPrepo} associated with this document contains python code that produces all results, from scratch, except for the last section on indirect inference.  The numerical results have been confirmed by showing that the answers that the raw python produces correspond to the answers produced by tools available in the \href{https://econ-ark.org}{Econ-ARK} toolkit, more specifically those in the {\HARKrepo} which has full {\HARKdocs}.  The MSM results at the end have have been superseded by tools in the {\EMDSOPrepo}.
...
\item \textbf{\Decision}: All exogenous variables (like income shocks, rate of return shocks, predictable income growth $\PermGroFac$ have been realized (so that, e.g., $\mNrm_{t}$'s value is known) and the agent solves the optimization problem)
  ...
  \Arrival    & ${\large \arvl}$ & $\vBegStge({\kNrm}_{t})$ & value of entering $t$ (before shocks)
\\  \Decision & (blank/none) & $\vMidStge({\mNrm}_{t})$ & value of $t$-decision (after shocks)
\\  \Continuation & $\cntn$ & $\vEndStge({\aNrm}_{t})$ & value of outgoing state (after decision)
  \end{tabular}
\end{center}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \uFunc^{{c}}({c}_{t})  & = \ExEndStge[\DiscFac \RNrm_{t+1}\PermGroFac_{t+1}^{1-\CRRA}\null{\vFunc}^{{m}}_{\MidStgep}({m}_{t+1})]  \label{eq:upceqEvtp1}
      \\                        & =  \ExEndStge[\DiscFac\Rfree\phantom{._{t+1}}\PermGroFac_{t+1}^{\phantom{1}-\CRRA}\null{\vFunc}^{{m}}_{\MidStgep}({m}_{t+1})]
    \end{aligned}\end{gathered}\end{equation}
and because the \handoutC{Envelope} theorem tells us that
\begin{equation}\begin{gathered}\begin{aligned}
      \null{\vFunc}^{{m}}_{\MidStge}({m}_{t})  & =  \Ex_{\EndStge} [\DiscFac\Rfree\PermGroFac_{t+1}^{-\CRRA}\null{\vFunc}^{{m}}_{\MidStgep}({m}_{t+1})] \label{eq:envelope}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \uFunc^{{c}}({c}_{t+1})  & = \null{\vFunc}^{{m}}_{\MidStgep}({a}_{t}\RNrm_{t+1}+\TranShkEmp_{t+1}) \label{eq:upctp1EqVpxtp1}
    \end{aligned}\end{gathered}\end{equation}
...
\input{./Equations/vBegtpdefn}\unskip %\begin{verbatimwrite}{./Equations/vEndtdefn.tex}   \begin{equation}\begin{gathered}\begin{aligned}         \vEndStge({a}_{\stge})  & = \DiscFac \Ex_{\BegStgep}[\vNormedPermGroFacp\null{\vFunc}_{\MidStgep}(\RNrm_{\stge+1} {a}_{\stge}+{\TranShkEmp}_{\stge+1})]  \label{eq:vEndtdefn}       \end{aligned}\end{gathered}\end{equation} \end{verbatimwrite} \input{./Equations/vEndtdefn.tex}\unskip
...
% \begin{equation}\begin{gathered}\begin{aligned}
%   {\vPBegStgep}({k}_{\stge})  & = \DiscFac \Ex_{\BegStgep}[\Rfree \PermGroFac_{\stge+1}^{-\CRRA} \null{\vFunc}_{\MidStgep}^{{m}}({m}_{\stge})]
% %\\   {\vPEndStge}({a}_{\stge})  & = \DiscFac \Ex_{\BegStgep}[\Rfree \PermGroFac_{\stge+1}^{-\CRRA} \null{\vFunc}_{\MidStgep}^{{m}}({m}_{\stge})]                               
%     \end{aligned}\end{gathered}\end{equation}
% or, substituting from equation (\ref{eq:upctp1EqVpxtp1}) (and dropping function arguments for clarity),
% \begin{equation}\begin{gathered}\begin{aligned}
% %  \vPEndStge({a}_{\stge})  & = \Ex_{\BegStgep}\left[\DiscFac \Rfree \PermGroFac_{\stge+1}^{-\CRRA} \uFunc^{{c}}\left(\cFunc_{\stge+1}(\RNrm_{\stge+1} {a}_{\stge}+{\TranShkEmp}_{\stge+1})\right)\right].  \label{eq:vEndPrimetOc}
%       \vPEndStge\astge  & = \DiscFac \Ex_{\BegStgep}\left[\Rfree \PermGroFac_{\stge+1}^{-\CRRA} \uFunc^{{c}}\left(\cFunc_{\stge+1}\mstge\right)\right] .  \label{eq:vEndPrimetOfc}                             
% \\                               & =  \DiscFac \Ex_{\BegStgep}[\vFunc^{{k}}_{\MidStgep}({k}_{\stge})]
%     \end{aligned}\end{gathered}\end{equation}
Finally, note for future use that since ${k}_{t+1}={a}_{t}$ and $\vEndStge({a}_{t})=\DiscFac \vBegStgep({{k}_{t+1}})$, the first order condition 
...
% PermGroFac clutters everything up; hide it if HideG is true
...
\begin{equation*}\begin{gathered}\begin{aligned}
      ...
      \DiscFac \Ex_{\EndStgem} \left[\vNormedPermGroFac{\vFunc}_{\MidStge}(\underbrace{({m}_{\stge-1}-{c}_{\stge-1})\RNrm_{\stge} + \TranShkEmp_{\stge}}_{{m}_{\stge}})\right],
\end{aligned}\end{gathered}\end{equation*}
where $\Ex_{\EndStgem}$  indicates that the expectation is taken as of the end of period $\stge-1$.
...
Substituting into our definition of $\vEndStgem$,
\begin{verbatimwrite}{./Equations/vDiscrete.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
        \vFunc_{\EndStgem}({a}_{\stge-1})  & =   \DiscFac \vNormedPermGroFac\left(\frac{1}{n_{\TranShkEmp}}\right)\sum_{i=1}^{n_{\TranShkEmp}}   \frac{\left(\RNrm_{\stge} {a}_{\stge-1} + \TranShkEmp_{i}\right)^{1-\CRRA}}{1-\CRRA} \label{eq:vDiscrete}
      \end{aligned}\end{gathered}\end{equation}
\end{verbatimwrite}%\renewcommand{\stge}{t}
\input{./Equations/vDiscrete}\unskip
so we can rewrite the problem that defines the middle stage of the period {$\dcsn$} as
...
\lstinputlisting{./Code/Python/snippets/equiprobable-max-using.py}\unskip

shows the key portion of the code that corresponds to equation \eqref{eq:vEndTm1}.
}{}

\begin{comment}
    % In the {\SMDSOPntbk} notebook, the section ``Discretization of the Income Shock Distribution'' provides code which instantiates the \texttt{DiscreteApproximation} class defined in the \texttt{resources} module. This class creates a 7-point discretization of the continuous log-normal distribution of transitory shocks to income by utilizing seven points, where the mean value is $-.5 \sigma^2$, and the standard deviation is $\sigma = .5$.

In the {\SMDSOPntbk} notebook, the section ``Discretization of the Income Shock Distribution'' 
...
Given a particular value of ${m}_{T-1}$, a numerical maximization routine can now find the ${c}_{T-1}$ that approximately maximizes \eqref{eq:vEndTm1} in a reasonable amount of time.

\ifthenelse{\boolean{ToFix}}{{\marginpar{\tiny See comments in .tex.}}{}}




The heart of the code responsible for computing an estimated consumption function begins in ``Solving the Model by Value Function Maximization,'' where a vector characterizing a set of possible values of market resources %
is arbitrarily initialized to the integers $[0., 1., 2., 3., 4.]$, and for each of these particular \texttt{mVec\_int} values, the consumption values ${\mathtt{cVec}}$ that solve the minimization problem equivalent to \eqref{eq:vEndTm1} are computed. %
...
Given a set of points on a function (in this case, the consumption function $\cFunc_{T-1}(\mNrm)$), we can create a piecewise linear `interpolating function' (a `spline') which when applied to an input ${m}$ will yield the value of $\cNrm$ that corresponds to a linear `connect-the-dots' interpolation of the value of $\cNrm$ from the nearest calculated points, creating a function that is an approximation of the function whose points have been sampled. We call the approximated consumption function $\Alt{\cFunc}$, and $\Alt{\cFunc}({m})-\cFunc({m})$ is the approximation error at any ${m}$. 

In {\SMDSOPntbk} this is accomplished in ``An Interpolated Consumption Function.'' %

When called with an ${m}_{T-1}$ that is equal to one of the points in \texttt{mVec\_int}, $\Alt{\cFunc}_{T-1}$ returns the associated value of $\vct{{c}}_{\mathtt{T-1}}$, and when called with a value of ${m}_{T-1}$ that is not exactly equal to one of the \texttt{mVec\_int}, returns the value of ${c}$ that reflects a linear interpolation between the $\vct{{c}}_{\mathtt{T-1}}$ points associated with the two \texttt{mVec\_int} points immediately above and below ${m}_{T-1}$.  %Thus if the function is called with ${m}_{T-1} = 1.75$ and the nearest gridpoints \ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny Go to the optimal consumption figure and show the connect-the-dots; point out that it's more obvious for the ${\vFunc}_{T-1}$.}}{} are ${m}_{j,T-1}=1$ and ${m}_{k,T-1}=2$ then the value of ${c}_{T-1}$ returned by the function would be $(0.25 {c}_{j,T-1}+0.75 {c}_{k,T-1})$. We can define a numerical approximation to the value function $\Alt{\vFunc}_{T-1}({m}_{T-1})$ in an exactly analogous way.
...
The approach above works well for generating a good approximation to the true optimal consumption function. However, there is a clear inefficiency in the program: Since it uses equation \eqref{eq:vEndTm1}, for every value of ${m}_{T-1}$ the program must calculate the utility consequences of many possible choices of $\cNrm_{T-1}$ as it searches for the best choice for that particular $\mNrm_{T-1}$.

For any given value of ${a}_{T-1}$, there is a good chance that the program may end up calculating the corresponding $\vMidStgem$ many times while maximizing utility from different ${m}_{T-1}$'s.  For example, it is possible that the program will calculate the value of ending the period with ${a}_{T-1}=0$ dozens of times.  It would be much more efficient if the program could make the calculation of $\vMidStgem(0.)$ once and then merely recall the value when it is needed again.
...
Define a grid of possible values for end-of-period assets at time $T-1$,
$\vct{{a}}_{\mathtt{T-1}}$
(\texttt{aVec}
in the code). %, designating the specific points $\vct{{a}}\mathtt{[i]_{T-1}}$.
%For each of these values in $\vct{{a}}_{\mathtt{T-1}}$,
Next, construct $\vct{v}_{\mathtt{\MidStgem}} =
\vFunc_{\MidStgem}(\vct{{a}}_{\mathtt{T-1}})$ using equation
\eqref{eq:vEndTm1}; then construct an approximating function 
$\Alt{\vFunc}_{\EndStgem}({a}_{T-1})$ by passing the lists \texttt{aVec} and \texttt{vVec} as arguments.
These lists contain the points of the $\vct{{a}}_{\mathtt{\stge-1}}$ and $\vct{v}_{\mathtt{\stge-1}}$ vectors, respectively.
...
of assets.\footnote{What we are doing here is essentially a distillation of the 
...
\begin{figure}
  ...
  \caption{End-Of-Period Value $\vFunc_{\EndStgem}({a}_{T-1})$ (solid) versus $\Alt{\vFunc}_{\EndStgem}({a}_{T-1})$ (dashed)}
...
Nevertheless, the consumption rule obtained when the approximating $\Alt{\vFunc}_{\EndStgem}({a}_{T-1})$
...
instead of $\vFunc_{\EndStgem}({a}_{T-1})$ is surprisingly bad, as
...
\ifthenelse{\boolean{ToFix}}{{\marginpar{\tiny In all figs, replace $\mathfrak{v}$ with the $\vEndStge$ notation}}{}}
...
\begin{figure}
  ...
  \caption{$\uFunc^{{c}}(c)$ versus $\vFunc_{\EndStgem}^{{a}}(3-c), \vFunc_{\EndStgem}^{{a}}(4-c), \Alt{\vFunc}_{\EndStgem}^{{a}}(3-c), \Alt{\vFunc}_{\EndStgem}^{{a}}(4-c)$}
  \label{fig:PlotuPrimeVSOPrime}
\end{figure}

In the notebook, the ``Value Function versus the First Order Condition'' section finds the values of consumption which satisfy the first order condition in \eqref{eq:FOCTm1}.%

\ifthenelse{\boolean{snippets}}{
\lstinputlisting{./Code/Python/snippets/marginal-value-interp.py}\unskip
}{}
...
$\Alt{\vFunc}_{\EndStgem}({a}_{T-1})$.  Because we have constructed
$\Alt{\vFunc}_{\EndStgem}$ as a linear interpolation, the slope of
$\Alt{\vFunc}_{\EndStgem}({a}_{T-1})$ between any two adjacent
...
$\Alt{\vFunc}_{\EndStgem}({a}_{T-1})$ follows a step function.

The solid-line step function in Figure \ref{fig:PlotuPrimeVSOPrime}
depicts the actual value of
$\Alt{\vFunc}_{\EndStgem}^{{a}}(3-{c}_{T-1})$.  When we attempt to find
optimal values of ${c}_{T-1}$ given ${m}_{T-1}$ using
$\Alt{\vFunc}_{\EndStgem}({a}_{T-1})$, the numerical optimization
routine will return the ${c}_{T-1}$ for which $\uFunc^{{c}}({c}_{T-1}) =
\Alt{\vFunc}^{{a}}_{\EndStgem}({m}_{T-1}-{c}_{T-1})$.  Thus, for
...
$\Alt{\vFunc}_{\EndStgem}^{{a}}(3-{c}_{T-1})$; as the diagram shows,
...
$\Alt{\vFunc}_{\EndStgem}^{{a}}(4-{c}_{T-1})$; and as the diagram shows,
...
$\vFunc_{\EndStgem}^{{a}}({m}_{T-1}-{c}_{T-1})$ if our estimate of
$\Alt{\vFunc}^{{a}}_{\EndStgem}$ were not a step function.  In
...
linear interpolating approximation to the \textit{expected marginal
  value of end-of-period assets function} $\vFunc^{{a}}_{\EndStgem}$:
\begin{equation}\begin{gathered}\begin{aligned}
      \vFunc_{\EndStgem}^{{a}}({a}_{T-1})  & =  \DiscFac \Rfree \uNormedPermGroFac \left(\frac{1}{n_{\TranShkEmp}}\right) \sum_{i=1}^{n_{\TranShkEmp}} \left(\RNrm_{T} {a}_{T-1} + \TranShkEmp_{i}\right)^{-\CRRA} \label{eq:vEndPrimeTm1}
    \end{aligned}\end{gathered}\end{equation}
at the points in \texttt{aVec} yielding $\vct{v}\mathtt{^{a}_{\EndStgem}}$ (the vector of expected end-of-period-$(T-1)$ marginal values of assets corresponding to \texttt{aVec}),  %$\{\{\vct{{a}}}\mathtt{_{T-1}},\vFunc_{\EndStgem}^{{a}}(\vct{{a}}[1]_{T-1}\},\{\vct{{a}}_{(T-1)},\vFunc_{\EndStgem}^{{a}}\}\ldots\}$
 and construct $\Alt{\vFunc}_{\EndStgem}^{{a}}({a}_{T-1})$ as the linear
...
\begin{figure}
  ...
  \caption{$\vFunc_{\EndStgem}^{{a}}({a}_{T-1})$ versus $\Alt{\vFunc}_{\EndStgem}^{{a}}({a}_{T-1})$}
...
Figure \ref{fig:PlotOPRawVSFOC} shows the results.  The linear
...
nonlinearity of $\vFunc_{\EndStgem}^{{a}}({a}_{T-1})$ at
...
future income) of $\hNrm_{\EndStgem}=1$ (because it is the value of human wealth as of the end of the period, and there is only one more period of income of 1 left).
...
${m}_{T-1}+\hNrm_{\MidStgem}= {m}_{T-1}+1$, and since
${\vFunc}_{\MidStge}^{{m}}({m}_{T-1}) = \uFunc^{{c}}({c}_{T-1})$ this
implies that \ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny Crucial
    ...
    function earlier.}}{}
\begin{equation}
  \vFunc^{{m}}_{\MidStgem}({m}_{T-1})  = \left(\frac{{m}_{T-1}+1}{2}\right)^{-\CRRA} \label{eq:vPLin}.
\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
      [\vFunc^{{m}}_{\MidStgem}({m}_{T-1})]^{-1/\CRRA}  & = \frac{{m}_{T-1}+1}{2}  .
    \end{aligned}\end{gathered}\end{equation}
...
to the end-of-period value function $\vFunc^{{a}}_{\MidStgem}({a}_{T-1})$, we can define
\begin{equation}\begin{gathered}\begin{aligned}
      \cFunc_{\EndStgem}({a}_{T-1})  & \equiv  \left(\vFunc^{{a}}_{\EndStgem}({a}_{T-1})\right)^{-1/\CRRA} \label{eq:cGoth}
    \end{aligned}\end{gathered}\end{equation}
...
of the $\vct{{a}}_{\mathtt{\stge-1}}$ gridpoints, with the idea that we will construct
$\Alt{\cFunc}_{\EndStgem}$ as the interpolating function connecting
these
points.
...
neglected: How should a function like $\Alt{\cFunc}_{\EndStgem}$
be evaluated outside the range of points spanned by
$\{\mathtt{{a}_{T-1}[1],...,{a}_{T-1}[n]}\}$ for which we have calculated
the corresponding $\cFunc_{\EndStgem}$ gridpoints used to produce our
linearly interpolating approximation $\Alt{\cFunc}_{\EndStgem}$?

\newcommand{\aVecMin}{a_{1}}\newcommand{\cMin}{c_{1}}
The easiest answer would be linear extrapolation; for example, if the bottommost gridpoint is $\aVecMin = \vct{{a}}\mathtt{_{T-1}[0]}$ and the corresponding level of consumption is $\cMin = \cFunc_{\EndStgem}(a_0)$ we could calculate the `marginal propensity to have consumed' $\varkappa_{1}=\Alt{\cFunc}_{\EndStgem}^{{a}}(\aVecMin)$ and construct the approximation as the linear extrapolation below $\aVecMin$:
\begin{equation}\begin{gathered}\begin{aligned}
      \Alt{\cFunc}_{\EndStgem}({a}_{T-1})  &  \equiv \cMin + ({a}_{T-1}-\aVecMin)\varkappa_{1}  \label{eq:ExtrapLin}
    \end{aligned}\end{gathered}\end{equation}
for values of ${a}_{T-1} < \aVecMin$ %, where $\Alt{\cFunc}_{\EndStgem}^{{a}}(\aVecMin)$ is the derivative of the $\Alt{\cFunc}_{\EndStgem}$ function at the bottommost gridpoint.
To see that this approach will lead us into difficulties, consider what happens to the true (not approximated) $\vFunc^{{a}}_{\EndStgem}({a}_{T-1})$ as ${a}_{T-1}$ approaches the value
$\underline{a}_{T-1}=-\underline{\TranShkEmp}\RNrm_{T}^{-1}$.  From
\eqref{eq:vEndPrimeTm1} we have
\begin{equation}\begin{gathered}\begin{aligned}
      \lim_{{a}_{T-1} \downarrow \underline{a}_{T-1}} \vFunc^{{a}}_{\EndStgem}({a}_{T-1})
...
and undefined marginal utility).\ifthenelse{\boolean{ToFix}}{{\marginpar{\tiny See comments in .tex.}}{}}
...
$\vFunc^{{a}}_{\EndStgem}$ function is approximated by a piecewise
linear function like $\Alt{\vFunc}^{{m}}_{\EndStgem}$, because there is no chance that the linear extrapolation below $a_{1}$ will correctly predict $\vFunc^{{a}}_{\EndStgem}(\underline{a}_{T-1})=\infty.$ To see what
will happen instead, note first that if we are approximating $\vFunc^{{a}}_{\EndStgem}$ the smallest value in
...
approximating $\vFunc^{{a}}_{\EndStgem}$ function is evaluated at
...
\cFunc_{\EndStgem}({a}_{T-1}) = (\vFunc^{{a}}_{\EndStgem}({a}_{T-1}))^{-1/\CRRA} = 0$;
...
augment each of the $\vct{a}_{T-1}$ and $\vct{c}_{T-1}$ vectors with an extra point so that the
...
\begin{figure}
  ...
  \caption{True $\cFunc_{\EndStgem}({a}_{T-1})$ vs its approximation $\Alt{\cFunc}_{\EndStgem}({a}_{T-1})$}
  \label{fig:GothVInvVSGothC}
\end{figure}


From there, we plot the lists that have been prepended with the natural borrowing contraint and the associated minimal level of consumption. Figure\ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny True $\cEndFunc$ is solid, linear approx is dashed.}}{} \ref{fig:GothVInvVSGothC} shows the result. The solid line calculates the exact numerical value of the consumed function $\cFunc_{\EndStgem}({a}_{T-1})$ while the dashed line is the linear interpolating approximation $\Alt{\cFunc}_{\EndStgem}({a}_{T-1}).$ This figure illustrates the value of the transformation: The true function is close to linear, and so the linear approximation is almost indistinguishable from the true function except at the very lowest values of ${a}_{T-1}$.

Figure~\ref{fig:GothVVSGothCInv} similarly shows that when we generate
$\Alt{\Alt{\vFunc}}_{\EndStgem}^{{a}}({a})$ using our augmented 
$[\Alt{\cFunc}_{\EndStgem}({a})]^{-\CRRA}$ (dashed line) we
obtain a \textit{much} closer approximation to the true function
$\vFunc^{{a}}_{\EndStgem}({a})$ (solid line) than we did in
the previous program which did not do the
transformation (Figure~\ref{fig:PlotOPRawVSFOC}). (The vertical axis label uses $\mathfrak{v}^{\prime}$ as an alternative notation for what in these notes we designate as $\vFunc^{a}_{\EndStgem}$). \ifthenelse{\boolean{ToFix}}{{\marginpar{\tiny In all figs, replace $\mathfrak{v}$ with the $\vEndStge$ notation}}{}}


\hypertarget{GothVVSGothCInv}{}
\begin{figure}
  \centerline{\includegraphics[width=6in]{./Figures/GothVVSGothCInv}}
  \caption{True $\vFunc^{{a}}_{\EndStgem}({a}_{T-1})$ vs. $\Alt{\Alt{\vFunc}}_{\EndStgem}^{{a}}({a}_{T-1})$ Constructed Using $\Alt{\cFunc}_{\EndStgem}({a}_{T-1})$}
...
point in $\vct{{m}}\mathtt{_{T-1}}$ (\texttt{mVec} in the code), to use a
numerical rootfinding algorithm to search for the value of ${c}_{T-1}$
that solves $\uFunc^{{c}}({c}_{T-1}) =
\vFunc^{{a}}_{\EndStgem}({m}_{T-1}-{c}_{T-1})$.  Unfortunately, rootfinding
is a notoriously computation-intensive (that is, slow!) operation.


It turns out that there is a way to completely skip the rootfinding step.  The method can be understood by noting that any arbitrary value of $\vct{\null{a}}\null{_{T-1}}$ (greater than its lower bound value $\aVecMin$) will be associated with \textit{some} marginal valuation as of the continuation ($\cntn$) stage of $T-1$ (that is, at the end of the period), and the further observation that it is trivial to find the value of ${c}$ that yields the same marginal valuation, using the first order condition \eqref{eq:upEqbetaOp},
\begin{equation}\begin{gathered}\begin{aligned}
      \uFunc^{{c}}({\vct{{c}}\mathtt{_{T-1}}})  & =
                                       \vFunc^{{a}}_{\EndStgem}(\vct{{a}}_{T-1}) \label{eq:eulerTm1}
\end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
       {\vct{{c}}\mathtt{_{T-1}}}  & = \left(\vFunc^{{a}}_{\EndStgem}(\vct{{a}}_{T-1})\right)^{-1/\CRRA}
%      \\  & = (\vFunc^{{a}}_{\EndStgem}({a}_{T-1,i}))^{-1/\CRRA}
%      \\  & \equiv  \cFunc_{\EndStgem}({a}_{T-1,i})
%      \\  & \equiv  \cFunc_{\EndStgem,i}
    \end{aligned}\end{gathered}\end{equation}
...
But with mutually consistent values of $\vct{{c}}\mathtt{_{T-1}}$ and $\vct{{a}}\mathtt{_{T-1}}$ (consistent, in the sense that they are the unique optimal
values that correspond to the solution to the problem), we can
obtain the $\vct{{m}}\mathtt{_{T-1}}$ vector that corresponds to both of them from
\begin{equation}\begin{gathered}\begin{aligned}
      \vct{{m}}\mathtt{_{T-1}}  & = {\vct{{\cNrm}}\mathtt{_{T-1}}+\vct{{a}}\mathtt{_{T-1}}}.
    \end{aligned}\end{gathered}\end{equation}
...
\ifthenelse{\boolean{ToFix}}{{\marginpar{\tiny the Gothic class needs to be renamed to something like \texttt{EndPrd}}}{}}
...
\texttt{mVec\_egm}. With the $\vct{{c}}_{\mathtt{\stge-1}}$ values in hand, the notebook can generate a set of $\vct{{m}}\mathtt{_{T-1}}$ and ${\vct{{c}}\mathtt{_{T-1}}}$
pairs that can be interpolated between in order to yield
$\Alt{\cFunc}_{\MidStgem}(m_{\stge-1})$ at virtually zero computational cost!\footnote{This is
...
One might worry about whether the $\{\vct{m},\vct{c}\}$ points obtained in this way will provide a
...
the approximated $\Alt{\cFunc}_{\EndStgem}$ function tends to be farthest from its true
value $\cFunc_{\EndStgem}$ at low values of ${a}$.  Combining this with our insight that
...
\begin{figure}
  ...
  \caption{$\cFunc_{\EndStgem}({a}_{T-1})$ versus
    $\Alt{\cFunc}_{\EndStgem}({a}_{T-1})$, Multi-Exponential \texttt{aVec}}
...
\begin{figure}
  ...
  \caption{$\vFunc^{{a}}_{\EndStgem}({a}_{T-1})$ vs.
    $\Alt{\Alt{\vFunc}}_{\EndStgem}^{{a}}({a}_{T-1})$, Multi-Exponential \texttt{aVec}}
...
\begin{verbatimwrite}{./cctwMoM/MoM-Prelims.tex}
  As a preliminary to our solution, define $\hNrm_{\EndStge}$ as
...
\begin{verbatimwrite}{./Equations/cFuncAbove.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
        \cFuncAbove_{t}(\mNrm_{t})  & = (\mNrm_{t} + \hNrm_{\EndStge})\MPCmin_{t} \label{eq:cFuncAbove}
      \end{aligned}\end{gathered}\end{equation}
...
\begin{verbatimwrite}{./cctwMoM/MoM-Words.tex}
  We similarly define $\hEndMin_{\EndStge}$ as `minimal human wealth,' the
  ...
  level of market resources is $\ushort{m}_{t} = -\hEndMin_{\EndStge}$, because
  ...
  \begin{equation*}\begin{gathered}\begin{aligned}
        \aboveMin \mNrm_{t}  & = {m}_{t}+\overbrace{\hEndMin_{\EndStge}}^{=-\ushort{m}_{t}}
        \\  \aboveMin \hNrm_{\EndStge}  & = \hNrm_{\EndStge}-\hEndMin_{\EndStge}.
      \end{aligned}\end{gathered}\end{equation*}
  ...
  wealth to be equal to its minimum feasible value $\hEndMin_{\EndStge}$ with certainty, so
  consumption is given by the perfect foresight solution
  \begin{equation*}\begin{gathered}\begin{aligned}
        \cFuncBelow_{t}(m_{t})  & = ({m}_{t}+\hEndMin_{\EndStge})\MPCmin_{t}
  ...
  \begin{equation*}\begin{gathered}\begin{aligned}
        \cFuncAbove_{t}(m_{t})  & = ({m}_{t} +\hEndMin_{\EndStge} - \hEndMin_{\EndStge} + \hNrm_{\EndStge} )\MPCmin_{t}
        \\    & = (\aboveMin \mNrm_{t} + \aboveMin \hNrm_{\EndStge})\MPCmin_{t}
        \\      & = \cFuncBelow_{t}(m_{t})+\aboveMin \hNrm_{\EndStge} \MPCmin_{t}
        .
      \end{aligned}\end{gathered}\end{equation*}
...
\mbox{    \begin{tabular}{rcl}
      $ \aboveMin \mNrm_{t} \MPCmin_{t} < $ & $ \cFunc_{t}(\ushort{m}_{t}+\aboveMin \mNrm_{t}) $  $< (\aboveMin \mNrm_{t}+\aboveMin \hNrm_{\EndStge})\MPCmin_{t} $
      \\  $- \aboveMin \mNrm_{t} \MPCmin_{t} > $ & $ -\cFunc_{t}(\ushort{m}_{t}+\aboveMin \mNrm_{t}) $ & $> -(\aboveMin \mNrm_{t}+\aboveMin \hNrm_{\EndStge})\MPCmin_{t} $
      \\  $ \aboveMin \hNrm_{\EndStge} \MPCmin_{t} > $ & $ \bar{\cFunc}_{t}(\ushort{m}_{t}+\aboveMin \mNrm_{t})-\cFunc_{t}(\ushort{m}_{t}+\aboveMin \mNrm_{t}) $ & $> 0$
      \\  $1 > $ & $ \underbrace{\left(\frac{\bar{\cFunc}_{t}(\ushort{m}_{t}+\aboveMin \mNrm_{t})-\cFunc_{t}(\ushort{m}_{t}+\aboveMin \mNrm_{t})}{\aboveMin \hNrm_{\EndStge} \MPCmin_{t}}\right)}_{\equiv \Hi{\koppa}_{t}} $ & $> 0$
\end{tabular}
}
  \end{center}
\end{verbatimwrite}
...
\begin{verbatimwrite}{./Equations/MoM-KoppaOfMu.tex}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        \Hi{\koppa}_{t}(\mu_{t})   & \equiv  \left(\frac{\bar{\cFunc}_{t}(\ushort{m}_{t}+e^{\mu_{t}})-\cFunc_{t}(\ushort{m}_{t}+e^{\mu_{t}})}{\aboveMin \hNrm_{\EndStge} \MPCmin_{t}}\right), \label{eq:koppa}
      \end{aligned}\end{gathered}\end{equation}
  and we now define
  \begin{equation}\begin{gathered}\begin{aligned}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \Hi{\koppa}_{t}^{\mu}(\mu_{t})   & = (\aboveMin \hNrm_{\EndStge} \MPCmin_{t})^{-1}e^{\mu_{t}}\left(\MPCmin_{t}-\overbrace{\cFunc^{\mNrm}_{t}(\ushort{m}_{t}+e^{\mu_{t}})}^{\equiv \MPCFunc_{t}(\mNrm_{t})}\right)  \label{eq:koppaPrime}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \Hi{\chiFunc}_{t}^{\mu}  & = \left(\frac{\MPCmin_{t} \aboveMin \mNrm_{t} \aboveMin \hNrm_{\EndStge} (\MPCmin_{t}-\MPC_{t})}
        {(\cFuncAbove_{t}-\cFunc_{t})(\cFuncAbove_{t}-\cFunc_{t} - \MPCmin_{t} \aboveMin \hNrm_{\EndStge})}\right).
    \end{aligned}\end{gathered}\end{equation}
...
$\mFunc_{\EndStge}({a}) \equiv \cFunc_{\EndStge}({a})+{a}$, and drop the (a) arguments to reduce clutter):
\begin{equation}\begin{gathered}\begin{aligned}
      \uFunc^{{c}}({\cFunc}_{\EndStge})   & = \hat{\vFunc}_{\EndStge}^{\aNrm}(\mFunc_{\EndStge}-{\cFunc}_{\EndStge}),
    \end{aligned}\end{gathered}\end{equation}
yielding a marginal propensity to
\textit{have consumed} $\cFunc_{\EndStge}^{\aNrm}$ at each gridpoint:
\begin{equation}\begin{gathered}\begin{aligned}
      \uPP(\cEndStge)\cEndStge^{\aNrm}  & = \hat{\vFunc}_{\EndStge}^{\aNrm}(\mFunc_{\EndStge}-{\cFunc}_{\EndStge})
      \\ \cEndStge^{\aNrm}  & = \hat{\vFunc}^{\aNrm}(\mFunc_{\EndStge}-{\cFunc}_{\EndStge})/\uPP(\cEndStge)
    \end{aligned}\end{gathered}\end{equation}
...
\begin{equation*}\begin{gathered}\begin{aligned}
      \cEndStge  & = \mFunc_{\EndStge} - \aNrm
      \\ \cEndStge^{\aNrm}+1  & = \mFunc_{\EndStge}^{\aNrm}
    \end{aligned}\end{gathered}\end{equation*}
which, together with the chain rule $\cEndStge^{\aNrm}  = \cFunc^{\mNrm}_{\MidStge}\mFunc_{\EndStge}^{\aNrm}$, yields the MPC from
\begin{equation}\begin{gathered}\begin{aligned}
      \cFunc^{\mNrm}(\overbrace{\cEndStge^{\aNrm}+1}^{=\mFunc_{\EndStge}^{\aNrm}})  & = \cEndStge^{\aNrm}
      \\ \cFunc^{\mNrm}  & = \cEndStge^{\aNrm}/(1+\cEndStge^{\aNrm}) \label{eq:MPCfromMPTHC}.
    \end{aligned}\end{gathered}\end{equation}
...
\begin{verbatimwrite}{./cctwMoM/value-Rest.tex}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        \hat{\Koppa}_{t}(\mu_{t})   & = \left(\frac{\bar{\vInv}_{t}(\ushort{m}_{t}+e^{\mu_{t}})-\vInv_{t}(\ushort{m}_{t}+e^{\mu_{t}})}{\aboveMin \hNrm_{\EndStge} \MPCmin_{t} (\PDVCoverc_{t}^{T})^{1/(1-\CRRA)}}\right) \label{eq:Koppa}
      \end{aligned}\end{gathered}\end{equation}
  \MPCMatch{with derivative (dropping arguments)
    \begin{equation}\begin{gathered}\begin{aligned}
          \hat{\Koppa}_{t}^{\mu}   & = (\aboveMin \hNrm_{\EndStge} \MPCmin_{t} (\PDVCoverc_{t}^{T})^{1/(1-\CRRA)})^{-1}e^{\mu_{t}}\left(\bar{\vInv}^{m}_{t}-\vInv^{m}_{t}\right) \label{eq:KoppaPrime}
          % \\  & =  (\aboveMin \hNrm_{\EndStge} \MPCmin_{t})^{-1}e^{\mu_{t}}\left((\PDVCoverc_{t}^{T})^{1/(1-\CRRA)}\MPCmin_{t}-\left((1-\CRRA)\vFunc_{t}({m}_{t})\right)^{-1+1/(1-\CRRA)}\vFunc_{t}^{m}({m}_{t})\right)  \notag
        \end{aligned}\end{gathered}\end{equation}}{}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        \hat{\vInv}_{t}  & = \bar{\vInv}_{t}-\overbrace{\left(\frac{1}{1+\exp(\hat{\Chi}_{t})}\right)}^{=\hat{\Koppa}_{t}} \aboveMin \hNrm_{\EndStge} \MPCmin_{t} (\PDVCoverc_{t}^{T})^{1/(1-\CRRA) }
      \end{aligned}\end{gathered}\end{equation}
...
The easiest case is where the stochastic interest factor is i.i.d.,
...
\begin{equation}\begin{gathered}\begin{aligned}
      \MPC  & = 1- \left(\DiscFac  \Ex_{\BegStge}[\Risky_{t}^{1-\CRRA}]\right)^{1/\CRRA} \label{eq:MPCExact}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
      {c}_{T-1}^{-\CRRA}  & = \vFunc^{{a}}_{\EndStgem}({m}_{T-1}-{c}_{T-1}) \label{eq:cUnc}
    \end{aligned}\end{gathered}\end{equation}
...
yields marginal value of $\vFunc^{{a}}_{\EndStgem}(0)$. Further, when the
...
\begin{equation}\begin{gathered}\begin{aligned}
      \uFunc^{{c}}({m}_{T-1})  & = \vFunc^{{a}}_{\EndStgem}(0)  \nonumber \\
      {m}_{T-1}  & = (\vFunc^{{a}}_{\EndStgem}(0))^{(-1/\CRRA)}  \nonumber
      \\        & = \cFunc_{\EndStgem}(0). \label{eq:LCbindsTm1}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{enumerate}
\item We redefine
  $\hEndMin_{\EndStge}$, which now is the PDV of receiving
...
\renewcommand{\stge}{t}
Recall now equation (\ref{eq:upEqbetaOp}):
\begin{equation*}\begin{gathered}\begin{aligned}
%      \vPEndStge({a}_{t})  & = \Ex_{\BegStge}[\DiscFac \Rfree \PermGroFac_{t+1}^{-\CRRA}       \uFunc^{{c}}(\cFunc_{t+1}(\RNrm_{t+1} {a}_{t}+{\TranShkEmp}_{t+1}))] \\
      \uFunc^{{c}}({c}_{t})   & = \vEndStge^{{a}}({m}_{t}-{c}_{t}).
    \end{aligned}\end{gathered}\end{equation*}
...
\begin{enumerate}
  \ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny Point out that we
      are defining $\vEndStge^{{a}}$ here by the literal summation
      operation \eqref{eq:vEndeq}.}}{}

\item For the vector of values $\vct{{a}}_{\stge}$ in \texttt{aVec\_eee}, numerically calculate the values
  of $\cFunc_{\EndStge}(\vct{{a}}_{\stge})$ and $\cFunc_{\EndStge}^{{a}}(\vct{{a}}_{\stge})$,
  ...
  generating vectors of values $\vct{\cFunc}_{t}$\MPCMatch{ and $\vct{\cFunc}^{a}_{\EndStge}$.}{.}

\item Construct a corresponding vector of values of $\vct{m}_{t}=\vct{\cNrm}_{t}+\vct{\aNrm}_{t}$\MPCMatch{; similarly construct a corresponding list of MPC's $\vct{\MPC}_{t}$ using equation \eqref{eq:MPCfromMPTHC}.}{.}

\item Construct a corresponding vector $\vct{\mu_{t}}$, the levels\MPCMatch{ and first derivatives}{} of $\vct{\koppa}_{t}$, and the levels\MPCMatch{ and first derivatives}{} of $\vct{\chi}_{t}$.
...
\item If we are to approximate the value function, construct a corresponding list of values of $\vct{\vFunc}_{t}$, the levels\MPCMatch{ and first derivatives of $\vct{\Koppa}_{t}$,}{,} and the levels\MPCMatch{ and first derivatives}{} of $\hat{\vct{\Chi}}_{t}$; and construct an interpolating approximation function $\hat{\Chi}_{t}$ that matches those points.
\end{enumerate}
...
The code creates the relevant $\Alt{\cFunc}_{t}({m}_{t})$
...
\begin{verbatimwrite}{./Equations/FOCw.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
        0  & = \Ex_{t}[{\vFunc}_{\MidStgep}^{{m}}({m}_{t+1})(\Risky_{t+1}-\Rfree){a}_{t}] \notag
...
\begin{equation}\begin{gathered}\begin{aligned}
      ...
      \\      0  & = \vFunc^{\varsigma}_{\EndStge}({a}_{t},\varsigma_{t}).
    \end{aligned}\end{gathered}\end{equation}
...
$\hat{\vFunc}_{\EndStge}^{{a}}$ and $\hat{\vFunc}_{\EndStge}^{\varsigma}$ in
...
Python to solve
\begin{equation}\begin{gathered}\begin{aligned}
      {c}_{t}^{-\CRRA}  & = \hat{\vFunc}^{a}_{\EndStge}({m}_{t}-{c}_{t},\varsigma_{t}) %\label{eq:FOCwrtcMultContr}
      \\      0  & = \hat{\vFunc}^{\varsigma}_{\EndStge}({m}_{t}-{c}_{t},\varsigma_{t}) \label{eq:FOCwrtw}
    \end{aligned}\end{gathered}\end{equation}
...
\providecommand{\Opt}{}
\renewcommand{\Opt}{\bar}
\providecommand{\vOpt}{}
\renewcommand{\vOpt}{\bar{\vFunc}}
\begin{equation}\begin{gathered}\begin{aligned}
      \vOpt_{\EndStge}({a}_{t})  & = \max_{\varsigma_{t}} ~~  \vEndStge({a}_{t},\varsigma_{t})
      \\      & \text{s.t.} \nonumber
      \\      0 \leq & \varsigma_{t} \leq 1
    \end{aligned}\end{gathered}\end{equation}
where the bar over $\vOpt(a)$ indicates that this is the $\vFunc$ that has been optimized with
...
${m}_{t}$ using the endogenous gridpoints method described above.  Thus we have transformed the multidimensional optimization problem into a sequence of two simple unidimensional optimization problems.
