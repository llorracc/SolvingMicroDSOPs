...
\usepackage{\LaTeXInputs/local}% LaTeX Shortcuts for whole paper
...
\providecommand{\move}{step}\renewcommand{\move}{step}
\providecommand{\moves}{steps}\renewcommand{\moves}{moves}
\providecommand{\interval}{period}\renewcommand{\interval}{period}
...
\newcommand{\ifhabits}{\ifthenelse{\boolean{habitsSolve}}}
  ...
  \maketitle
  \footnotesize
  \noindent  Note: The \href{https://github.com/$owner/SolvingMicroDSOPs/blob/master/SolvingMicroDSOPs.ipynb}{Jupyter Notebook}  associated with this document contains python code that produces all results, from scratch, except for the last section on indirect inference.  The numerical results have been confirmed by showing that the answers that the raw python produces correspond to the answers produced by tools available in the \href{https://econ-ark.org}{Econ-ARK} toolkit, more specifically the \href{https://github.com/econ-ark/HARK}{HARK Framework}.  The MSM results at the end has specifically have been superseded by the \href{https://econ-ark.org/materials/solvingmicrodsops?launch}{SolvingMicroDSOPs} \href{https://github.com/econ-ark/REMARK}{REMARK}.
  \normalsize
...
\ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny Goal: Convert 2-state to 1-state problem; makes computational solutions much easier and easier to debug.}}{} The single most powerful method for speeding the solution of such models is to redefine the problem in a way that reduces the number of state variables (if at all possible).  Here, the obvious idea is to see whether the problem can be rewritten in terms of the ratio of various variables to permanent noncapital (`labor') income $\pLvl_{t}$ (henceforth for brevity referred to simply as `permanent income.')
...
\begin{equation*}\begin{gathered}\begin{aligned}
    ...
    \end{aligned}\end{gathered}\end{equation*}
and so on back to all earlier periods.  Hence, if we solve the problem \eqref{vNormed}
which (when optimal consumption is chosen) has only a single state variable $\mNrm_{t}$, we can obtain the levels of
...
This allows us to capture the fact that the value of the consumer's problem can be computed at any of the three steps.
...
Using this new notation, the first order condition for \eqref{vNormed} with respect to ${c}_{t}$ is
\begin{equation}\begin{gathered}\begin{aligned}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \null{\vFunc}^{{m}}_{\dcsn(t)}({m}_{t})  & =  \Ex_{t} [\DiscFac\Rfree\PermGroFac_{t+1}^{-\CRRA}\null{\vFunc}^{{m}}_{\MidStgep}({m}_{t+1})] \label{eq:envelope}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
      {\vPEndStge}({a}_{\stge})  & = \Ex_{\stge}[\DiscFac \Rfree \PermGroFac_{\stge+1}^{-\CRRA} \null{\vFunc}_{\MidStgep}^{{m}}(\RNrm_{\stge+1}{a}_{\stge}+{\TranShkEmp}_{\stge+1})]  \label{eq:vEndPrimetdefn}
    \end{aligned}\end{gathered}\end{equation}
...
Our peculiar notation described above is useful not only in designating the {\move} of a value function; it is also useful in clarifying the perspective from which an expectation is taken.  So, using this notation, the problem in the second-to-last period of life is:
\begin{equation*}\begin{gathered}\begin{aligned}
      \null{\vFunc}_{T-1}({m}_{T-1})  & = \max_{{c}_{T-1}} ~~ \uFunc({c}_{T-1}) +
      \DiscFac \Ex_{\cntn(T-1)} \left[\PermGroFac_{T}^{1-\CRRA}\null{\vFunc}_{\cntn(T)}(({m}_{T-1} - {c}_{T-1})\RNrm_{T} + \TranShkEmp_{T})\right],
\end{aligned}\end{gathered}\end{equation*}
where, to review, $\Ex_{\cntn(T-1)}$  indicates that the expectation is taken as of the end of period $T-1$.
...
so we can rewrite the maximization problem that defines the middle stage of the period {$\sim$} as \ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny Be sure to discuss \eqref{eq:vEndTm1} explicitly, as it is implemented directly in the program as per equation \eqref{eq:vEndTm1prog}.}}{}
...
After defining the utility function, the natural borrowing constraint is defined to be $\underline{a}_{T-1}=-\underline{\TranShkEmp}\RNrm{T}^{-1}$, which will be discussed in greater depth in section \ref{subsec:LiqConstrSelfImposed}. Following the reformulation of the maximization problem, an instance of the \texttt{gothic\_class} is created using the specifications and the discretized distribution described in the prior lines of code; this is required to provide the numerical solution.
...
$\null{a}_{T-1,i}$, calculate the vector $\vec{\vFunc}_{\BegStgem}$ as the collection of points $\vFunc_{\EndStgem,i} =
\vFunc_{\EndStgem}(\null{a}_{T-1,i})$ using equation
(\ref{eq:vEndtdefn}); then construct an
\texttt{InterpolatingUnivariateSpline} object
$\Alt{\vFunc}_{\EndStgem}({a}_{T-1})$ by passing the lists \texttt{aVec} and \texttt{gothicvVec} as arguments.
These lists contain the points of the $\vec{a}_{T-1}$ and $\vec{\vFunc}_{\BegStgem}$ vectors, respectively.
...
of assets.\footnote{What we are doing here is closely related to `the
  ...
  method is essentially a nonparametric version.}  The problem is solved in the same block with the remaining following lines of code. Figure~\ref{fig:PlotOTm1RawVSInt}
...
Nevertheless, the resulting consumption rule obtained when the approximating $\Alt{\vFunc}_{\EndStgem}({a}_{T-1})$
...
\begin{equation}\begin{gathered}\begin{aligned}
      \vFunc_{\EndStgem}^{{a}}({a}_{T-1})  & =  \DiscFac \Rfree \PermGroFac_{T}^{-\CRRA} \left(\frac{1}{n_{\TranShkEmp}}\right) \sum_{i=1}^{n_{\TranShkEmp}} \left(\RNrm_{T} {a}_{T-1} + \TranShkEmp_{i}\right)^{-\CRRA} \label{eq:vEndPrimeTm1}
    \end{aligned}\end{gathered}\end{equation}
at the points in \texttt{aVec} yielding
$\{\{\null{a}_{T-1,1},\vFunc_{\EndStgem,1}^{{a}}\},\{{a}_{T-1,2},\vFunc_{\EndStgem,2}^{{a}}\}
\ldots\}$ and construct
...
passing it \texttt{aVec} and \texttt{vpVec} as arguments. Note that in defining the list of values \texttt{vpVec}, we again make use of the
...
to the end-of-period value function $\vFunc^{{a}}_{\EndStgem}({a}_{T-1})$, we can define
\begin{equation}\begin{gathered}\begin{aligned}
...
procedure is to calculate the values of $\cFunc_{\EndStgem,i}$ at each
...
these
points (the `consumed function').  % A comparison of the result to the true consumed function is presented in figure~\ref{fig:GothVInvVSGothC}.  The two solutions are quite close.
...
the corresponding $\cFunc_{\EndStgem,i}$ gridpoints used to produce our
linearly interpolating approximation $\Alt{\cFunc}_{\EndStgem}$ (as described in section~\ref{subsec:LinInterp})?

The natural answer would seem to be linear extrapolation; for example, we could use
\begin{equation}\begin{gathered}\begin{aligned}
      \Alt{\cFunc}_{\EndStgem}({a}_{T-1})  & = \Alt{\cFunc}_{\EndStgem}({a}_{T-1,1})+\Alt{\cFunc}_{\EndStgem}^{{a}}({a}_{T-1,1})({a}_{T-1}-{a}_{T-1,1}) \label{eq:ExtrapLin}
    \end{aligned}\end{gathered}\end{equation}
for values of ${a}_{T-1} < {a}_{T-1,1}$, where $\Alt{\cFunc}_{\EndStgem}^{{a}}({a}_{T-1,1})$ is the derivative of the $\Alt{\cFunc}_{\EndStgem}$ function at the bottommost gridpoint (see below).  Unfortunately, this approach
will lead us into difficulties.  To see why, consider what
happens to the true (not approximated) $\vFunc^{{a}}_{\EndStgem}({a}_{T-1})$ as
...
`self-imposed' (or `natural') borrowing constraint (which results from the
...
linear function like $\Alt{\vFunc}^{{m}}_{\EndStgem}$, because a
linear approximation can never reach the correct gridpoint for
$\vFunc^{{a}}_{\EndStgem}(\underline{a}_{T-1})=\infty.$ To see what
...
Our next trick lets us completely skip the rootfinding step.  The method can be understood by noting that any arbitrary value of ${a}_{T-1,i}$ (greater than its lower bound value $\underline{a}_{T-1}$) will be associated with \textit{some} marginal valuation as of the $\EndMark$ stage of $T-1$ (that is, at the end of the period), and the further observation that it is trivial to find the value of ${c}$ that yields the same marginal valuation, using the first order condition,
\begin{equation}\begin{gathered}\begin{aligned}
      \uFunc^{{c}}({\cNrm}_{T-1,i})  & =
                                       \vFunc^{{a}}_{\EndStgem}({a}_{T-1,i}) \label{eq:eulerTm1}
\end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
       {\cNrm}_{T-1,i}  & = \left(\vFunc^{{a}}_{\EndStgem}({a}_{T-1,i})\right)^{-1/\CRRA}
%      \\  & = (\vFunc^{{a}}_{\EndStgem}({a}_{T-1,i}))^{-1/\CRRA}
      \\  & \equiv  \cFunc_{\EndStgem}({a}_{T-1,i})
      \\  & \equiv  \cFunc_{\EndStgem,i}
      .
    \end{aligned}\end{gathered}\end{equation}
where the $\EndMark$ emphasizes that these are points on the `consumed' function (that is, the function that reveals how much an optimizing consumer must have consumed in order to end the period with ${a}_{T-1,i}$.
...
routine to locate the corresponding optimal ${c}_{T-1}$. This routine is performed in ``Endogenous Gridpoints.'' First, the \texttt{gothic.C\_Tminus1} function is called for each of the pre-specfied
...
\texttt{mVec\_egm}. With the $\vec{\cFunc}_{\EndStgem}$ values in hand, we can generate a set of ${m}_{T-1,i}$ and ${\cNrm}_{T-1,i}$
...
\begin{equation*}\begin{gathered}\begin{aligned}
      {\vFunc}_{T-1}({m}_{T-1})  & = \max_{\cNrm_{T-1}} ~~ \uFunc({c}_{T-1}) + \Ex_{T-1} [\DiscFac \PermGroFac_{T}^{1-\CRRA}{\vFunc}_{\cntn(T)}({m}_{T})] \label{eq:ConstrArt}
      \\ & \mbox{s.t.}  \nonumber
