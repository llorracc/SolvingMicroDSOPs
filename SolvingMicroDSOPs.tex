...
\ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny Assumption on $\TranShkEmp$ implies $\Ex_{t}[\TranShkEmp_{t+1}] = 1$.}}{}
Using the fact about lognormally distributed variables \handoutM{ELogNorm}\footnote{This fact is referred to as \texttt{ELogNorm}
  ...
  \handoutM{LogELogNorm} is the name of the fact that implies that $\log \Ex[{\TranShkEmp}]=0$.} that if $\log \EPrem \sim
\mathcal{N}(\eprem,\sigma_{\eprem}^{2})$ then $\log \Ex[{\EPrem}] =
\eprem +\sigma_{\eprem}^{2}/2$, assumption the assumption about the distribution of shocks guarantees that $\log \Ex[{\TranShkEmp}] = 0$ which means that
$\Ex[{\TranShkEmp}]$=1 (the mean value of the transitory shock is 1).

Equation \eqref{eq:permincgrow} indicates that we are allowing
for a predictable average profile of income growth over the lifetime $\{\PermGroFac\}_{0}^{T}$ 
...
\begin{verbatimwrite}{./Equations/vrecurse.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
        {\pmb{\vLvl}}
_{t}(\mLvl_{t},\pLvl_{t})  & = \max_{\cLvl_{t}}~ \uFunc(\cLvl_{t}) + \Ex_{t}[{\DiscAlt} {\pmb{\vLvl}}
_{t+1}({\mLvl}_{t+1},\pLvl_{t+1})]\label{eq:vrecurse}
      \end{aligned}\end{gathered}\end{equation}
\end{verbatimwrite}
\input{./Equations/vrecurse.tex}
subject to the Dynamic Budget Constraint (DBC) \eqref{DBCLevel} given above, where ${\pmb{\vLvl}}
_{t}$ measures total expected discounted utility from behaving optimally now
and henceforth.
...
\ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny Tell them to look at
    the figure; $\TranShkEmp$ is the vertical axis, and the points
    $\sharp^{-1}_{i}$ correspond to the intersections of the dashing
    lines with the curve.}}{} Define a set of points from $\sharp_{0}$ to $\sharp_{n_{\TranShkEmp}}$ on the $[0,1]$ interval
...
\begin{verbatimwrite}{./cctwMoM/value-Rest.tex}
  ...
  \begin{equation*}\begin{gathered}\begin{aligned}
        ...
        \\  \hat{\vFunc}^{m}_{t}  & = \uFunc^{\prime}(\hat{\vInv}_{t}) \hat{\vInv}^{m}
        \MPCMatch{\\  \hat{\vFunc}^{mm}_{t}  & = \uFunc^{\prime\prime}(\hat{\vInv}_{t}) (\hat{\vInv}^{m})^{2} + \uFunc^{\prime}(\hat{\vInv}_{t})\hat{\vInv}^{mm}}{}
        .
      \end{aligned}\end{gathered}\end{equation*}
...
\begin{verbatimwrite}{./cctwMoM/TighterThreeFuncsExplain.tex}
  ...
  \begin{equation*}\begin{gathered}\begin{aligned}
        \bar{\vFunc}_{t}(\mNrm)  & = \uFunc(\bar{\cNrm}_{t}(\mNrm))+ \bar{\vEnd}_{t}(\mNrm-\bar{\cNrm}_{t}(\mNrm)),
      \end{aligned}\end{gathered}\end{equation*}
  ...
  \begin{equation*}\begin{gathered}\begin{aligned}
        \bar{\vFunc}_{t}(\mNrm)  & = \uFunc( \MPCmax_{t} \aboveMin \mNrm)+ \bar{\vEnd}_{t}((1-\MPCmax_{t})\aboveMin \mNrm),
      \end{aligned}\end{gathered}\end{equation*}
  which is easy to compute because $\bar{\vEnd}_{t}(\aNrm_{t}) = \DiscFac \bar{\vFunc}_{t+1}(\aNrm_{t}\RNrm+1)$
...
\ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny Read this carefully
    ...
    consume everything and get the same $\uFunc^{\prime}$, then happy.  If
    consumed $\TranShkEmp$ less, mv of saving would be $> \uFunc^{\prime}(c).$}}{}
...
\item For the grid of values ${a}_{t,i}$ in \texttt{aVec}$_{t}$, numerically calculate the values
  ...
  \begin{verbatimwrite}{./Equations/vEndeq.tex}
    \begin{equation}\begin{gathered}\begin{aligned}
          ...
          \MPCMatch{\\        \cEndFunc^{a}_{t,i}  & = -(1/\CRRA)\left(\vEnd_{t}^{\prime}({a}_{t,i})\right)^{-1-1/\CRRA} \vEnd_{t}^{\prime\prime}(\aNrm_{t,i}),}{}
        \end{aligned}\end{gathered}\end{equation}
  \end{verbatimwrite}
  \input{./Equations/vEndeq.tex}
  generating vectors of values $\vec{\cEndFunc}_{t}$\MPCMatch{ and $\vec{\cEndFunc}^{a}_{t}$.}{.}

\item Construct a corresponding list of values of ${\cNrm}_{t,i}$ and
  ${m}_{t,i}$ from ${\cNrm}_{t, i} = \cEndFunc_{t, i}$ and ${m}_{t, i} =
  {\cNrm}_{t, i}+{a}_{t, i}$\MPCMatch{; similarly construct a corresponding list of $\MPC_{t, i}$ using equation \eqref{eq:MPCfromMPTHC}.}{.}
...
\item For each ${a}_{t,i}$ in \texttt{aVec}, construct \texttt{$\cEndFunc$} as follows:
  \begin{equation}\begin{gathered}\begin{aligned}
        \cEndFunc_{t}({a}_{t,i})  & = \left( \DiscFac \Ex_{t} \left[\Rfree \PermGroFac_{t+1}^{-\CRRA} (\Alt{\Hi{\cFunc}}_{t+1}(\RNrm_{t+1} {a}_{t,i} +         {\TranShkEmp}_{t+1}))^{-\CRRA}\right]\right)^{-1/\CRRA}
...
\begin{equation*}\begin{gathered}\begin{aligned}
      \vEnd_{t}^{{a}}  & = \Ex_{t}[\DiscFac {\Rport}_{t+1}{\vFunc}_{t+1}^{m}({m}_{t+1})]
      \\      \vEnd_{t}^{\varsigma}  & = \Ex_{t}[\DiscFac (\Risky_{t+1}-\Rfree){\vFunc}_{t+1}^{m}({m}_{t+1})  ]a_{t}
    \end{aligned}\end{gathered}\end{equation*}
...
\begin{comment} % at suggestion of WW, this section was removed as unnecessary for the current model, which solves for the converged rule very fast
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        \\  \vOpt^{\prime}({a}_{t})  & = \DiscFac \Rfree \PermGroFac_{t+1}^{-\CRRA} \bar{\vFunc}^{\prime}(\RNrm_{t+1} {a}_{t}+1).
      \end{aligned}\end{gathered}\end{equation}
