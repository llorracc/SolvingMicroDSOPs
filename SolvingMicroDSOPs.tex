...
\documentclassegin{align\*}titlepage, headings=optiontotocandhead]{\econtex}
...
\renewcommand{\onlyinsubfile}egin{align\*}1]{}\renewcommand{\notinsubfile}egin{align\*}1]{#1}
...
\begin{verbatimwrite}{./Equations/MaxProb.tex}
  \begin{equation}\label{eq:MaxProb}
    \max ~ \Ex_{t}\leftegin{align\*} \sum_{n=0}^{T-t} {\DiscAlt}^{n} \uFunc({\cLvl}_{t+n})\right].
  \end{equation}
\end{verbatimwrite}
...
% \ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny Assumption on $\TranShkEmp$ implies $\Ex_{t}egin{align\*}\TranShkEmp_{t+1}] = 1$.}}{} Using the fact about lognormally distributed variables \handoutM{ELogNorm}\footnote{This fact is referred to as \texttt{ELogNorm} in the handout \handoutM{MathFactsList}, in the references as \cite{MathFacts}; further citation to facts in that handout will be referenced simply by the name used in the handout for the fact in question, e.g.\ \handoutM{LogELogNorm} is the name of the fact that implies that $\log \Exegin{align\*}{\TranShkEmp}]=0$.} that if $\log \EPrem \sim \mathcal{N}(\eprem,\sigma_{\eprem}^{2})$ then $\log \Exegin{align\*}{\EPrem}] = \eprem +\sigma_{\eprem}^{2}/2$, our assumption about the distribution of the transitory shock guarantees that $\log \Exegin{align\*}{\TranShkEmp}] = 0$ which means that $\Exegin{align\*}{\TranShkEmp}]$=1 (the mean value of the transitory shock is 1).
...
\begin{verbatimwrite}{./Equations/vrecurse.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
        {\vLvl}
_{t}(\mLvl_{t},\pLvl_{t})  & = \max_{\cLvl_{t}}~ \uFunc(\cLvl_{t}) + {\DiscAlt}\Ex_{t}egin{align\*} {\vLvl}
...
\begin{verbatimwrite}{./Equations/vNormed.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        {\DiscFac}\Ex_{t}egin{align\*} \PermGroFac_{t+1}^{1-\CRRA}\null{\vFunc}_{t+1}({m}_{t+1})] \label{vtNorm}
        \\         & \text{s.t.} &   \\
        {a}_{t}    & = {m}_{t}-{c}_{t} 
...
\begin{equation}\begin{gathered}\begin{aligned}
      \uFunc^{{c}}({c}_{t})  & = \Ex_{t}egin{align\*}\DiscFac \RNrm_{t+1}\PermGroFac_{t+1}^{1-\CRRA}\null{\vFunc}^{{m}}_{t+1}({m}_{t+1})]  \label{eq:upceqEvtp1}
      \\                        & =  \Ex_{t}egin{align\*}\DiscFac\Rfree\phantom{._{t+1}}\PermGroFac_{t+1}^{\phantom{1}-\CRRA}\null{\vFunc}^{{m}}_{t+1}({m}_{t+1})] 
    \end{aligned}\end{gathered}\end{equation}
and because the \handoutC{Envelope} theorem tells us that
\begin{equation}\begin{gathered}\begin{aligned}
      \null{\vFunc}^{{m}}_{t}({m}_{t})  & =  \Ex_{t} egin{align\*}\DiscFac\Rfree\PermGroFac_{t+1}^{-\CRRA}\null{\vFunc}^{{m}}_{t+1}({m}_{t+1})] \label{eq:envelope}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{verbatimwrite}{./Equations/cEuler.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
        \uFunc^{{c}}({c}_{t})  & = \Ex_{t}egin{align\*}\DiscFac\Rfree \PermGroFac_{t+1}^{-\CRRA}\uFunc^{{c}}({c}_{t+1})]. \label{eq:cEuler}
                                  \end{aligned}\end{gathered}\end{equation}
\end{verbatimwrite}
...
\begin{verbatimwrite}{./Equations/vBegStge}
  \begin{equation}\begin{gathered}\begin{aligned}
\vBegStge(\kNrm_{\stge}) & = \Ex_{\BegStge}egin{align\*}\vMidStge(\overbrace{\kNrm_{\stge} \Rnorm_{\stge} + \TranShkEmp_{\stge}}^{=\mNrm_{\stge}})]  \label{eq:vBegStge}
      \end{aligned}\end{gathered}\end{equation}
\end{verbatimwrite}
...
\begin{equation}\begin{gathered}\begin{aligned}
      {\vPEndStge}({a}_{\stge})  & = \Ex_{\stge}egin{align\*}\DiscFac \Rfree \PermGroFac_{\stge+1}^{-\CRRA} \null{\vFunc}_{\stge+1}^{{m}}(\RNrm_{\stge+1}{a}_{\stge}+{\TranShkEmp}_{\stge+1})]  \label{eq:vEndPrimetdefn}
    \end{aligned}\end{gathered}\end{equation}
or, substituting from equation (\ref{eq:upctp1EqVpxtp1}),
\begin{equation}\begin{gathered}\begin{aligned}
      \vPEndStge({a}_{\stge})  & = \Ex_{\stge}\leftegin{align\*}\DiscFac \Rfree \PermGroFac_{\stge+1}^{-\CRRA} \uFunc^{{c}}\left(\cFunc_{\stge+1}(\RNrm_{\stge+1} {a}_{\stge}+{\TranShkEmp}_{\stge+1})\right)\right].  \label{eq:vEndPrimetOfc}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{equation*}\begin{gathered}\begin{aligned}
      \Exegin{align\*}g(\TranShkEmp)] & = \int_{\TranShkEmpMin}^{\TranShkEmpMax}g(\TranShkEmp)d\FDist(\TranShkEmp) \\
...
Define a set of points from $\sharp_{0}$ to $\sharp_{n_{\TranShkEmp}}$ on the $egin{align\*}0,1]$ interval
...
\begin{equation}\begin{gathered}\begin{aligned}
      \TranShkEmp_{i} \equiv \Exegin{align\*}\TranShkEmp | \sharp_{i-1}^{-1} \leq \TranShkEmp < \sharp_{i}^{-1}]  & = \int_{\sharp^{-1}_{i-1}}^{\sharp^{-1}_{i}} \vartheta ~ d\FDist_{\phantom{\TranShkEmp}}(\vartheta)  .
    \end{aligned}\end{gathered}\end{equation}

The method is illustrated in Figure~\ref{fig:discreteapprox}.  The solid continuous curve represents
the ``true'' CDF $\FDist(\TranShkEmp)$ for a lognormal distribution such that $\Exegin{align\*}\TranShkEmp] = 1$, $\sigma_{\TranShkEmp} = 0.1$.  The short vertical line segments represent the $n_{\TranShkEmp}$
...
\begin{verbatimwrite}{\econtexRoot/Figures/discreteApprox.tex}
  ...
  \begin{figure}
    \includegraphicsegin{align\*}width=6in]{./Figures/discreteApprox}
...
\begin{figure}
  \centerline{\includegraphicsegin{align\*}width=6in]{./Figures/PlotcTm1Simple}}
...
\begin{figure}
  \centerline{\includegraphicsegin{align\*}width=6in]{./Figures/PlotVTm1Simple}}
...
\begin{figure}
  \centerline{\includegraphicsegin{align\*}width=6in]{./Figures/PlotOTm1RawVSInt}}
...
\begin{figure}
  \centerline{\includegraphicsegin{align\*}width=6in]{./Figures/PlotComparecTm1AB}}
  \caption{$\cFunc_{T-1}({m}_{T-1})$ (solid) versus $\Alt{\cFunc}_{T-1}({m}_{T-1})$ (dashed)}
...
\begin{equation}
  \Exegin{align\*}-({c} - \cancel{c})^{2}] < - (\Exegin{align\*}{c}] - \cancel{c})^{2}
\end{equation}
...
by risk:
\begin{equation}
  \Exegin{align\*}-2({c} - \cancel{c})] = - 2(\Exegin{align\*}{c}] - \cancel{c}).
\end{equation}
...
\begin{verbatimwrite}{./Equations/FOCTm1.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
        \uFunc^{{c}}({c}_{T-1})       & = \DiscFac \Ex_{T-1} egin{align\*}\PermGroFac_{T}^{-\CRRA}\Rfree \uFunc^{{c}}({c}_{T})]  %\label{eq:focraw}
...
\begin{figure}
  \centerline{\includegraphicsegin{align\*}width=6in]{./Figures/PlotuPrimeVSOPrime}}
...
\begin{figure}
  \centerline{\includegraphicsegin{align\*}width=6in]{./Figures/PlotOPRawVSFOC}}
...
\begin{figure}
  \centerline{\includegraphicsegin{align\*}width=6in]{./Figures/PlotcTm1ABC}}
...
\begin{equation}\begin{gathered}\begin{aligned}
      egin{align\*}\vFunc^{{m}}_{T-1}({m}_{T-1})]^{-1/\CRRA}  & = \frac{{m}_{T-1}+1}{2}  .
    \end{aligned}\end{gathered}\end{equation}
...
(between \texttt{aVecegin{align\*}0]} and \texttt{aVecegin{align\*}1]}), a
...
\begin{figure}
  \centerline{\includegraphicsegin{align\*}width=6in]{./Figures/GothVInvVSGothC}}
...
$egin{align\*}\Alt{\cFunc}_{T^{+}-1}({a}_{T-1})]^{-\CRRA}$ (dashed line) we
...
\begin{figure}
  \centerline{\includegraphicsegin{align\*}width=6in]{./Figures/GothVVSGothCInv}}
...
\begin{figure}
  \centerline{\includegraphicsegin{align\*}width=6in]{./Figures/PlotComparecTm1AD}}
  \caption{$\cFunc_{T-1}({m}_{T-1})$ (solid) versus $\Alt{\cFunc}_{T-1}({m}_{T-1})$ (dashed)}
...
\begin{figure}
  \centerline{\includegraphicsegin{align\*}width=6in]{./Figures/GothVInvVSGothCEEE}}
...
\begin{figure}
  \includegraphicsegin{align\*}width=6in]{./Figures/GothVVSGothCInvEEE}
...
\begin{verbatimwrite}{./cctwMoM/EndogGptsProbs.tex}
  ...
  \begin{figure}
    \includegraphicsegin{align\*}width=6in]{./Figures/ExtrapProblemPlot}
...
\begin{verbatimwrite}{./cctwMoM/MoM-Prelims.tex}
  ...
  \PermShkOn
  {1, $\TranShkEmp_{t+n} = \Exegin{align\*}\TranShkEmp]=1~\forall~n>0$ and $\PermShk_{t+n} = \Exegin{align\*}\PermShk]=1~\forall~n>0$.}
  {1, $\TranShkEmp_{t+n} = \Exegin{align\*}\TranShkEmp]=1~\forall~n>0$.}
...
\begin{verbatimwrite}{\econtexRoot/Figures/IntExpFOCInvPesReaOptNeedHiPlot.tex}
  ...
  \begin{figure}
    \includegraphicsegin{align\*}width=6in]{./Figures/IntExpFOCInvPesReaOptNeedHiPlot}
...
\begin{verbatimwrite}{./cctwMoM/MoM-End.tex}
  ...
  \begin{figure}
    \includegraphicsegin{align\*}width=6in]{./Figures/ExtrapProblemSolvedPlot}
...
\begin{figure}
  \includegraphicsegin{align\*}width=6in]{./Figures/IntExpFOCInvPesReaOptGapPlot}
...
\begin{verbatimwrite}{./cctwMoM/Tighter.tex}
  ...
  \begin{comment} % Old text needs to be revised or eliminated
    ...
    \begin{figure}
      \includegraphicsegin{align\*}width=6in]{./Figures/IntExpFOCInvPesReaOptNeed45Plot}
    ...
    \begin{figure}
      \includegraphicsegin{align\*}width=6in]{./Figures/IntExpFOCInvPesReaOptNeed45ValuePlot}
...
\begin{verbatimwrite}{./cctwMoM/TighterThreeFuncsExplain.tex}
  ...
  \begin{comment}
    ...
    \begin{figure}
      \includegraphicsegin{align\*}width=6in]{./Figures/IntExpFOCInvPesReaOpt45GapPlot}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \MPC  & = 1- \left(\DiscFac  \Ex_{t}egin{align\*}\Risky_{t+1}^{1-\CRRA}]\right)^{1/\CRRA} \label{eq:MPCExact}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
      ...
      \Ex_{t}egin{align\*}{\DiscFac}_{t+1}\PermGroFac_{t+1}^{1-\CRRA}\null{\vFunc}_{t+1}({m}_{t+1},\risky_{t+1})] \label{vtNormRisky}
      \\         & \text{s.t.} &   \nonumber \\
      {a}_{t}    & = {m}_{t}-{c}_{t} \nonumber
      \\      \risky_{t+1}-\risky  & = (\risky_{t}-\risky)\gamma + \epsilon_{t+1} \notag
...
% When done, you will fill in the egin{align\*}how] text below.
...
\begin{equation*}\begin{gathered}\begin{aligned}
      {\vFunc}_{T-1}({m}_{T-1})  & = \max_{\cNrm_{T-1}} ~~ \uFunc({c}_{T-1}) + \Ex_{T-1} egin{align\*}\DiscFac \PermGroFac_{T}^{1-\CRRA}{\vFunc}_{T}({m}_{T})] \label{eq:ConstrArt}
      \\ & \mbox{s.t.}&  \nonumber
...
\begin{verbatimwrite}{./Equations/LiqCons.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
        \grave{\cFunc}_{T-1}({m}_{T-1})  & = \minegin{align\*}{m}_{T-1},\grave{\cFunc}^{\ast}_{T-1}({m}_{T-1})] \label{eq:LiqCons}.
      \end{aligned}\end{gathered}\end{equation}
\end{verbatimwrite}
...
\hypertarget{cVScCon}{}
\begin{figure}
  \includegraphicsegin{align\*}width=6in]{./Figures/cVScCon}
...
\begin{equation*}\begin{gathered}\begin{aligned}
      \vPEndStge({a}_{t})  & = \Ex_{t}egin{align\*}\DiscFac \Rfree \PermGroFac_{t+1}^{-\CRRA}
...
\item For the grid of values ${a}_{t,i}$ in \texttt{aVec\_eee}, numerically calculate the values
  ...
  \begin{verbatimwrite}{./Equations/vEndeq.tex}
    \begin{equation}\begin{gathered}\begin{aligned}
          ...
          \\                             & = \left(\DiscFac \Ex_{t} \leftegin{align\*}\Rfree \PermGroFac_{t+1}^{-\CRRA}(\grave{\cFunc}_{t+1}(\RNrm_{t+1} {a}_{t,i} +      {\TranShkEmp}_{t+1}))^{-\CRRA}\right]\right)^{-1/\CRRA}, \label{eq:vEndeq}
...
\begin{figure}
  \includegraphicsegin{align\*}width=6in]{./Figures/PlotCFuncsConverge}
...
\Habits{
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        \Ex_{t}egin{align\*}\DiscFac  {\vFunc}_{t+1}({m}_{t+1},h_{t}) ]\}
        \\ & \text{s.t.} & \nonumber \\
        {m}_{t+1}  & = egin{align\*}{m}_{t}-{c}_{t}]\RNrm_{t+1} + \TranShkEmp_{t+1}\\
        h_{t}  & = h_{t-1} + \MPS ({c}_{t} - h_{t-1} ) \\
        {c}_{t} & \leq  {m}_{t} .
      \end{aligned}\end{gathered}\end{equation}
  ...
  stock $h_{t}$:
  \begin{equation}\begin{gathered}\begin{aligned}
        \vEndStge({a}_{t},h_{t})  & = \Ex_{t}egin{align\*}\DiscFac {v}_{t+1}(\RNrm_{t+1} {a}_{t}+{\TranShkEmp}_{t+1},h_{t}))]
      \end{aligned}\end{gathered}\end{equation}
  For future reference note that the derivatives are
  \begin{equation}\begin{gathered}\begin{aligned}
        \vEndStge^{{a}}  & = \DiscFac \Ex_{t} egin{align\*}\RNrm_{t+1} {v}_{t+1}^{m}] \label{eq:vEndsdefn} \\
        \vEndStge^{h}  & = \DiscFac \Ex_{t} egin{align\*}{\vFunc}_{t+1}^{h}] \label{eq:vEndhdefn}
      \end{aligned}\end{gathered}\end{equation}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        0  & = \uFunc^{c}_{t} + \DiscFac \Ex_{t} \leftegin{align\*}{\vFunc}^{m}_{t+1}(-\Rfree)+{\vFunc}^{h}_{t+1} \MPS\right]  \\
        \uFunc^{c}_{t}  & = \DiscFac \Ex_{t} egin{align\*}\RNrm_{t+1} {\vFunc}_{t+1}^{m}-\MPS {\vFunc}_{t+1}^{h}] \label{eq:ctfoc}
        \\            & = \leftegin{align\*}\vEndStge^{{a}}-\MPS \vEndStge^{h} \right].
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        {c}_{t}^{-\CRRA}h_{t-1}^{\CRRA\gamma-\gamma}  & = \leftegin{align\*}\vEndStge^{{a}}-\MPS \vEndStge^{h} \right]
        \\  {c}_{t}                                     & = \leftegin{align\*}h_{t-1}^{\gamma-\CRRA\gamma} \left(\vEndStge^{{a}}-\MPS \vEndStge^{h} \right) \right]^{-1/\CRRA} \label{eq:habitsfoc}.
      \end{aligned}\end{gathered}\end{equation}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        {m}_{t}  & = \leftegin{align\*}h_{t-1}^{\gamma-\CRRA\gamma} \left(\vEndStge^{{a}}(0,h_{t-1}+\MPS({m}_{t}-h_{t-1}))-\MPS \vEndStge^{h}(0,h_{t-1}+\MPS({m}_{t}-h_{t-1}))\right)  \right]^{-1/\CRRA}, \label{eq:LCbindsHabits}
      \end{aligned}\end{gathered}\end{equation}
  or incorporating the transition equation for
  and $h_{t-1}$ that
  \begin{equation}\begin{gathered}\begin{aligned}
        {c}_{t}  & = \leftegin{align\*}h_{t-1}^{\gamma-\CRRA\gamma}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        {\cNrm}_{k}^{\ell}  & = \leftegin{align\*}(h_{k}^{\ell})^{\gamma-\CRRA\gamma}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        {\vFunc}_{t}^{m}  & = \DiscFac  \Ex_{t} egin{align\*}\RNrm_{t+1} {\vFunc}_{t+1}^{m}] \label{eq:envelopex}
      \end{aligned}\end{gathered}\end{equation}

  Substituting this into the FOC equation~(\ref{eq:ctfoc}) gives
  \begin{equation}\begin{gathered}\begin{aligned}
        \uFunc^{c}_{t}  & = {\vFunc}_{t}^{m}-\Ex_{t}egin{align\*}\DiscFac \MPS {\vFunc}_{t+1}^{h}] \label{eq:ctfocsubx}
        \\ {\vFunc}_{t}^{m}  & = \uFunc^{c}_{t}+\Ex_{t}egin{align\*}\DiscFac \MPS {\vFunc}_{t+1}^{h}] \label{eq:vtfoc}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        \leftegin{align\*}{\vFunc}_{t+1}(({m}_{t}-{c}_{t})\RNrm_{t+1}+{\TranShkEmp}_{t+1},h_{t-1}+\MPS({c}_{t}-h_{t-1}))
        \right] \nonumber
      \end{aligned}\end{gathered}\end{equation}
  Substituting in the fact that ${c}_{t}={m}_{t}$ (because the consumer is constrained)
  \begin{equation}\begin{gathered}\begin{aligned}
        {\vFunc}_{t}({m}_{t},h_{t-1})  & = \uFunc({c}_{t},h_{t-1}) +  \Ex_{t}\leftegin{align\*}\DiscFac
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        \\              & = \uFunc_{t}^{c} + \DiscFac \Ex_{t} egin{align\*}\MPS {\vFunc}_{t+1}^{h}]
      \end{aligned}\end{gathered}\end{equation}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        \\  & = \uFunc^{h}_{t} + \DiscFac \Ex_{t} \leftegin{align\*}{\vFunc}_{t+1}^{h}\frac{\partial h_{t}}{\partial h_{t-1}} \right]\nonumber
        \\  & = \uFunc^{h}_{t} + \DiscFac \Ex_{t} egin{align\*}(1-\MPS) {\vFunc}_{t+1}^{h}] \nonumber
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        n_{T}^{c}  & = \leftegin{align\*}\uFunc^{c}_{T}\right]^{-1/\CRRA}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        \Lambda_{t}^{h} 
        & =                           \leftegin{align\*}\frac{h_{t-1}^{1-\gamma(\CRRA-1)}{\vFunc}_{t}^{h}}{-\gamma}\right]^{1/(1-\CRRA)}
      \end{aligned}\end{gathered}\end{equation}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        & =                     ({m}_{T-1}-{c}_{T-1})\left(\frac{h_{T-1}}{h_{T-2}}\right)^{\gamma(1/\CRRA-1)}\left(1 +
          \gamma \MPS \leftegin{align\*}\frac{{m}_{T-1}-{c}_{T-1}}{(1-\MPS)h_{T-2}+\MPS
  ...
  \begin{equation*}\begin{gathered}\begin{aligned}
        1  & = ({m}_{T-1}/{c}_{T-1})\left(\frac{\MPS
          ...
          \leftegin{align\*}\frac{{m}_{T-1}}{{c}_{T-1}}\right]\right)^{-1/\CRRA}
        \\    
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        \cFunc_{\overline{t}}^{{a}}({a}_{t},h_{t})        & = egin{align\*}\vEndStge^{{a}}({a}_{t},h_{t})]^{-1/\CRRA} h_{t}^{\gamma(1-1/\CRRA)} \label{eq:vEndstransform} \\
        \Lambda_{t}^{m}({m}_{t},h_{t-1})   & = egin{align\*}{\vFunc}_{t}^{m}({m}_{t},h_{t-1})]^{-1/\CRRA} h_{t-1}^{\gamma(1-1/\CRRA)} \label{eq:vxtransform} \\
        \cFunc_{\overline{t}}^{h}({a}_{t},h_{t})       
        & =                                                  egin{align\*}-h_{t}^{1-\gamma(\CRRA-1)}\vEndStge^{h}({a}_{t},h_{t})/\gamma]^{1/(1-\CRRA)}  \label{eq:vEndhtransform}
        \\      \Lambda_{t}^{h}({m}_{t},h_{t-1})   & = egin{align\*}-h_{t-1}^{1-\gamma(\CRRA-1)}{\vFunc}_{t}^{h}({m}_{t},h_{t-1})/\gamma]^{1/(1-\CRRA)} \label{eq:vhtransform}
        \\      \Lambda_{t}({m}_{t},h_{t-1})       & = egin{align\*}(1-\CRRA)
  ...
  \item Loop over the $m$ values of $h$ in \texttt{hVec}, indexing them
    ...
    \item Loop over ${a}_{i}$ finding the optimal ${c}_{a}$ associated
      ...
      \eqref{eq:habitsfoc}:
      \begin{equation}
        {\cNrm}_{i} = \leftegin{align\*}h_{j}^{\gamma-\CRRA \gamma}
    ...
    \item Construct \texttt{ctegin{align\*}mt,j]} as a linear interpolation of the
      $\{{m}_{i},{\cNrm}_{i}\}$
    \end{itemize}
  \end{enumerate}
...
\begin{verbatimwrite}{./Equations/PortProb.tex}
  \begin{equation*}\begin{gathered}\begin{aligned}
        ...
        \Ex_{t}egin{align\*}{\vFunc}_{t+1}({m}_{t+1})]
        \\      & \text{s.t.} & \nonumber
...
or, more compactly,
\begin{equation*}\begin{gathered}\begin{aligned}
      {\vFunc}_{t}({m}_{t})  & = \max_{\{\cFunc_{t},\varsigma_{t}\}} ~~  \uFunc({c}_{t}) +  \Ex_{t}egin{align\*}\DiscFac {\vFunc}_{t+1}(({m}_{t}-{c}_{t}){\Rport}_{t+1} +        {\TranShkEmp}_{t+1})]
      \\                       & \text{s.t.} & \nonumber
...
\begin{verbatimwrite}{./Equations/valfuncFOCRtilde.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
        \uFunc^{{c}}({c}_{t})  & = \DiscFac \Ex_{t} egin{align\*}{\Rport}_{t+1} \vFunc^{{m}}_{t+1}({m}_{t+1})] \label{eq:valfuncFOCRtilde},
      \end{aligned}\end{gathered}\end{equation}
\end{verbatimwrite}
...
\begin{verbatimwrite}{./Equations/EulercRiskyR.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
        \uFunc^{{c}}({c}_{t})  & = \Ex_{t}egin{align\*}\DiscFac {\Rport}_{t+1} \uFunc^{{c}}({c}_{t+1})]. \label{eq:EulercRiskyR}
      \end{aligned}\end{gathered}\end{equation}
\end{verbatimwrite}
...
\begin{verbatimwrite}{./Equations/FOCw.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
        0  & = \Ex_{t}egin{align\*}{\vFunc}_{t+1}^{{m}}({m}_{t+1})(\Risky_{t+1}-\Rfree){a}_{t}] \notag
        \\         & = {a}_{t}\Ex_{t}\leftegin{align\*}\uFunc^{{c}}\left(\cFunc_{t+1}({m}_{t+1})\right)(\Risky_{t+1}-\Rfree)\right] \label{eq:FOCw}.
      \end{aligned}\end{gathered}\end{equation}
\end{verbatimwrite}
...
\begin{equation*}\begin{gathered}\begin{aligned}
      \vEndStge({a}_{t},\varsigma_{t})  & = \Ex_{t}egin{align\*}\DiscFac {\vFunc}_{t+1}({m}_{t+1})]
    \end{aligned}\end{gathered}\end{equation*}
which has derivatives
\begin{equation}\begin{gathered}\begin{aligned}
      \vEndStge^{{a}}  & = \Ex_{t}egin{align\*}\DiscFac {\Rport}_{t+1}{\vFunc}_{t+1}^{m}({m}_{t+1})] = \Ex_{t}egin{align\*}\DiscFac {\Rport}_{t+1}{\uFunc}_{t+1}^{{c}}(\cFunc_{t+1}({m}_{t+1}))]
      \\      \vEndStge^{\varsigma}  & = \Ex_{t}egin{align\*}\DiscFac (\Risky_{t+1}-\Rfree){\vFunc}_{t+1}^{m}({m}_{t+1})  ]a_{t} = \Ex_{t}egin{align\*}\DiscFac (\Risky_{t+1}-\Rfree){\uFunc}_{t+1}^{{c}}(\cFunc_{t+1}({m}_{t+1}))  ]a_{t} \notag
    \end{aligned}\end{gathered}\end{equation}
...
over the returns $\rfree$ available on the riskless asset.\footnote{This guarantees that $\Exegin{align\*}\Risky] = \EPrem$ is invariant to the choice of $\sigma^{2}_{\eprem}$; see \handoutM{LogELogNorm}.}
...
\begin{figure}
  \includegraphicsegin{align\*}width=6in]{./Figures/PlotctMultContr}
...
\begin{figure}
  \includegraphicsegin{align\*}width=6in]{./Figures/PlotRiskySharetOfat}
...
\begin{equation}
  \Ex_t egin{align\*}{\mNrm}_{t+1}/\mNrm_t] = 1 \mbox{~if~} \mNrm_t = \mTrgNrm  \label{eq:mTrgNrmet}
\end{equation}
...
\begin{comment} % at suggestion of WW, this section was removed as unnecessary for the current model, which solves for the converged rule very fast
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        \bar{\cFunc}({m}_{t})  & = \overbrace{(1-\Rfree^{-1}(\Rfree
          ...
          \underline{\MPC}}\leftegin{align\*}{m}_{t}-1+\left(\frac{1}{1-1/\Rfree}\right)\right]
        \label{eq:pfinfhorc}
      \end{aligned}\end{gathered}\end{equation}
...
\begin{verbatimwrite}{./Equations/lifecyclemax.tex}
  \begin{equation}\begin{gathered}\begin{aligned}
\vFunc_{t}(\pLvl_{t},\mLvl_{t}) & =    \max_{\{\cFunc\}_{t}^{T}}~~ \uFunc(\cLvl_{t})+\Ex_{t}\leftegin{align\*}\sum_{n=1}^{T-t} {\beth}^{n} \Alive_{t}^{t+n}\hat{\DiscFac}_{t}^{t+n} \uFunc(\cLvl_{t+n}) \right]   \label{eq:lifecyclemax}
\end{aligned}\end{gathered}  \end{equation}
\end{verbatimwrite}
...
\begin{figure}egin{align\*}h]
  \includegraphicsegin{align\*}width=6in]{./Figures/PlotTimeVaryingParam}
  \caption{Time Varying Parameters}
  \label{fig:TimeVaryingParam}
\end{figure}

\begin{table}egin{align\*}h]
...
\begin{verbatimwrite}{./Equations/LifeCycleMaxNormed.tex}
  \begin{equation*}\begin{gathered}\begin{aligned}
        {\vFunc}_{t}({m}_{t})  & = \max_{{c}_{t}}~~~ \uFunc({c}_{t})+\beth\Alive_{t+1}\hat{\DiscFac}_{t+1}
          \Ex_{t}egin{align\*}(\PermShk_{t+1}\PermGroFac_{t+1})^{1-\CRRA}{\vFunc}_{t+1}({m}_{t+1})]   \\
        & \text{s.t.} &   \nonumber \\
        {a}_{t}    & = {m}_{t}-{c}_{t} \nonumber
...
\begin{equation}\begin{gathered}\begin{aligned}
      \uFunc^{{c}}({c}_{t}) & = \beth\Alive_{t+1}\hat{\DiscFac}_{t+1}\Rfree \Ex_{t}\leftegin{align\*}\uFunc^{{c}}\left(\PermShk_{t+1}\PermGroFac_{t+1}\cFunc_{t+1}\left({a}_{t}\RNrm_{t+1}+\TranShkEmp_{t+1}\right)\right)\right]\label{eq:FOCLifeCycle}
      .
    \end{aligned}\end{gathered}\end{equation}
\end{verbatimwrite}
...
\begin{figure}
  % \includegraphicsegin{align\*}width=6in]{./Figures/PlotMeanMedianSCF}} % weird mean value
  \includegraphicsegin{align\*}width=6in]{./Figures/PlotMeanMedianSCFcollegeGrads}
...
%  \begin{equation}\begin{gathered}\begin{aligned}
%        \lefteqn{    \texttt{GapEmpiricalSimulatedMedians$egin{align\*}\CRRA,\beth]$:=}}    \nonumber \\
%        &egin{align\*}&\texttt{ConstructcFuncLife$egin{align\*}\CRRA,\beth]$;}\nonumber\\
%        &\texttt{Simulate;}\nonumber\\
...
\begin{figure}
  \includegraphicsegin{align\*}width=6in]{./Figures/Sensitivity.pdf}
...
\begin{figure}
  \includegraphicsegin{align\*}width=6in]{./Figures/SMMcontour.pdf}
...
\write18{if egin{align\*} ! -f \texname.bib ]; then touch \texname.bib  ; fi}\write18{if egin{align\*} ! -f \texname-Add.bib ]; then touch \texname-Add.bib  ; fi}\bibliography{economics,\texname,\texname-Add}

\trp{
\pagebreak
\hypertarget{Appendices}{} % Allows link to egin{align\*}url-of-paper]#Appendices
\ifthenelse{\boolean{Web}}{}{% Web version has no page headers
  \cheadegin{align\*}Appendices]{Appendices}      % but PDF version does
