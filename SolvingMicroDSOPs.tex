...
\newcommand{\ifFix}{\ifthenelse{\boolean{ToFix}}}
...
%  \mbox{% Only known way to center the table in HTML version
  \begin{center}
    \mbox{
...
\ifFix{\marginpar{\tiny In all figs, replace gothic h with notation corresponding to the lecture notes.}}
...
the corresponding $\cFunc_{\EndStepm}$ values used to produce our
...
The easiest answer would be linear extrapolation; for example, if the bottommost gridpoint is $\aVecMin = \vctr{{a}}\mathtt{_{T-1}[1]}$ and the corresponding level of consumption is $\cMin = \cFunc_{\EndStepm}(a_1)$ we could calculate the `marginal propensity to have consumed' $\varkappa_{1}=\Alt{\cFunc}_{\EndStepm}^{{a}}(\aVecMin)$ and construct the approximation as the linear extrapolation below $\aVecMin$:
\begin{equation}\begin{gathered}\begin{aligned}
      \Alt{\cFunc}_{\EndStepm}({a}_{T-1})  &  \equiv \cMin + ({a}_{T-1}-\aVecMin)\varkappa_{1}  \label{eq:ExtrapLin}
    \end{aligned}\end{gathered}\end{equation}
...
linear function like $\Alt{\vFunc}^{{m}}_{\EndStepm}$, because there is no chance that the linear extrapolation below $\aMin$ will correctly predict $\vFunc^{{a}}_{\EndStepm}(\underline{a}_{T-1})=\infty.$ To see what
...
(because the expectation for any ${a}_{\stge-1} \leq \underline{a}_{T-1}$ is undefined).
