...
\begin{verbatimwrite}{./cctwMoM/TighterThreeFuncsExplain.tex}
  ...
  \begin{equation*}\begin{gathered}\begin{aligned}
        \bar{\vFunc}_{t}(\mNrm)  & = \uFunc(\bar{\cNrm}_{t}(\mNrm))+ \bar{\vEOPt}(\mNrm-\bar{\cNrm}_{t}(\mNrm)),
      \end{aligned}\end{gathered}\end{equation*}
  ...
  \begin{equation*}\begin{gathered}\begin{aligned}
        \bar{\vFunc}_{t}(\mNrm)  & = \uFunc( \MPCmax_{t} \aboveMin \mNrm)+ \bar{\vEOPt}((1-\MPCmax_{t})\aboveMin \mNrm),
      \end{aligned}\end{gathered}\end{equation*}
  which is easy to compute because $\bar{\vEOPt}(\aNrm_{t}) = \DiscFac \bar{\vFunc}_{t+1}(\aNrm_{t}\RNrm+1)$
...
\item For each ${a}_{t,i}$ in \texttt{aVec}, construct \texttt{$\cFunc_{\bar{t}}$} as follows:
  \begin{equation}\begin{gathered}\begin{aligned}
      ...
      \end{aligned}\end{gathered}\end{equation}
  \MPCMatch{and similarly construct the corresponding $\cFunc_{\overline{t}}^{a}({a}_{t,i})$}{}
  We also construct
...
to the derivatives of $\vEOPt({a}_{t},\varsigma_{t})$.
\ifhabits{Structurally these functions are very similar to the
  $\vEOPt({a}_{t},h_{t})$ functions defined in
...
$\bar{\vEOPt}^{a}({a}_{t})$ and $\hat{\varsigma}_{t}({a}_{t})$ will be
constructed.
...
\begin{comment} % at suggestion of WW, this section was removed as unnecessary for the current model, which solves for the converged rule very fast
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        \\  \Opt{\vFunc}^{\prime}({a}_{t})  & = \DiscFac \Rfree \PermGroFac_{t+1}^{-\CRRA} \bar{\vFunc}^{\prime}(\RNrm_{t+1} {a}_{t}+1).
      \end{aligned}\end{gathered}\end{equation}
...
agents are dead by $s=91$ as assumed by Cagetti. % Note that in addition to a typical time-invariant discount factor $\beth$, there is a time-varying discount factor $\byage{\DiscFac}_{s}$ in (\ref{eq:lifecyclemax}) which can be used to capture the effect of age-varying demographic variables (e.g.\ changes in family size).
