...
# ---------------------------------------------------------------------------
# Write a venv-local Jupyter kernel spec with an absolute Python path.
# This lives INSIDE the venv (no global ~/Library/Jupyter registration),
# so multiple repos can coexist without overwriting each other's kernels.
# Cursor/VS Code discovers kernels inside the selected interpreter's venv.
#
# Also removes any stale global "solvingmicrodsops" kernel that older
# setups may have installed via `ipykernel install --user`.  That global
# kernel hardcodes a single repo's Python path and breaks every other
# clone that uses the same name.
# ---------------------------------------------------------------------------
ensure_venv_jupyter_kernel() {
    local venv="$1"
    [[ -z "$venv" ]] && return 0
    "$venv/bin/python" -c "import ipykernel" 2>/dev/null || return 0

    # --- Remove stale global kernel if present ---
    local global_kernel=""
    if [[ -d "$HOME/Library/Jupyter/kernels/solvingmicrodsops" ]]; then
        global_kernel="$HOME/Library/Jupyter/kernels/solvingmicrodsops"
    elif [[ -d "$HOME/.local/share/jupyter/kernels/solvingmicrodsops" ]]; then
        global_kernel="$HOME/.local/share/jupyter/kernels/solvingmicrodsops"
    fi
    if [[ -n "$global_kernel" ]]; then
        rm -rf "$global_kernel"
        echo "  Removed stale global Jupyter kernel at $global_kernel"
    fi

    # --- Write venv-local kernel spec ---
    local kernel_dir="$venv/share/jupyter/kernels/python3"
    mkdir -p "$kernel_dir"
