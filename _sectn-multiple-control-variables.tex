...
Assume we have the approximations $\Alt{\vFunc}_{\Cntn}^{\partial a}$ and $\Alt{\vFunc}_{\Cntn}^{\partial \shr}$ in hand and want to proceed.  Given the formulation in \eqref{eq:Bellmanundated}, nonlinear equation solvers can find the solution to a set of simultaneous equations.  Thus we could ask one to solve
\begin{equation}\begin{gathered}\begin{aligned}
      \cNrm_{\prdt}^{-\CRRA}  & = \Alt{\vFunc}^{\partial a}_{{\cntn}(\prdt)}(\mNrm_{\prdt}-\cNrm_{\prdt},\shr_{\prdt}) %\label{eq:FOCwrtcMultContr}
      \\      0  & = \Alt{\vFunc}^{\partial \shr}_{{\cntn}(\prdt)}(\mNrm_{\prdt}-\cNrm_{\prdt},\shr_{\prdt}) \label{eq:FOCwrtw}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{itemize}
  \item \textbf{Employed branch.} The consumer receives ordinary labor income
    ...
    $\mNrm^{\text{emp}} = \RNrmByG\,\kNrm + \tranShkEmp$.
  \item \textbf{Unemployed branch.} The consumer receives unemployment
    ...
    $\mNrm^{\text{unemp}} = \RNrmByG\,\kNrm + \mu$.
\end{itemize}
...
which chooses $\cNrm$ given $\mNrm$. The DAG for a single {\interval} is:
...
\begin{equation*}
  \vFunc_{\text{branch},\arvl}(\kNrm)
  \;=\;
  p_{\text{emp}}\,\vFunc_{\text{emp},\arvl}(\kNrm)
  \;+\;
  (1 - p_{\text{emp}})\,\vFunc_{\text{unemp},\arvl}(\kNrm).
\end{equation*}
...
$\DiscFac$                   & \verb|\DiscFac|              & \texttt{DiscFac}     & Time-preference discount factor \\
$\vFunc_{\DiscFac,\dcsn}$    & \verb|\vFunc_{\DiscFac,\dcsn}| & \texttt{vDcsn}    & Decision value: $\DiscFac \cdot \vCntn$ (no optimization) \\
$\vFunc_{\DiscFac,\cntn}$    & \verb|\vFunc_{\DiscFac,\cntn}| & \texttt{vCntn}    & Continuation value function (value at exit) \\
$\vFunc_{\DiscFac,\arvl}$    & \verb|\vFunc_{\DiscFac,\arvl}| & \texttt{vArvl}    & Arrival value function ($= \vDcsn$; no shocks) \\
\hline
\end{tabular}
\end{center}
\end{table}
...
\begin{verbatimwrite}{./Equations/FOCport.tex}
\begin{equation}\begin{gathered}\begin{aligned}
  0 & = \Ex_{\dcsn}\left[
...
\end{aligned}\end{gathered}\end{equation}
\end{verbatimwrite}
\input{./Equations/FOCport.tex}\unskip
...
\UnifiedNote{𝒱_port decision: ς*(k) = argmax_ς E_ζ[ℰ_port(gᵥₑ(k, ς, ζ))] where gᵥₑ: (k, ς, ζ) → m̌ = k(ςR̃ + (1−ς)R) + θ, ζ = (R̃, θ)}
         \end{aligned}\end{gathered}\end{equation}
\end{verbatimwrite}
\input{./Equations/shrDecision}\unskip
...
Math Symbol & \LaTeX{} Macro & Python Variable & Description \\
\hline
$\kNrm$             & \verb|\kNrm|             & \texttt{kNrm}        & Normalized capital (arrival state, k-type) \\
...
$\check{\mNrm}$     & \verb|\check{\mNrm}|     & \texttt{mNrmCheck}   & Post-shock market resources (continuation state, m-type) \\
$\Rport$            & \verb|\Rport|            & \texttt{Rport}       & Portfolio-weighted return factor: $\Rfree + (\Risky - \Rfree)\shr$ \\
$\Risky$            & \verb|\Risky|            & \texttt{Risky}       & Stochastic risky-asset return factor \\
$\Rfree$            & \verb|\Rfree|            & \texttt{Rfree}       & Risk-free return factor \\
$\tranShkEmp$       & \verb|\tranShkEmp|       & \texttt{tranShkEmp}  & Transitory income shock \\
$\RNrmByG$          & \verb|\RNrmByG|          & \texttt{RNrmByG}     & Normalized return factor $\Rfree / \PermGroFac$ (used when $\bar{\shr}=0$) \\
$\vFunc_{\shr,\dcsn}$ & \verb|\vFunc_{\shr,\dcsn}| & \texttt{vDcsn}  & Decision value: $\max_{\shr} \Ex[\vCntn]$ or $\Ex[\vCntn]$ if $\shr$ fixed \\
$\vFunc_{\shr,\cntn}$ & \verb|\vFunc_{\shr,\cntn}| & \texttt{vCntn}  & Continuation value function (post-shock) \\
$\vFunc_{\shr,\arvl}$ & \verb|\vFunc_{\shr,\arvl}| & \texttt{vArvl}  & Arrival value function ($= \vDcsn$; no pre-decision shocks) \\
\hline
\end{tabular}
\end{center}
\end{table}
...
Math Symbol & \LaTeX{} Macro & Python Variable & Description \\
\hline
$\mNrm$        & \verb|\mNrm|        & \texttt{mNrm}        & Normalized market resources (arrival state, m-type) \\
$\cNrm$        & \verb|\cNrm|        & \texttt{cNrm}        & Normalized consumption (control variable) \\
$\aNrm$        & \verb|\aNrm|        & \texttt{aNrm}        & Normalized end-of-stage assets (continuation state, k-type) \\
$\vFunc_{\cFunc,\dcsn}$ & \verb|\vFunc_{\cFunc,\dcsn}| & \texttt{vDcsn} & Decision value function: $\max_{\cNrm} \uFunc(\cNrm) + \vCntn(\mNrm - \cNrm)$ \\
$\vFunc_{\cFunc,\cntn}$ & \verb|\vFunc_{\cFunc,\cntn}| & \texttt{vCntn} & Continuation value function at stage exit \\
$\vFunc_{\cFunc,\arvl}$ & \verb|\vFunc_{\cFunc,\arvl}| & \texttt{vArvl} & Arrival value function ($= \vDcsn$; no pre-decision shocks) \\
$\uFunc$       & \verb|\uFunc|       & \texttt{uFunc}       & CRRA utility function \\
$\CRRA$        & \verb|\CRRA|        & \texttt{CRRA}        & Coefficient of relative risk aversion \\
\hline
\end{tabular}
\end{center}
\end{table}
...
\UnifiedNote{𝒱_cons_noshocks(m) = argmax_c u(c) + ℰ_cons_noshocks(m − c) [shock-free cons-noshocks stage; β=1 at stage; ℰ_cons_noshocks includes β via disc stage; tex vCntn ≡ ℰ_cons_noshocks]}
      \end{aligned}\end{gathered}\end{equation}
\end{verbatimwrite}
\input{./Equations/vBalances.tex}\unskip
...
\UnifiedNote{c⃗ = (ℰ^{\partial}_cons_noshocks(a⃗))^{−1/ρ} [EGM inversion; ℰ_cons_noshocks includes β via disc stage; tex vCntn' ≡ ℰ^{\partial}_cons_noshocks so (vCntn')^{−1/ρ} suffices]}
    \end{aligned}\end{gathered}\end{equation}
which, following \ifthenelse{\boolean{shortVersion}}{the endogenous gridpoints method (EGM)}{the procedure in subsection~\ref{subsec:egm}}, yields an approximated consumption function $\Alt{\cFunc}_{\prdt}(\mNrm)$.
