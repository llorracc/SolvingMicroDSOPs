...
\end{verbatimwrite}
\input{\econtexRoot/Equations/vEndTm1.tex}\unskip
where 
\begin{equation*}\begin{gathered}\begin{aligned}
...
Given any particular value of $\mStte$, a numerical maximization tool can now find the $\cNrm$ that solves \eqref{eq:vEndTm1} in a reasonable amount of time.
...
Piecewise linear `spline' interpolation as described above works well for generating a good approximation to the true optimal consumption function. However, there is a clear inefficiency in the program: Since it uses equation \eqref{eq:vEndTm1}, for every value of $\mStte$ the program must calculate the utility consequences of various possible choices of $\cNrm$ (and therefore $\aNrm_{\prdT-1}$) as it searches for the best choice.
...
as a function of $\cNrm$ under the assumption that $\mStte=3$.
...
intersect slightly below $\cNrm=2.5$.  Thus, increasing $\mStte$
...
$\cNrm$ given $\mStte$ using $\Aprx{\vFunc}_{({\prdT-1})_\cntn}({a}_{\prdT-1})$, the numerical optimization routine will
return the $\cNrm$ for which
$\uFunc^{{c}}(\cNrm) = \Aprx{\vFunc}^{{a}}_{({\prdT-1})_\cntn}(\mStte-\cNrm)$.  Thus, for
$\mStte=3$ the program will return the value of $\cNrm$ for which the downward-sloping
...
Similarly, if we ask the routine to find the optimal $\cNrm$ for $\mStte=4$, it finds the point of
...
function plotted earlier returned values very close to $\cNrm=2$ for both $\mStte=3$ and $\mStte=4$.

We would obviously obtain much better estimates of the point of intersection between $\uFunc^{{c}}(\cNrm)$ and $\vFunc_{({\prdT-1})_\cntn}^{{a}}(\mStte-\cNrm)$ if our estimate of $\Aprx{\vFunc}^{{a}}_{({\prdT-1})_\cntn}$ were not a step function.  In fact, we already know how to construct linear interpolations to functions, so the obvious next step is to construct a linear interpolating approximation to the \textit{expected marginal value of end-of-period assets function} at the points in $\vctr{a}$:
\begin{equation}\begin{gathered}\begin{aligned}
...
The well-known optimal solution is to spend half of total lifetime resources in period $\prdT-1$ and the remainder in period $\prdT (=\trmT)$.  Since total resources are known with certainty to be $\mStte+\hNrm_{\prdT-1}= \mStte+1$, and since ${\vFunc}_{\MidStp}^{{m}}(\mStte) = \uFunc^{{c}}(\cNrm)$, this implies that
\begin{equation}
