...
The well-known optimal solution is to spend half of total lifetime resources in period $\prdT-1$ and the remainder in period $\prdT (=\trmT)$.  Since total resources are known with certainty to be $\mNrm+\hNrm_{\prdT-1}= \mNrm+1$, and since $\vFunc_{\MidStg}^{m}(\mNrm) = \uFunc^{c}(\cNrm)$, this implies that
\begin{equation}
