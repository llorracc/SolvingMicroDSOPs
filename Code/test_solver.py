...
class TestConnector:
    ...
    def test_m_next_shape(self, connector, params):
        a = np.array([0.0, 1.0, 2.0])
        m = connector.m_next(a)
        assert m.shape == (3, params.n_shocks)

    def test_m_next_increasing_in_a(self, connector):
...
class TestTerminalPeriod:
    ...
    def test_vda_equals_marginal_utility(self, params):
        sol = make_terminal_solution(params)
        m = np.array([1.0, 2.0, 3.0])
        vda = sol[STAGE_NAME]['dcsn'].vda(m)
        expected = _marginal_utility(m, params.CRRA)
        np.testing.assert_allclose(vda, expected)
...
class TestOneBackwardStep:
    ...
    def test_marginal_value_positive(self, pile_T):
        ...
        vda = np.array([sol[STAGE_NAME]['dcsn'].vda(mi) for mi in m])
        assert np.all(vda > 0)
...
class TestCrossValidation:
    def test_cCntn_Tm1_matches_legacy(self, params, connector):
        ...
        new_vda = sol[STAGE_NAME]['cntn'].vda
        new_c = _inv_marginal_utility(np.array([new_vda(a) for a in a_test]),
                                      params.CRRA)
