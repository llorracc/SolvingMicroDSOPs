...
#
# The classes included in Resources are used by many different projects and are gathered one place for convenience. Specific significant classes (such as the EndOfPrd class) get their own library files.
#
# **_NOTE:_** If at all possible, please add to the "resources.py" file by updating _this_ Resources.ipynb file and then manually exporting to the "resources.py" file in the same directory (and then, of course, opening a Python command prompt and executing "import resources" to overwrite and update the "resource.pyc" file, which is ultimately used by import commands in other Python programs).
#
...
class DiscreteApproximation(object):
    ...
    def E(self, f=None):
        ...
        if f is None:
            ...
            if self.precise_summation:
                # High precision
                return math.fsum(np.multiply(self.pmf, f(self.X)))
            else:
...
class DiscreteApproximationTwoIndependentDistribs(object):
    def __init__(
        self, N1, cdf1, pdf1, invcdf1, N2, cdf2, pdf2, invcdf2, precise_summation=False
    ):
        """
        Given a cdf, pdf, and invererse cdf, and number of evenly-spaced bins
        (a partition) of a state space, find the conditional expected values
        ...
        state_cutoffs1 = invcdf1(probs_cutoffs1)
        bin_probs1 = np.zeros(N1)

        # Find the E[X|bin] values:
        def F1(x):
            return x * self.pdf1(x)
        ...
        # State cutoff values, each bin
        state_cutoffs2 = invcdf2(probs_cutoffs2)
        bin_probs2 = np.zeros(N2)

        # Find the E[X|bin] values:
        def F2(x):
            return x * self.pdf2(x)
    ...
    def E(self, f=None):
        ...
        if f is None:
            ...
            if self.precise_summation:
                return np.array(
                    [math.fsum(self.pmf1 * self.X1), math.fsum(self.pmf2 * self.X2)]
                )
            else:
            ...
            if self.precise_summation:
                return math.fsum(
                    a.flatten()
                )  # High-floating-point precision summation.
            else:
    ...
    def plot(self, x0, x1, x20, x21, gridsize=100):
        ...
        plt.hlines(
            self.pmf1.cumsum(), x0, x1, color="black", linestyles="--"
        )  # added by Tao to show equiprobable ranges
        plt.axis([x0, x1, 0, 1])
        plt.title("Discrete Approximation to Lognormal Distribution")
        plt.xlabel("theta")
        ...
        plt.hlines(
            self.pmf2.cumsum(), x0, x1, color="black", linestyles="--"
        )  # added by Tao to show equiprobable ranges
...
if __name__ == "__main__":
    ...
    z1 = stats.lognorm(sigma1, 0, np.exp(mu1))

    sigma2 = 0.1
    mu2 = -0.5 * (sigma2**2)
    N2 = 8
    # Create "frozen" distribution instance
    z2 = stats.lognorm(sigma2, 0, np.exp(mu2))

    TwoDimDiscApprox = DiscreteApproximationTwoIndependentDistribs(
        N1, z1.cdf, z1.pdf, z1.ppf, N2, z2.cdf, z2.pdf, z2.ppf
    )

    # Check that "raw expectation" is correct:
    assert (
        np.max(np.abs(TwoDimDiscApprox.E() - 1.0)) < 1e-16
    ), "Unconditional expectation is not correct!"
    ...
    assert (
        np.abs(manual_EgXY - EgXY) < 1e-12
    ), "Eg(X,Y) not equal between the values that have expectations."

    TwoDimDiscApprox.plot(0.7, 1.4, 0.7, 1.4)


# + code_folding=[0, 8]
class DiscreteApproximationTwoMeanOneIndependentLognormalDistribs(
    ...
    def __init__(self, N1, sigma1, N2, sigma2, precise_summation=False):
        ...
        # Create the class:
        super(
            DiscreteApproximationTwoMeanOneIndependentLognormalDistribs, self
        ).__init__(
            N1=N1,
            cdf1=distrib1.cdf,
            pdf1=distrib1.pdf,
            invcdf1=distrib1.ppf,
            N2=N2,
            cdf2=distrib2.cdf,
            pdf2=distrib2.pdf,
            invcdf2=distrib2.ppf,
            precise_summation=precise_summation,
        )
...
if __name__ == "__main__":
    ...
    assert (
        np.max(np.abs(TwoDimDiscApprox.mu1 - mu1)) < 1e-16
    ), "Mu1 not calculated correctly!"
    assert (
        np.max(np.abs(TwoDimDiscApprox.mu2 - mu2)) < 1e-16
    ), "Mu2 not calculated correctly!"
    ...
    assert (
        np.abs(manual_EgXY - EgXY) < 1e-12
    ), "Eg(X,Y) not equal manually calculated joint value."
    print("Eg(X,Y) *does* equal manually calculated joint value:")
    print(
        "\tE[g(X1, X2)] from class:"
        + str(EgXY)
        + "\n\tand from manual calc:"
        + str(manual_EgXY)
        + "\n\tnp.abs(manual_EgXY - EgXY):"
        + str(np.abs(manual_EgXY - EgXY))
    )
...
# + code_folding=[]
class DiscreteApproximationToTwoMeanOneIndependentLognormalDistribsWithDiscreteProb_Z_Event(
    DiscreteApproximationTwoMeanOneIndependentLognormalDistribs
):
    """
    ----------------------------------------------------------------------------
    ...
    def __init__(self, N1, sigma1, N2, sigma2, pZevent, z=0.0, precise_summation=False):
        ...
        if not 0 <= pZevent <= 1:
            raise Exception(
                "The probability that discrete event z = "
                + z
                + " has a probability not in the range [0,1]: pZevent = "
                + str(pZevent)
            )
        ...
        if z in self.X2:
            warn(
                "Discrete shock value "
                + z
                + " already exists in the RV discrete space self.X2. Please confirm that inputs are correct"
            )
        else:
...
if __name__ == "__main__":
    ...
    # Same here; as we increase order of magnitude, we'll see "error" between manual and numpy calc grow.
    z_value = 500.0
    ...
    TwoDimDiscApproxZ = DiscreteApproximationToTwoMeanOneIndependentLognormalDistribsWithDiscreteProb_Z_Event(
        ...
        N2=N2,
        sigma2=sigma2,
        pZevent=prob_Z_event,
        z=z_value,
        precise_summation=True,
    )
    # Try precise_summation=False to see errors emerge.

    # Check that mu calculated correctly:
    assert (
        np.max(np.abs(TwoDimDiscApproxZ.mu1 - mu1)) < 1e-16
    ), "Mu1 not calculated correctly!"
    assert (
        np.max(np.abs(TwoDimDiscApproxZ.mu2 - mu2)) < 1e-16
    ), "Mu2 not calculated correctly!"
    print("M1 and Mu2 were both calculated correctly!")
    ...
    assert (
        np.max(np.abs(TwoDimDiscApproxZ.E() - manual_EX)) < 1e-12
    ), "Unconditional expectation is not correct!"
    print(
        "Unconditional expectation *is* correct: E(X1, X2) =\n\tFrom class:"
        + str(TwoDimDiscApproxZ.E())
        + "\n\tManual calc:"
        + str(manual_EX)
    )
...
# + code_folding=[9]
class SimpleDiscreteRandomVariable:
    def __init__(
        self,
        values,
        probs,
    ...
    def E(self, f=None):
        ...
        if f is None:
            if self.precise_summation:
                # High precision
                return math.fsum(np.multiply(self.pk, self.xk))
            else:
                return self.mean()
        else:
...
if __name__ == "__main__":
    ...
    test = SimpleDiscreteRandomVariable(
        vals,
        probs,
        1.0,
        precise_summation=True,
    ...
    # With the manually calculated value:
    assert (
        np.max(np.abs(test.E() - manual_EX)) < 1e-12
    ), "Unconditional expectation is not correct!"
    print(
        "Unconditional expectation *is* correct: E(X) =\n\tFrom class:"
        + str(test.E())
        + "\n\tManual calc:"
        + str(manual_EX)
    )
    ...
    # Check for a random function:
    def g(x1, x2):
        return x1**2 + np.pi * x2  # g = lambda x1: x1**2 + 1.5
    ...
    assert (
        np.abs(manual_EgX - EgX) < 1e-16
    ), "Eg(X) not equal manually calculated joint value."
...
def setup_grids_expMult(minval, maxval, size, timestonest=20):
    """
    Create a 1D grid with multi-exponential spacing (nested exp) so that the
    growth rate from each point to the next is constant. Used for asset grids
    in DSOP solutions (multi-exponential grid spacing).

    Args:
        minval (float): Minimum grid value.
        maxval (float): Maximum grid value.
        size (int): Number of points in the grid.
        timestonest (int): Number of nested exponentiations (default 20).

    Returns:
        np.ndarray: Grid of length size.
    """
    i = 1
    gMaxNested = maxval
    while i <= timestonest:
...
def set_up_improved_EEE_a_grid(minval, maxval, size):
    """
    Create a 1D grid with triple-log spacing: points are spaced evenly in
    L(x) = log(1 + log(1 + log(1 + x))), then mapped back to x.
    ...
    gMinMin = 0.01 * minval
    gMaxMax = 10 * maxval
    gridMin = log(1 + log(1 + log(1 + gMinMin)))
    gridMax = log(1 + log(1 + log(1 + gMaxMax)))
    points = np.linspace(gridMin, gridMax, size)
    new_a_grid = exp(exp(exp(points) - 1) - 1) - 1
    return new_a_grid
...
class Utility(object):
    """
    Define a CRRA utility function (\\uFunc) in levels and
    ...
    def δ(self, c):
        """First derivative \\uFunc^{\\partial c}(c) = marginal utility."""
        if self.CRRA == 1:
            # Log case:
            return 1.0 / c
        else:
    ...
    def δδ(self, c):
        """Second derivative \\uFunc^{\\partial c\\,\\partial c}(c)."""
        if self.CRRA == 1:
            # Log case:
            return -(c ** (-2.0))
        else:
...
class UtilityWithCMin:
    """
    Define a CRRA utility function (\\uFunc) in levels and in
    first derivative, with an imposed lower bound on
    utility (via a lower bound on consumption, \\cMin).
    ...
    CRRA : float
        Coefficient of Relative Risk Aversion (\\CRRA).
    cMin : float
        Minimum consumption floor (\\cMin).
    """

    def __init__(self, CRRA, cMin):
        self.CRRA = CRRA
        self.cMin = cMin
        if CRRA == 1:
            # Log case:
            ...
            self.δ = self.δ_regular

    def δ_regular(self, c):
        """
        First derivative \\uFunc^{\\partial c} when CRRA != 1.
        """
        return c**-self.CRRA
    ...
    def call_regular(self, c):
        return np.maximum(c, self.cMin) ** (1.0 - self.CRRA) / (1.0 - self.CRRA)
