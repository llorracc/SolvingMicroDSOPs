...
class TestDiscStage:
    ...
    def test_marginal_scaling(self, params):
        ...
        rho = params.CRRA
        v_cntn = lambda x: np.sin(x)
        # consumed function consistent with v'_cntn = cos(x)
        cδ_cntn = lambda x: np.maximum(np.cos(x), 1e-300) ** (-1.0 / rho)
        stg = solve_disc(v_cntn, cδ_cntn, params)
        ...
        np.testing.assert_allclose(
            stg['arvl'].vδ(x), expected_factor * np.cos(x), rtol=1e-12)

    def test_cntn_perch_stores_original(self, params):
        v_cntn = lambda x: x ** 2
        cδ_cntn = lambda x: 2 * x
        stg = solve_disc(v_cntn, cδ_cntn, params)

        x = np.array([3.0])
        assert stg['cntn'].v(x) == 9.0
        assert stg['cntn'].cδ(x) == 6.0
...
class TestConsNoshocks:
    def test_monotone_consumption(self, params):
        ...
        cδ_cntn = lambda a: np.maximum(a, 1e-300)

        stg = solve_cons_noshocks(v_cntn, cδ_cntn, params, a_min=0.0)
        c_func = stg['dcsn'].c
...
class TestPortableShocksOnly:
    ...
    def test_matches_old_connector_value(self, params, shock_dstn):
        ...
        rho = params.CRRA
        v_nxt = lambda m: np.maximum(m, 1e-300) ** (1 - rho) / (1 - rho)
        cδ_nxt = lambda m: np.maximum(m, 1e-300)

        a_grid = build_a_grid(1e-10, params.a_max, params.a_grid_size)

        # Old: includes discount
        v_old = connector.continuation_value(v_nxt, a_grid)

        # New portable: no discount, just shock integration
        k_grid = a_grid.copy()
        stg = solve_portable(v_nxt, cδ_nxt, params, shock_dstn,
    ...
    def test_matches_old_connector_marginal(self, params, shock_dstn):
        """portable(Shr=0) vδ should match Connector.marginal_continuation_value
        ...
        rho = params.CRRA
        v_nxt = lambda m: np.maximum(m, 1e-300) ** (1 - rho) / (1 - rho)
        vδ_nxt = lambda m: np.maximum(m, 1e-300) ** (-rho)
        cδ_nxt = lambda m: np.maximum(m, 1e-300)

        a_grid = build_a_grid(1e-10, params.a_max, params.a_grid_size)

        vδ_old = connector.marginal_continuation_value(vδ_nxt, a_grid)
        ...
        discount = params.DiscFac * params.PermGroFac ** (1.0 - rho)
        vδ_new = stg['arvl'].vδ(k_grid)

        np.testing.assert_allclose(vδ_old, discount * vδ_new, rtol=1e-3)
