...
\end{comment}
  ...
  riskless and a risky asset is also solved.  Finally, a simple example
    ...
    \begin{verbatimwrite}{./Excerpts/the-problem.texinput}
      The usual analysis of dynamic stochastic programming problems packs a great many events (intertemporal choice, stochastic shocks, intertemporal returns, income growth, and more) into a single step in which the agent makes an optimal choice taking account of all the elements above. For the detailed analysis here, we will be careful to disarticulate everything that happens in the problem explicitly into separate steps so that each element can be scrutinized and understood in isolation.
...
\end{aligned}\end{gathered}\end{equation}
available to be spent on consumption $\cLvl_{t}$ (even for a consumer subject to a liquidity constraint $\cLvl \leq \mLvl$).
...
\item \textbf{\Decision}: All exogenous variables (like income shocks, rate of return shocks, predictable income growth $\PermGroFac$ have been realized (so that, e.g., $\mNrm_{t}$'s value is known) and the agent solves the optimization problem
  ...
  When we want to refer to a specific step within period $t$ we will do so by parenthesizing the $(t)$ and preceding it by an indicator character:
  \begin{center}
  \begin{tabular}{r|c|l|l}
    Step     & Indicator & Example & Explanation \\ \hline
  \Arrival    & ${\large \arvl}$ & $\vFunc_{\arvl(t)}({\kNrm}_{t})$ & value entering $t$
\\  \Decision & $\dcsn$ & $\vFunc_{\dcsn(t)}({\mNrm}_{t})$ & value of $t$-decision
\\  \Continuation & $\cntn$ & $\vFunc_{\cntn(t)}({\aNrm}_{t})$ & value of outgoing state
  \end{tabular}
\end{center}

This allows us to capture the fact that the value of the consumer's problem can be computed at any of the three steps (and is a function of a different state variable at each step).

Note that there is no need to use these subscripts for the model's variables; while a variable like $a_{t}=m_{t}-c_{t}$ takes on its value in the transition from the {\Decision} to the {\Continuation} step, it have only one unique value over the course of the period and therefore a notation like $\aNrm_{\arvl(t)}$ would be useless because the variable does not have a value until the continuation step is reached. Each variable in our problem has a unique value defined at some point during the period, so there is no ambiguity in referring to them with normal notation like $\aNrm_{t}$.
...
\begin{equation}\begin{gathered}\begin{aligned}
      \null{\vFunc}^{{m}}_{\dcsn(t)}({m}_{t})  & =  \Ex_{\BegStge} [\DiscFac\Rfree\PermGroFac_{t+1}^{-\CRRA}\null{\vFunc}^{{m}}_{\MidStgep}({m}_{t+1})] \label{eq:envelope}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \uFunc^{{c}}({c}_{t+1})  & = \null{\vFunc}^{{m}}_{\dcsn(t+1)}({a}_{t}\RNrm_{t+1}+\TranShkEmp_{t+1}) \label{eq:upctp1EqVpxtp1}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
      {\vPEndStge}({a}_{\stge})  & = \Ex_{\BegStge}[\DiscFac \Rfree \PermGroFac_{\stge+1}^{-\CRRA} \null{\vFunc}_{\MidStgep}^{{m}}({m}_{\stge})]  \label{eq:vEndPrimetdefn}
    \end{aligned}\end{gathered}\end{equation}
or, substituting from equation (\ref{eq:upctp1EqVpxtp1}) (and dropping function arguments for clarity),
\begin{equation}\begin{gathered}\begin{aligned}
%  \vPEndStge({a}_{\stge})  & = \Ex_{\BegStge}\left[\DiscFac \Rfree \PermGroFac_{\stge+1}^{-\CRRA} \uFunc^{{c}}\left(\cFunc_{\stge+1}(\RNrm_{\stge+1} {a}_{\stge}+{\TranShkEmp}_{\stge+1})\right)\right].  \label{eq:vEndPrimetOc}
      \vPEndStge\astge  & = \Ex_{\BegStge}\left[\DiscFac \Rfree \PermGroFac_{\stge+1}^{-\CRRA} \uFunc^{{c}}\left(\cFunc_{\stge+1}\mstge\right)\right].  \label{eq:vEndPrimetOfc}                             
    \end{aligned}\end{gathered}\end{equation}
...
The problem in the second-to-last period of life can now be expressed as
\begin{equation*}\begin{gathered}\begin{aligned}
      \null{\vFunc}_{\stge-1}({m}_{\stge-1})  & = \max_{{c}_{\stge-1}} ~~ \uFunc({c}_{\stge-1}) +
      \DiscFac \Ex_{\cntn(\stge-1)} \left[\vNormedPermGroFac{\vFunc}_{\MidStge}(({m}_{\stge-1}-{c}_{\stge-1})\RNrm_{\stge} + \TranShkEmp_{\stge})\right],
\end{aligned}\end{gathered}\end{equation*}
where, to review, $\Ex_{\cntn(\stge-1)}$  indicates that the expectation is taken as of the end of period $\stge-1$.
...
\renewcommand{\stge}{T} % For much of this document, the problem will be about T-1 vs T
...
so we can rewrite the maximization problem that defines the middle stage of the period {$\dcsn$} as \ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny Be sure to discuss \eqref{eq:vEndTm1} explicitly, as it is implemented directly in the program as per equation \eqref{eq:vEndTm1prog}.}}{}
...
The heart of the program responsible for computing an estimated consumption function begins in ``Solving the Model by Value Function Maximization,'' where a grid characterizing the possible values of market resources ($m$) is initialized, and for each of the $m$ values, the consumption values $c$ that solve the minimization problem equivalent to \eqref{eq:vEndTm1} are computed using \href{https://scipy.org}{scipy}'s minimize function. %This optimized ${c}$ is used to calculate the value function at each point in the ${m}$ grid. The process is repeated until all $m$ grid points have been evaluated.
...
This is accompished in ``An Interpolated Consumption Function,'' where the \texttt{InterpolatedUnivariateSpline} function is called from the \texttt{scipy} module to define an approximation to the consumption function, which we will designate $\Alt{\cFunc}_{T-1}({m}_{T-1})$. That is, when called with an ${m}_{T-1}$ that is equal to one of the points in \texttt{mVec\_int}, $\Alt{\cFunc}_{T-1}$ returns the associated value of $\vec{\mathtt{c}}_{\mathtt{T-1}}$, and when called with a value of ${m}_{T-1}$ that is not exactly equal to one of the \texttt{mVec\_int}, returns the value of ${c}$ that reflects a linear interpolation between the $\vec{\mathtt{c}}_{\mathtt{T-1}}$ points associated with the two \texttt{mVec\_int} points immediately above and below ${m}_{T-1}$.  Thus if the function is called with ${m}_{T-1} = 1.75$ and the nearest gridpoints \ifthenelse{\boolean{MyNotes}}{\marginpar{\tiny Go to the optimal consumption figure and show the connect-the-dots; point out that it's more obvious for the ${\vFunc}_{T-1}$.}}{} are ${m}_{j,T-1}=1$ and ${m}_{k,T-1}=2$ then the value of ${c}_{T-1}$ returned by the function would be $(0.25 {c}_{j,T-1}+0.75 {c}_{k,T-1})$. We can define a numerical approximation to the value function $\Alt{\vFunc}_{T-1}({m}_{T-1})$ in an exactly analogous way.
...
For any given value of ${a}_{T-1}$, notice that there is a good chance that the program may end up calculating the corresponding $\vMidStge$ many times while maximizing utility from different ${m}_{T-1}$'s.  For example, it is possible that the program will calculate the value of ending the period with ${a}_{T-1}=0$ dozens of times.  It would be much more efficient if the program could make the calculation of $\vMidStge(0)$ once and then merely recall the value when it is needed again.
...
$\vec{\mathtt{a}}_{\mathtt{T-1}}$
(\texttt{aVec}
in the code). %, designating the specific points $\vec{\mathtt{a}}\mathtt{[i]_{T-1}}$.
%For each of these values in $\vec{\mathtt{a}}_{\mathtt{T-1}}$,
Next, construct $\vec{\mathtt{\vFunc}}_{\mathtt{\MidStgem}} =
\vFunc_{\MidStgem}(\vec{\mathtt{a}}_{\mathtt{T-1}})$ using equation
...
These lists contain the points of the $\vec{a}_{\mathtt{\stge-1}}$ and $\vec{\vFunc}_{\mathtt{\stge-1}}$ vectors, respectively.
...
\begin{equation}\begin{gathered}\begin{aligned}
    ...
    \end{aligned}\end{gathered}\end{equation}
at the points in \texttt{aVec} yielding $\vec{\mathtt{\vFunc}}\mathtt{^{a}_{\EndStgem}}$ (the vector of expected end-of-period-$(T-1)$ marginal values of assets corresponding to \texttt{aVec}),  %$\{\{\vec{\mathtt{a}}}\mathtt{_{T-1}},\vFunc_{\EndStgem}^{{a}}(\vec{\mathtt{a}[1]}_{T-1}\},\{\vec{\mathtt{a}}_{(T-1)},\vFunc_{\EndStgem}^{{a}}\}\ldots\}$
 and construct
$\Alt{\vFunc}_{\EndStgem}^{{a}}({a}_{T-1})$ as the linear
...
of the $\vec{\mathtt{a}}_{\mathtt{\stge-1}}$ gridpoints, with the idea that we will construct
...
The easiest answer would be linear extrapolation; for example, if the bottommost gridpoint is $\aVecMin = \vec{\mathtt{a}}\mathtt{_{T-1}[1]}$ and the corresponding level of consumption is $\cMin = \cFunc_{\EndStgem}(a_0)$ we could calculate the `marginal propensity to have consumed' $\varkappa_{0}=\Alt{\cFunc}_{\EndStgem}^{{a}}(\aVecMin)$ and construct the approximation as the linear extrapolation below $\aVecMin$:
\begin{equation}\begin{gathered}\begin{aligned}
      \Alt{\cFunc}_{\EndStgem}({a}_{T-1})  &  \equiv \cMin + ({a}_{T-1}-\aVecMin)\varkappa_{0}  \label{eq:ExtrapLin}
    \end{aligned}\end{gathered}\end{equation}
...
linear function like $\Alt{\vFunc}^{{m}}_{\EndStgem}$, because there is no chance that the linear extrapolation below $a_{0}$ will correctly predict $\vFunc^{{a}}_{\EndStgem}(\underline{a}_{T-1})=\infty.$ To see what
...
transformation (Figure~\ref{fig:PlotOPRawVSFOC}). (The vertical axis label uses $\mathfrak{v}^{\prime}$ as an alternative notation for what in these notes we designate as $\vFunc^{a}_{\EndStge}$).
...
point in $\vec{\mathtt{m}}\mathtt{_{T-1}}$ (\texttt{mVec} in the code), to use a
...
\begin{equation}\begin{gathered}\begin{aligned}
       {\vec{\mathtt{\cNrm}}\mathtt{_{T-1}}}  & = \left(\vFunc^{{a}}_{\EndStgem}(\vec{\mathtt{a}}_{T-1})\right)^{-1/\CRRA}
...
But with mutually consistent values of $\vec{\mathtt{c}}\mathtt{_{T-1}}$ and $\vec{\mathtt{a}}\mathtt{_{T-1}}$ (consistent, in the sense that they are the unique optimal
values that correspond to the solution to the problem), we can
obtain the $\vec{\mathtt{m}}\mathtt{_{T-1}}$ vector that corresponds to both of them from
\begin{equation}\begin{gathered}\begin{aligned}
      \vec{\mathtt{m}}\mathtt{_{T-1}}  & = {\vec{\mathtt{\cNrm}}\mathtt{_{T-1}}+\vec{\mathtt{a}}\mathtt{_{T-1}}}.
    \end{aligned}\end{gathered}\end{equation}
...
\texttt{mVec\_egm}. With the $\vec{\cFunc}_{\EndStgem}$ values in hand, the notebook can generate a set of $\vec{\mathtt{m}}\mathtt{_{T-1}}$ and ${\vec{\mathtt{\cNrm}}\mathtt{_{T-1}}}$
...
\begin{equation}\begin{gathered}\begin{aligned}
      \MPC  & = 1- \left(\DiscFac  \Ex_{\BegStge}[\Risky_{t+1}^{1-\CRRA}]\right)^{1/\CRRA} \label{eq:MPCExact}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{equation*}\begin{gathered}\begin{aligned}
      \vPEndStge({a}_{t})  & = \Ex_{\BegStge}[\DiscFac \Rfree \PermGroFac_{t+1}^{-\CRRA}
