...
\begin{figure}
  ...
  \caption{End-Of-Period Value $v_{({\prd-1})_\cntn}(a_{\prd-1})$ (solid) versus $\Aprx{v}_{({T-1})_\cntn}(a_{T-1})$ (dashed)}
...
\begin{equation}\begin{gathered}\begin{aligned}
    ...
    \end{aligned}\end{gathered}\end{equation}
yielding $\vctr{v}{^{a}_{({\prd-1})_\cntn}}$ (the vector of expected end-of-period-$(T-1)$ marginal values of assets corresponding to \code{aVec}),  %$\{\{\vctr{a}}\code{_{\prdT-1}},\vFunc_{({\prdT-1})_\cntn}^{a}(\vctr{{a}[1]}_{\prdT-1}\},\{\vctr{a}_{(T-1)},\vFunc_{({\prdT-1})_\cntn}^{a}\}\ldots\}$
and construct
