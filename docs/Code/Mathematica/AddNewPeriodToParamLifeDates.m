...
AddNewPeriodToParamLifeDates := Block[{},
...
AppendTo[RLife, RFree]; 

(* Assume that discount factor for the latest period is \[Beta] *)
AppendTo[\[Beta]Life, \[Beta]]; 

(* Assume that growth factor for the latest period is the constant \[ScriptCapitalG] *)
AppendTo[\[ScriptCapitalG]Life, \[ScriptCapitalG]]; 
...
(* This defines the maximum amount that is accessible to a consumer at the beginning
of the period, who does not have a constraint this period.
  Suppose we have solved the problem (constrained or not) up to t+1
, now 
(1) if there is no constraint this period 
(i.e. Last[yBorrowableLife]>=Last[yMinLife])
, then we can simply solve the problem knowing
 comfortably that Last[yBorrowableLife] can be borrowed against.
, then the maximum accessible is 
  Last[\[ScriptCapitalG]Life]*(Last[yBorrowableLife]+Last[\[GothicH]BorrowableLife])/Last[RLife]
. And that is it.
(2) if there is a constraint this period (i.e. Last[yBorrowableLife]<Last[yMinLife])
, we first solve the problem assuming the constraint is not there
, and then apply the constraint to the unconstrained solution.
  Hence here the maximum amount that is accessible to him/her 
(who we assume is not constrained this period) is defined by 
  Last[\[ScriptCapitalG]Life]*(Last[yMinLife]+Last[\[GothicH]BorrowableLife])/Last[RLife]
.
 *)
...
AppendTo[\[Kappa]MaxLife,1/(1+\[Theta]Prob[[1]]^(1/\[Rho])*Last[\[Lambda]MaxLife]/Last[\[Kappa]MaxLife])];
];
