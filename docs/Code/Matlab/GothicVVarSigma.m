function GothicVVarSigma = GothicVVarSigma(a,Rho,BetaLife,RLife,GammaLife,NumOfThetaShockPoints,ThetaVals,mLowerBoundLife,DeltaGothicHLife,KappaMin,PeriodsUntilT,chiIntData,Constrained,calRVals,VarSigma)
%==========================================================================%
...
%   Outputs:                                                               %
%       GothicVVarSigma - Marginal Value function next period              %
%                                                                          %
%__________________________________________________________________________%
%PeriodsUntilT
GothicVVarSigma = zeros(size(a));
for i = 1:length(calRVals)
    for j = 1:NumOfThetaShockPoints
        ...
        elseif PeriodsUntilT == 1
            for k = 1:length(a)
                bbR = RLife(PeriodsUntilT+1)+ (calRVals(i)-RLife(PeriodsUntilT+1)).*VarSigma;
                mtp1 = a(k).*bbR./GammaLife(PeriodsUntilT+1)+ThetaVals(j);
                GothicVVarSigma(k) = GothicVVarSigma(k)+BetaLife(PeriodsUntilT+1).*((GammaLife(PeriodsUntilT+1)).^(-Rho)).*(1/NumOfThetaShockPoints).*(1/length(calRVals)).*(calRVals(i)-RLife(PeriodsUntilT+1)).*uP(mtp1,Rho);
            end;
        else
            for k = 1:length(a)
                bbR = RLife(PeriodsUntilT+1)+ (calRVals(i)-RLife(PeriodsUntilT+1)).*VarSigma;
                ...
                GothicVVarSigma(k) = GothicVVarSigma(k) + BetaLife(PeriodsUntilT+1).*((GammaLife(PeriodsUntilT+1)).^(-Rho)).*(1/NumOfThetaShockPoints).*(1/length(calRVals)).*(calRVals(i)-RLife(PeriodsUntilT+1)).*uP(con,Rho);
            end;
        end
    end
end
