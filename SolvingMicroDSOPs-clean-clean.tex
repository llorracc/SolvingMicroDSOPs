...
\makeatother 
 % allow latex to find custom things
...
\begin{equation}\begin{gathered}\begin{aligned}
    ...
    \end{aligned}\end{gathered}\end{equation}
available to be spent on consumption $\mathbf{c}_{t}$ for a consumer subject to a liquidity constraint that requires $\mathbf{c} \leq \mathbf{m}$ (though we are not imposing such a constraint yet - see subsection~\ref{subsec:LiqConstrSelfImposed} for further discussion).
...
Now define a new optimization problem:
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        a_{t}                       & = m_{t}-c_{t}                                                                     \\
        k_{t+1}                     & = a_{t}                                                                                \\
        b_{t+1}                     & = \underbrace{\left(R/\mathcal{G}_{t+1}\right)}_{\equiv \mathcal{R}_{t+1}}k_{t+1} \\
...
%  }
  ...
  Notice that the value functions at different {\move}s of the {\stg} have distinct state variables.  Only $k$ is known at the beginning of the {\stg}, and other variables take on their values with equations like $b = k \mathcal{R}$ and $m = b+\pmb{\theta}.$  We will refer to such within-the-{\stg} creation of variables as {\evltns}.%  Thus, the consumption problem has two {\evltns}: from $\kNrm$ to $\mNrm$ and from $\mNrm$ to $\aNrm$.

\subsection{\Trnsns}

  In the backward-induction world of Bellman solutions, to solve the problem of a particular {\interval} we must start with an end-of-{\interval} (continuation) value function, which we designate by explicitly including the {\interval} indicator in the subscript:
  \begin{equation}\begin{gathered}\begin{aligned}
...
Similarly, in order to solve the problem of any {\stg}, we must endow it with an end-of-{\stg} continuation-value function.  For the last {\stg} in a {\interval} the end-of-{\stg} function is taken to be end-of-{\interval} value function:
  \begin{equation}\begin{gathered}\begin{aligned}
...
%%% DE: the arguments of the functions being plotted should not have a T-1 subscript, i.e. the horizontal axis of figure 2 should be labled m not m_{T-1}; same with figures 3 etc
...
%%% AL: Please provide href for your interpolation package and add to global bib
...
\end{figure}
%%% DE: In this figure, the argument to v' should be (m-c) and of u' should be c, and the horizontal axis should be labeled c not c_{T-1}
...
%%% DE: same kinds of comments as before

%%% DE: Also, make the functions math roman rather than gothic font
...
\begin{equation}\begin{gathered}\begin{aligned}
    ...
    \end{aligned}\end{gathered}\end{equation}
with the idea that we will construct an approximation of the consumed function $\Aprx{\mathrm{c}}_{(t-1)_\cntn}(a)$ as the interpolating function connecting these $\{\vctr{a},\vctr{c}\}$ points.
...
%%% DE: Please always include a link rather than just using \texttt (I've replaced your \texttt{scipy.interpolate} below with what should have been there)
...
\begin{comment} % at suggestion of WW, this section was removed as unnecessary for the current model, which solves for the converged rule very fast
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        ...
        \\  \tilde{v}^{m}(a_{t})  & = \beta R \mathcal{G}_{t+1}^{-\rho} \bar{v}^{m}(\mathcal{R}_{t+1} a_{t}+1).
      \end{aligned}\end{gathered}\end{equation}
...
%%% AL: This needs to be rewritten in light of the foregoing discussion.
...
\begin{comment}
  ...
  and the maximization problem is
    \begin{equation*}\begin{gathered}\begin{aligned}
          ...
          \\      m_{t+1}  & = (m_{t}-c_{t})\mathfrak{R}_{t+1} + \pmb{\theta}_{t+1}
          \\  0       \leq & \Shr_{t}  \leq 1, \label{eq:noshorts}
        \end{aligned}\end{gathered}\end{equation*}

  The first order condition with respect to $c_{t}$ is almost identical to that in the single-control problem, equation (\ref{eq:upceqEvtp1}); the only difference is that the nonstochastic interest factor $R$ is now replaced by the portfolio return ${\mathfrak{R}}_{t+1}$,
    \begin{equation}\begin{gathered}\begin{aligned}
          u^{c}(c_{t})  & = \beta \ExEndStg [{\mathfrak{R}}_{t+1} v^{m}_{t+1}(m_{t+1})] \label{eq:valfuncFOCRtilde},
        \end{aligned}\end{gathered}\end{equation}
  and the Envelope theorem derivation remains the same, yielding the Euler equation for consumption
    \begin{equation}\begin{gathered}\begin{aligned}
          u^{c}(c_{t})  & = \ExEndStg[\beta {\mathfrak{R}}_{t+1} u^{c}(c_{t+1})]. \label{eq:EulercRiskyR}
        \end{aligned}\end{gathered}\end{equation}

  The first order condition with respect to the risky portfolio share is
    \begin{equation}\begin{gathered}\begin{aligned}
          0  & = \ExEndStg[{v}_{\MidStgNxt}^{m}(m_{t+1})({\mathbf{R}}_{t+1}-R){a}_{t}] \notag
          \\         & = \ExEndStg\left[u^{c}\left(\mathrm{c}_{t+1}(m_{t+1})\right)({\mathbf{R}}_{t+1}-R)\right]{a}_{t} 
          \\         & = \ExEndStg\left[u^{c}\left(\mathrm{c}_{t+1}(m_{t+1})\right)({\mathbf{R}}_{t+1}-R)\right], \label{eq:FOCw}        
        \end{aligned}\end{gathered}\end{equation}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        \vMidStg^a  & = \ExEndStg[\beta {\mathfrak{R}}_{t+1}v_{t+1}^{m}(m_{t+1})] = \ExEndStg[\beta {\mathfrak{R}}_{t+1}{u}_{t+1}^{c}(\mathrm{c}_{t+1}(m_{t+1}))]
      \end{aligned}\end{gathered}\end{equation}
  \begin{equation}\begin{gathered}\begin{aligned}
        \vMidStg^{\Shr}  & = \ExEndStg[\beta ({\mathbf{R}}_{t+1}-R){v}_{t+1}^{m}(m_{t+1})  ]a_{t} = \ExEndStg[\beta ({\mathbf{R}}_{t+1}-R){u}_{t+1}^{c}(\mathrm{c}_{t+1}(m_{t+1}))  ]a_{t} \notag
      \end{aligned}\end{gathered}\end{equation}
...
Since in this setup bank balances have been determined before the consumption problems starts, we need to rewrite the consumption {\stg}  as a function of bank balances that will have resulted from the portfolio investment $b$, combined with the income shocks $\pmb{\theta}$:
\begin{equation}\begin{gathered}\begin{aligned}
...
%%% AL: This needs to be reviewed (1) to ensure consistency with the foregoing (ince it is a one-stage-per-period problem, the notation probably will not need to change much if at all); and (2) to be sure it correctly describes what we do in the structural estimation HARK tool now.
...
%%% DE: Need reference for andrews2017measuring.  Put in SolvingMicroDSOPs-Add-Refs.bib
