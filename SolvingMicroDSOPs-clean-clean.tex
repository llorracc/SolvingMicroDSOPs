...
are rewritten from level variables $(\mathbf{m}, \mathbf{p})$ to the
...
\paragraph{Expectation operators across {\prchs}.}  The subscript on an expectation operator ${\mathbb{E}}$ indicates the information set at that {\prch}: ${\mathbb{E}}_{\arvl}$ conditions on the {\Arrival} state but not on any shocks realized between {\Arrival} and {\Decision}.  For adjacent {\prchs} at a {\interval} boundary---$\ExCntnPrd$ ({\Continuation} of {\interval} $t$) and ${\mathbb{E}}_{\ArvlPrdNxt}$ ({\Arrival} of {\interval} $t+1$)---the information sets are identical, so the two operators are mathematically interchangeable; the notational distinction reflects viewpoint (looking backward from $t$'s exit versus forward from $(t+1)$'s entry).
...
\begin{equation}\begin{gathered}\begin{aligned}
  ...
  v_{{\cntn}(t)}(a_{t}) = \beta {\mathbb{E}}_{\ArvlPrdNxt}[\mathcal{G}_{t+1}^{1-\rho}v_{\dcsn(t+1)}(\overbrace{(R / \mathcal{G}_{t+1})a_{t} +\tranShkEmp_{t+1}}^{m_{t+1}})], \label{eq:vCntnExpansion}
\end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
      u^{\partial}(m_{t}-a_{t}) = \vCntnPrd^{\partial}(a_{t}) & = {\mathbb{E}}_{\ArvlPrdNxt}[\beta \RNrmByG_{t+1}\mathcal{G}_{t+1}^{1-\rho}{v}^{\dm}_{\dcsn(t+1)}(m_{t+1})]  \label{eq:upceqEvtp1}
      \\                        & =  {\mathbb{E}}_{\ArvlPrdNxt}[\beta R\phantom{._{t+1}}\mathcal{G}_{t+1}^{\phantom{1}-\rho}{v}^{\dm}_{\dcsn(t+1)}(m_{t+1})]
      %
...
\begin{equation}\begin{gathered}\begin{aligned}
      {v}^{\dm}_{\dcsn(t)}(m_{t})  & =  {\mathbb{E}}_{\ArvlPrdNxt} [\beta R \mathcal{G}_{t+1}^{-\rho}{v}^{\dm}_{\dcsn(t+1)}(m_{t+1})] \label{eq:envelope}
      %
...
\hypertarget{eq-cEuler}{}
  \begin{equation}\begin{gathered}\begin{aligned}
        u^{\partial}(c_{t})  & = \ExCntnPrd[\beta R \mathcal{G}_{t+1}^{-\rho}u^{\partial}(c_{t+1})] \label{eq:cEuler}.
        %
...
\begin{equation*}\begin{gathered}\begin{aligned}
  v_{\CntnPrdLsT}(a)  & \leftassign \beta v_{\ArvlPrd}(a) 
                           \equiv \beta {\mathbb{E}}_{\ArvlPrd} \left[\PermGroFacAdjV v_{\DcsnPrd}(\underbrace{a \RNrmByG_{t} + \tranShkEmp_{t}}_{{m}_{t}})\right].
    \end{aligned}\end{gathered}\end{equation*}
...
\begin{equation}\begin{gathered}\begin{aligned}
      v_{\ArvlPrd}(a)   & = \PermGroFacAdjV\int_{0}^{\infty} \frac{\left(a \RNrmByG_{t}+ \tranShkEmpDummy\right)^{1-\rho}}{1-\rho}  d\mathcal{F}(\tranShkEmpDummy) \label{eq:NumDefInt}
...
\begin{equation}\begin{gathered}\begin{aligned}
  \mathrm{c}_{\DcsnPrdLsT}(m)  & = \argmax_{c} ~~ u(c) + \beta \int_{0}^{\infty} \frac{\left((m - c) \RNrmByG_{t}+ \tranShkEmpDummy\right)^{1-\rho}}{1-\rho}  d\mathcal{F}(\tranShkEmpDummy) . \label{eq:cFuncTm1}
...
\begin{figure}
  ...
  \caption{$v_{t-1}$ (solid) versus $\grave{v}_{t-1}(m)$ (dashed)}
...
\begin{figure}
  ...
  \caption{End-Of-Period Value $v_{{\cntn}(t-1)}(a_{t-1})$ (solid) versus $\grave{v}_{{\cntn}(t-1)}(a_{t-1})$ (dashed)}
...
Nevertheless, the consumption rule obtained when the approximating $\grave{v}_{{\cntn}(t-1)}(a_{t-1})$ is used instead of $v_{{\cntn}(t-1)}(a_{t-1})$ is surprisingly bad, as shown in figure \ref{fig:PlotComparecTm1AB}.  For example, when $m$ goes from 2 to 3, $\grave{\mathrm{c}}_{t-1}$ goes from about 1 to about 2, yet when $m$ goes from 3 to 4, $\grave{\mathrm{c}}_{t-1}$ goes from about 2 to about 2.05.  The function fails even to be concave, which is distressing because Carroll and Kimball~\citeyearpar{ckConcavity} prove that the correct consumption function is strictly concave in a wide class of problems that includes this one.
...
\begin{figure}
  ...
  \caption{$u^{\mathrm{c}}(\mathrm{c})$ versus $v_{{\cntn}(t-1)}^{\partial}(3-\mathrm{c}), v_{{\cntn}(t-1)}^{\partial}(4-\mathrm{c}), \grave{v}_{{\cntn}(t-1)}^{\partial}(3-\mathrm{c}), \grave{v}_{{\cntn}(t-1)}^{\partial}(4-\mathrm{c})$}
...
Now consider the derivative of $\grave{v}_{(t-1)}$.  Because the function is piecewise linear, its derivative $\grave{v}_{{\cntn}(t-1)}^{\partial}(a_{t-1})$ is a step function: constant between adjacent gridpoints, with jumps at each gridpoint.

The solid-line step function in Figure \ref{fig:Plot_ud_VS_vCntnd} depicts the actual value of
$\grave{v}_{{\cntn}(t-1)}^{\partial}(3-c)$.  When we attempt to find optimal values of
$c$ given $m$ using $\grave{v}_{{\cntn}(t-1)}(a_{t-1})$, the numerical optimization routine will
return the $c$ for which
$u^{c}(c) = \grave{v}^{\partial}_{{\cntn}(t-1)}(m-c)$.  Thus, for
$m=3$ the program will return the value of $c$ for which the downward-sloping
$u^{c}(c)$ curve intersects with the
$\grave{v}_{{\cntn}(t-1)}^{\partial}(3-c)$; as the diagram shows, this value is exactly equal to 2.
Similarly, if we ask the routine to find the optimal $c$ for $m=4$, it finds the point of
intersection of $u^{c}(c)$ with $\grave{v}_{{\cntn}(t-1)}^{\partial}(4-c)$; and as the diagram shows, this
...
and construct
$\grave{v}_{{\cntn}(t-1)}^{\partial}(a_{t-1})$ as the linear
...
\begin{figure}
  ...
  \caption{$v_{{\cntn}(t-1)}^{\partial}(a_{t-1})$ versus $\grave{v}_{{\cntn}(t-1)}^{\partial}(a_{t-1})$}
...
\begin{figure}
  ...
  \caption{$\mathrm{c}_{t-1}(m)$ (solid) Versus Two Methods for Constructing $\grave{\mathrm{c}}_{t-1}(m)$}
...
which would be linear in the perfect foresight case.\footnote{There is a corresponding inverse for the value function: $\vInv_{\cntn}(a_{t})=((1-\rho)v_{\cntn})^{1/(1-\rho)}$, and for the marginal marginal value function etc.}  We then construct a piecewise-linear interpolating approximation to the $\vInv_{t}^{\partial}$ function, $\grave{\vInv}_{\cntn}^{\partial}(a_{t})$, and for any $a$ that falls in the range $\{\vctr{a}[1],\vctr{a}[-1]\}$ we obtain our approximation of marginal value from:
\begin{equation}\begin{gathered}\begin{aligned}
      \grave{v}_{\dcsn}^{\partial}(a) & =
      [\grave{\vInv}^{\partial}(a)]^{-\rho}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \grave{\mathrm{c}}_{\cntn}(a) & \equiv \grave{\vInv}^{\partial}_{\cntn}(a_{t}) \label{eq:consumedfn}.    
...
\UnifiedNote{[no direct counterpart] — grid evaluation: π-vector from consumed function at xₑ gridpoints}
    \end{aligned}\end{gathered}\end{equation}
with the idea that we will construct an approximation of the consumed function $\grave{\mathrm{c}}_{{\cntn}(t-1)}(a)$ as the interpolating function connecting these $\{\vctr{a},\vctr{c}\}$ points.
...
%\renewcommand{\prdt}{T} 
This is the appropriate moment to ask an awkward question: How should an interpolated, approximated `consumed' function like $\grave{\mathrm{c}}_{{\cntn}(t-1)}(a_{t-1})$ be extrapolated to return an estimated `consumed' amount when evaluated at an $a_{t-1}$ outside the range spanned by $\{\vctr{a}[1],...,\vctr{a}[n]\}$?
...
\grave{\mathrm{c}}_{{\cntn}(t-1)}^{\partial}(\aVecMin)$ and construct the approximation as the linear extrapolation below $\vctratm[1]$ from:
\begin{equation}\begin{gathered}\begin{aligned}
      \grave{\mathrm{c}}_{{\cntn}(t-1)}(a)  &  \equiv \cMin + (a-\aVecMin)\varkappa_{1}  \label{eq:ExtrapLin}.
...
\end{figure}
Figure~\ref{fig:ComparecTm1AD} plots the actual consumption function $\mathrm{c}_{t-1}$ and the approximated consumption function $\grave{\mathrm{c}}_{t-1}$ derived by the method of endogenous grid points. Compared to the approximate consumption functions illustrated in Figure~\ref{fig:PlotcTm1ABC}, $\grave{\mathrm{c}}_{t-1}$ is quite close to the actual consumption function.
...
\begin{figure}
  ...
  \caption{$\mathrm{c}_{{\cntn}(t-1)}(a)$ versus
    $\grave{\mathrm{c}}_{{\cntn}(t-1)}(a)$, Multi-Exponential \code{aVec}}
...
\begin{figure}
  ...
  \caption{$v^{\partial}_{{\cntn}(t-1)}(a)$ vs.
    $\grave{\grave{v}}_{{\cntn}(t-1)}^{\partial}(a)$, Multi-Exponential \code{aVec}}
...
$\grave{\mathrm{c}}_{t}$ by interpolating a finite set of
...
\begin{itemize}
  ...
  \begin{equation}
    \cFuncAbove_{t}(m) \;=\;
      (m + h_{\Cntn})\,{\uline{\kappa}}_{t},
      \label{eq:cFuncAboveBrief}
  \end{equation}
  where ${\uline{\kappa}}_{t}$ is the minimal marginal propensity to
  ...
  $\cFuncBelow_{t}(m) < \mathrm{c}_{t}(m) <
...
\begin{equation}
  \omega_{t}(m) \;\equiv\;
    \frac{\mathrm{c}_{t}(m) - \cFuncBelow_{t}(m)}
...
is the logit-transformed moderation ratio.  Since the recovered $\mathrm{c}$
...
optimist and below by the pessimist.
  ...
  \begin{figure}
    ...
    \caption[Moderation Illustrated]{Moderation Illustrated: $\cFuncBelow_{t-1} < \grave{\mathrm{c}}_{t-1} < \cFuncAbove_{t-1}$}
...
Assume we have the approximations $\grave{v}_{\Cntn}^{\partial a}$ and $\grave{v}_{\Cntn}^{\partial \shr}$ in hand and want to proceed.  Given the formulation in \eqref{eq:Bellmanundated}, nonlinear equation solvers can find the solution to a set of simultaneous equations.  Thus we could ask one to solve
\begin{equation}\begin{gathered}\begin{aligned}
      c_{t}^{-\rho}  & = \grave{v}^{\partial a}_{{\cntn}(t)}(m_{t}-c_{t},\shr_{t}) %\label{eq:FOCwrtcMultContr}
      \\      0  & = \grave{v}^{\partial \shr}_{{\cntn}(t)}(m_{t}-c_{t},\shr_{t}) \label{eq:FOCwrtw}
    \end{aligned}\end{gathered}\end{equation}
...
$\mathfrak{R}$            & \verb|\mathfrak{R}|            & \texttt{Rport}       & Portfolio-weighted return factor: $R + ({\mathbf{R}} - R)\shr$ \\
${\mathbf{R}}$            & \verb|{\mathbf{R}}|            & \texttt{Risky}       & Stochastic risky-asset return factor \\
$R$            & \verb|R|            & \texttt{Rfree}       & Risk-free return factor \\
$\tranShkEmp$       & \verb|\tranShkEmp|       & \texttt{tranShkEmp}  & Transitory income shock \\
$\RNrmByG$          & \verb|\RNrmByG|          & \texttt{RNrmByG}     & Normalized return factor $R / \mathcal{G}$ (used when $\bar{\shr}=0$) \\
$v_{\shr,\dcsn}$ & \verb|v_{\shr,\dcsn}| & \texttt{vDcsn}  & Decision value: $\max_{\shr} {\mathbb{E}}[\vCntn]$ or ${\mathbb{E}}[\vCntn]$ if $\shr$ fixed \\
...
$v_{\mathrm{c},\dcsn}$ & \verb|v_{\mathrm{c},\dcsn}| & \texttt{vDcsn} & Decision value function: $\max_{c} u(c) + \vCntn(m - c)$ \\
$v_{\mathrm{c},\cntn}$ & \verb|v_{\mathrm{c},\cntn}| & \texttt{vCntn} & Continuation value function at stage exit \\
$v_{\mathrm{c},\arvl}$ & \verb|v_{\mathrm{c},\arvl}| & \texttt{vArvl} & Arrival value function ($= \vDcsn$; no pre-decision shocks) \\
...
Following the sequential approach outlined in section~\ref{subsec:JointProblem}, we solve the portfolio {\stg} numerically for the optimal $\shr$ at a vector of $\vctr{k}$ and construct an approximated optimal portfolio share function $\grave{\shr}(k)$ as the interpolating function among the members of the $\{\vctr{k},\vctr{\shr}\}$ mapping.  Having done this, we calculate a vector of values and marginal values at that grid:
\begin{equation}\begin{gathered}\begin{aligned}
...
\UnifiedNote{c⃗ = (ℰ^{\partial}_cons_noshocks(a⃗))^{−1/ρ} [EGM inversion; ℰ_cons_noshocks includes β via disc stage; tex vCntn' ≡ ℰ^{\partial}_cons_noshocks so (vCntn')^{−1/ρ} suffices]}
    \end{aligned}\end{gathered}\end{equation}
which, following \ifthenelse{\boolean{shortVersion}}{the endogenous gridpoints method (EGM)}{the procedure in subsection~\ref{subsec:egm}}, yields an approximated consumption function $\grave{\mathrm{c}}_{t}(m)$.
...
\hypertarget{eq-lifecyclemax}{}
  \begin{equation}\begin{gathered}\begin{aligned}
        v_{\dcsn(t)}(\mathbf{p}_{t},\mathbf{m}_{t}) & =    \max_{\{\mathrm{c}\}_{t}^{T}}~~ u(\mathbf{c}_{t})+\ExCntnPrd\left[\sum_{n=1}^{T-t}\beth^{n} \mathcal{L}_{t}^{t+n}\hat{\beta}_{t}^{t+n} u(\mathbf{c}_{t+n}) \right]   \label{eq:lifecyclemax}
        %
