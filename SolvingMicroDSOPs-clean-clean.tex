...
\\                          & = \beta {\mathbb{E}}_{\BegPrd} \left[\PermGroFacAdjV {v}_{\MidPrd}(\underbrace{a \mathcal{R}_{t} + \pmb{\theta}_{t}}_{{m}_{t}})\right]
    \end{aligned}\end{gathered}\end{equation*}
...
\begin{equation}\begin{gathered}\begin{aligned}
      {v}_{\BegPrd}(a)   & = \PermGroFacAdjV\int_{0}^{\infty} \frac{\left(a \mathcal{R}_{t}+ \TranShkEmpDummy\right)^{1-\rho}}{1-\rho}  d\mathcal{F}(\TranShkEmpDummy) \label{eq:NumDefInt}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
      v_{t}({m})  & = \max_{\{\mathrm{c},\Shr\}} ~~  u({c}) + \ExMidStg[\beta {v}_{t+1}(({m}-{c})\mathfrak{R}(\Shr) + {\pmb{\theta}}_{t+1})] \label{eq:Bellmanundated}
    \end{aligned}\end{gathered}\end{equation}
...
\begin{comment}
  ...
  The first order condition with respect to ${c}_{t}$ is almost identical to that in the single-control problem, equation (\ref{eq:upceqEvtp1}); the only difference is that the nonstochastic interest factor $R$ is now replaced by the portfolio return ${\mathfrak{R}}_{t+1}$,
    \begin{equation}\begin{gathered}\begin{aligned}
          u^{{c}}({c}_{t})  & = \beta \ExEndStg [{\mathfrak{R}}_{t+1} v^{{m}}_{t+1}({m}_{t+1})] \label{eq:valfuncFOCRtilde},
        \end{aligned}\end{gathered}\end{equation}
  and the Envelope theorem derivation remains the same, yielding the Euler equation for consumption
    \begin{equation}\begin{gathered}\begin{aligned}
          u^{{c}}({c}_{t})  & = \ExEndStg[\beta {\mathfrak{R}}_{t+1} u^{{c}}({c}_{t+1})]. \label{eq:EulercRiskyR}
        \end{aligned}\end{gathered}\end{equation}

  The first order condition with respect to the risky portfolio share is
    \begin{equation}\begin{gathered}\begin{aligned}
          0  & = \ExEndStg[{v}_{\MidStgNxt}^{{m}}({m}_{t+1})({\mathbf{R}}_{t+1}-R){a}_{t}] \notag
          \\         & = \ExEndStg\left[u^{{c}}\left(\mathrm{c}_{t+1}({m}_{t+1})\right)({\mathbf{R}}_{t+1}-R)\right]{a}_{t} 
          \\         & = \ExEndStg\left[u^{{c}}\left(\mathrm{c}_{t+1}({m}_{t+1})\right)({\mathbf{R}}_{t+1}-R)\right], \label{eq:FOCw}        
        \end{aligned}\end{gathered}\end{equation}
  ...
  \begin{equation}\begin{gathered}\begin{aligned}
        \vMidStg^{{a}}  & = \ExEndStg[\beta {\mathfrak{R}}_{t+1}{v}_{t+1}^{m}({m}_{t+1})] = \ExEndStg[\beta {\mathfrak{R}}_{t+1}{u}_{t+1}^{{c}}(\mathrm{c}_{t+1}({m}_{t+1}))]
      \end{aligned}\end{gathered}\end{equation}
  \begin{equation}\begin{gathered}\begin{aligned}
        \vMidStg^{\Shr}  & = \ExEndStg[\beta ({\mathbf{R}}_{t+1}-R){v}_{t+1}^{m}({m}_{t+1})  ]a_{t} = \ExEndStg[\beta ({\mathbf{R}}_{t+1}-R){u}_{t+1}^{{c}}(\mathrm{c}_{t+1}({m}_{t+1}))  ]a_{t} \notag
      \end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \vEndStg^{{a}}({a}_{t},\Shr_{t})  & = \beta \left(\frac{1}{n_{{\mathbf{r}}} n_{\pmb{\theta}}}\right)\sum_{i=1}^{n_{\pmb{\theta}}}\sum_{j=1}^{n_{{\mathbf{r}}} }\mathfrak{R}_{i,j} \left(\mathrm{c}_{t+1}(\mathfrak{R}_{i,j}{a}_{t}+\pmb{\theta}_{i})\right)^{-\rho}
      \\      \vEndStg^{\Shr}({a}_{t},\Shr_{t})  & = \beta \left(\frac{1}{n_{{\mathbf{r}}} n_{\pmb{\theta}}}\right)\sum_{i=1}^{n_{\pmb{\theta}}}\sum_{j=1}^{n_{{\mathbf{r}}} }({\mathbf{R}}_{i,j}-R)\left(\mathrm{c}_{t+1}(\mathfrak{R}_{i,j}{a}_{t}+\pmb{\theta}_{i})\right)^{-\rho}.
    \end{aligned}\end{gathered}\end{equation}
...
\begin{equation}\begin{gathered}\begin{aligned}
      \tilde{v}_{{t_\cntn}}({a}_{t})  & = \max_{\Shr_{t}} ~~  \vEndStg({a}_{t},\Shr_{t})
      \\      & \text{s.t.} \nonumber
      \\      0 \leq & \Shr_{t} \leq 1
    \end{aligned}\end{gathered}\end{equation}
